...
The Health Check policy monitors your API by making GET requests to the specified API base path. Requests are made continually at the specified minute intervals with the first request occuring at the specified minute interval after the policy is applied. You must specify a GET method, but any GET method is acceptable.
...
You are required to create an alert for the Health Check policy. Applying the policy navigates you to the Create Alert page, where you must create an alert. For information on how to configure your alert, see xref:api-manager::add-api-alert-task.adoc[Add API Alerts].
...
|Response code  | The expected status code of the GET request specified in the base path | Yes
|Schedule | The interval in minutes between health checks | Yes
|===
