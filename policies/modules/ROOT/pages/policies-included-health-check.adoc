...
The Health Check policy monitors the upstream health of an API at specified intervals. Health check allows you to monitor if an API is active and allows you to create an alert if the status of the API changes.

The Health Check policy monitors your API by running a GET function on the API

You are required to add an alert for the health check policy.
...
To overwrite any other existing message header, remove it before adding your header. See the xref:policies::policies-included-header-removal.adoc[Header Removal] policy.
====
...
|Upstream URL  | List of headers to be injected at the beginning of the message processing.
|Base path  | The HTTP path that is requested during health checks
|Response code  | String or Dataweave expression to be used as header value at the beginning of the message.
