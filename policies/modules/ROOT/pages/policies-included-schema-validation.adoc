...
+
For example, if the `pattern` keyword is specified, the policy validates the property is a regular expression.
...
- policyRef:
  ...
  config:
    ...
    strictHeadersValidation: <boolean> // OPTIONAL
----
...
| `strictHeadersValidation`
| Optional
| `false`
| Determines whether to validate header parameters not defined in specification.
