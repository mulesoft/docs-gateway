...
* Block the request then log and return the error.
* Allow the request then log the error.
...
| Block request | Select to block an incoming request that does not conform to the API specification. Not selecting only logs the the request. | No | No|
