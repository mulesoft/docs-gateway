...
The Schema Validation (OAS3) policy validates request headers, queries, or path parameters.
...
| Block request | Select to block an incoming request that does not conform to the API specification. Not selecting only logs the the request. Choosing to block invalid requests returns a 400 status when the request is invalid. | Block enabled | No|
