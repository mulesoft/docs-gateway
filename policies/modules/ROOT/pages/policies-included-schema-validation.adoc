...
>s|Returned Status Codes| 400 - The request properties are not valid.
|===
...
Flex Gateway supports schemas defined in JSON or YAML, contained in a file or in an external URI. Additionally, when applying the policy in Flex Gateway Connected Mode, the platform automatically transforms RAML into OAS3.

To publish an OAS3 specification for an API via API Designer, see xref:design-center::design-publish.adoc[Publishing API Specifications].
