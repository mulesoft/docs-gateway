...
| LLM Token Based Rate Limit ^| xref:gateway::policies-included-llm-token-rate-limit.adoc[✓] ^| | Limits LLM Proxy usage based on the number of tokens consumed (OpenAI-format APIs)
...
| In-Task Authorization Code ^| xref:gateway::policies-outbound-a2a-intask-authorization-code.adoc[✓] ^| | Ensures secondary credentials are present for in-task authentication using OAuth 2.0 Authorization Code flow
