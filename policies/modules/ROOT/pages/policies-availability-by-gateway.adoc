...
| External Processing ^| xref:gateway::policies-included-external-processing.adoc[✓] ^|  ^| | Sends the incoming HTTP requests or the outgoing HTTP responses to an external gRPC service for additional processing
