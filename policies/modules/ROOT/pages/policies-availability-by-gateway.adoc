...
| OAuth 2.0 OBO Credential Injection ^| xref:gateway::policies-outbound-oauth-obo.adoc[✓] ^| | Exchanges incoming bearer tokens using OAuth 2.0 Token Exchange (RFC 8693), Microsoft Entra ID On-Behalf-Of, or OAuth 2.0 Token Exchange with CIBA
...
| Regex Prompt Guard ^| xref:gateway::policies-included-regex-prompt-guard.adoc[✓] ^| | Blocks LLM requests that match deny-list regex patterns
