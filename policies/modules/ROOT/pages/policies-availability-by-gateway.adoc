...
| A2A Agent Card ^| xref:policies-included-a2a-agent-card.adoc[✓] ^| | | Rewrites the Agent card URL to match the server instance public URL

| A2A PII Detector ^| xref:policies-included-a2a-pii-detector.adoc[✓] ^| | | Detects personally identifiable information (PII) in requests sent to the agent

| A2A Prompt Decorator ^| xref:policies-included-a2a-prompt-decorator.adoc[✓] ^| | | Decorates prompts with context information

| A2A Schema Validation ^| xref:policies-included-a2a-schema-validation.adoc[✓] ^| | | Validates Agent requests to ensure they conform to the A2A specification
...
| MCP Attribute-Based Access Control ^| xref:policies-included-mcp-attribute-access-control.adoc[✓] ^| | | Controls access to tools, resources, and prompts based on user information such as Tiers, IP, Headers, or Claims

| MCP Schema Validation ^| xref:policies-included-mcp-schema-validation.adoc[✓] ^| | | Validates MCP requests to ensure they conform to the MCP specification

| MCP Support ^| xref:policies-included-mcp-support.adoc[✓] ^| | | Adds MCP support to a Flex Gateway MCP server instance. This policy is required for your MCP server instance to function properly.
...
| SSE Logging ^| xref:policies-included-sse-logging.adoc[✓] ^| | | Logs every SSE event while streaming
