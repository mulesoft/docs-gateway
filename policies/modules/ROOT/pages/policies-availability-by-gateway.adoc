...
| A2A Token Based Rate Limit ^| xref:gateway::policies-included-a2a-token-rate-limit.adoc[✓] ^| | Limits API usage based on the number of GPT-4o-mini tokens consumed by the agents

| LLM Token Based Rate Limit ^| xref:gateway::policies-included-llm-token-rate-limit.adoc[✓] ^| | Rate limits LLM Proxy usage based on token consumption

| Regex Prompt Guard ^| xref:gateway::policies-included-regex-prompt-guard.adoc[✓] ^| | Blocks LLM requests when prompt content matches configured deny-listed regex patterns
...
| Credential Injection API Key ^| xref:gateway::policies-outbound-api-key.adoc[✓] ^| | Injects an API key or client credentials headers into outgoing requests
...
| Credential Injection JWT Generation ^| xref:gateway::policies-outbound-jwt-generation.adoc[✓] ^| | Generates and injects a JWT token into outgoing requests

| OAuth 2.0 OBO Credential Injection ^| xref:gateway::policies-outbound-oauth-obo.adoc[✓] ^| | Exchanges incoming bearer tokens using the OAuth 2.0 Token Exchange (RFC 8693) or Microsoft Entra ID On-Behalf-Of protocols
...
| DataWeave Body Transformation ^| xref:gateway::policies-included-dataweave-body-transformation.adoc[✓] ^| ^| Transforms request or response body content with a DataWeave script

| DataWeave Headers Transformation ^| xref:gateway::policies-included-dataweave-headers-transformation.adoc[✓] ^| ^| Transforms request or response headers with a DataWeave script

| DataWeave Request Filter ^| xref:gateway::policies-included-dataweave-request-filter.adoc[✓] ^| ^| Filters requests by using a DataWeave script
...
| Injection Protection ^| xref:gateway::policies-included-injection-protection.adoc[✓] ^| ^| Scans request headers, path, query, and body to protect against SQL, XSS, and custom injection attacks in incoming requests
...
| A2A In-Task Authorization Code ^| xref:gateway::policies-outbound-a2a-intask-authorization-code.adoc[✓] ^| | Ensures secondary credentials are present for in-task authentication using OAuth 2.0 Authorization Code flow
...
| MCP Global Access ^| xref:gateway::policies-included-mcp-global-access.adoc[✓] ^| | Restricts which MCP tools are exposed by defining Allow and Block rules

| MCP PII Detector ^| xref:gateway::policies-included-mcp-pii-detector.adoc[✓] ^| | Blocks elicitation responses containing personally identifiable information (PII) from reaching MCP servers
...
| MCP Tool Mapping ^| xref:gateway::policies-included-mcp-tool-mapping.adoc[✓] ^| | Renames MCP tool names in requests and responses to provide flexible tool naming
...
| Outbound Message Logging ^| xref:gateway::policies-outbound-message-logging.adoc[✓] ^| ^| Logs custom messages from outbound requests sent to upstream services, responses from upstream services, or information from other outbound policies applied to the same upstream service
...
| SOAP Schema Validation ^| xref:gateway::policies-included-soap-schema-validation.adoc[✓] ^|  ^| Validates incoming traffic against a specified WSDL schema
...
| XML Threat Protection ^| xref:gateway::policies-included-xml-threat-protection.adoc[✓] ^| xref:mule-gateway::policies-included-xml-threat-protection.adoc[✓] ^| Protects against malicious XML in API requests
