...
| Response Timeout ^| xref:gateway::policies-included-response-timeout.adoc[✓] ^| | | Sets a maximum duration Flex Gateway waits for a response from an upstream service
...
| Stream Idle Timeout ^| xref:gateway::policies-included-stream-idle-timeout.adoc[✓] ^| | | Sets a maximum duration a stream can remain idle without receiving a request or response
...
| Upstream Idle Timeout ^| xref:gateway::policies-outbound-upstream-idle-timeout.adoc[✓] ^| | | Sets a maximum duration a stream between Flex Gateway and a upstream service can remain idle
