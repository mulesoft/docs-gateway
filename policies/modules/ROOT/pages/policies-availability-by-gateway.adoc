...
| A2A In-Task Authentication ^| xref:gateway::policies-included-a2a-intask-authentication.adoc[✓] ^| | Handles in-task authentication for A2A agents by extracting OAuth 2.0 tokens from request bodies and returning authentication challenges when tokens are missing
