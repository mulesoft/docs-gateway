...
Telstra starts developing a custom policy by first creating a xref:policies::policies-custom-getting-started.adoc[custom policy archetype project].
...
For more information about how to set up of the YAML file, see xref:policies::policies-custom-mule-4-reference#yaml-configuration-file[YAML Configuration File].
...
+
In this case, they need only the xref:policies::policies-custom-http-transform.adoc[http-policy-transform-extension], which helps them to easily modify the response information.
+
...
Note that the pieces of code in double braces ({{}}) correspond to xref:policies::policies-custom-mule-4-reference#handlebars[Handlebars] syntax, which uses a template engine to compose the policy template with the configuration defined in the YAML file. The values used to configure the template must be the keys defined earlier in the YAML configuration file. Both values must match. Misspelling in any of the parameters can generate errors.
