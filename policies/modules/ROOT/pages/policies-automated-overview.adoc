...
Configuring and using automated policies enables your API instance to comply with common security and logging requirements by automatically applying the same set of policies to all APIs running in Flex Gateway, or in Mule. With policy automation, API owners can quickly design, build, and deploy secure and consistent APIs.
...
All of the xref:policies-included-directory.adoc[included policies], and any xref:policies::policies-custom-overview.adoc[custom policy] can be configured as automated policies and can be applied at all the API’s running under an environment.

== Automated Policies in Mule

The following rules apply to automated policies in Mule:
