...
HTTPS uses encryption to protect the communication between the client and Flex Gateway. To use HTTPS as your schema, bind the Transport Layer Security policy to your API instance. Flex Gateway supports both TLS and mutual authentication TLS (mTLS).
...
When configuring the `ciphers` parameter, see <<local-cipher, TLS Cipher Support on Flex Gateway>> for the supported ciphers.
