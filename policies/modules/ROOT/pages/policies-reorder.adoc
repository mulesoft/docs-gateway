...
In Mule Gateway and Flex Gateway Connected Mode, you can configure the order of execution of included and custom API level policies via *API Manager* if it is necessary for certain policies to execute before others. However, you cannot configure API level policies to execute before automated policies. The following steps demonstrate how to reorder API level policies.

In Flex Gateway Local Mode, you configure policy ordering via a `PolicyBinding` YAML resource. For information about Local Mode policy order, see the xref:gateway::flex-local-configuration-reference-guide.adoc#policy-binding[Declarative Configuration Reference Guide].
