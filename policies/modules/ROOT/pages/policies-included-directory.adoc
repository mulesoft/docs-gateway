...
| xref:policies::policies-included-tls.adoc[Transport Layer Security (TLS) - Inbound] ^| ✓ ^| | Enables authentication between a client and the proxy
| xref:policies::policies-included-tls-outbound.adoc[Transport Layer Security (TLS) - Outbound] ^| ✓ ^| | Enables authentication between the proxy and an upstream service
