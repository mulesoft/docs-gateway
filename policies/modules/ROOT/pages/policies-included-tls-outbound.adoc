...
You can configure an outbound mTLS context to enable authentication between an API proxy and an upstream service, by binding the Transport Layer Security policy to your API instance. Flex Gateway supports both TLS and mutual authentication TLS (mTLS).
...
When configuring the `ciphers` parameter, ensure that your upstream supports the listed ciphers. For the supported ciphers, see <<local-cipher, TLS Cipher Support on Flex Gateway>>.
