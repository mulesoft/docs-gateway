...
>s| First Flex version available | v1.2.0
...
Distributed Rate-Limiting requires Shared Storage to be enabled. For more information about Shared Storage, refer to xref:gateway::flex-conn-shared-storage-config.adoc[Configuring Shared Storage for Flex Gateway in Connected Mode].
