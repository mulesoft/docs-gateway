...
. From the side navigation panel, click *Policies*.
. Click the more options button (image:more-options-menu.png[1%,1%]) of the policy you want to upgrade, and then click *Edit configuration*.
. Select the *latest* version for *Policy version*.
. Click *Save*.
. Click the more options button (image:more-options-menu.png[1%,1%]) of the policy you want to upgrade, and then click *Check for implementation updates*.
+
[NOTE]
====
You must manually check for implementation version updates when upgrading only upgrading the *patch* version of a definition (for example, 1.0.0 to 1.0.1). For *minor* and *major* version upgrades (for example, 1.0.0  to 1.1.0 or 1.0.0 t0 2.0.0), the implementation updates automatically.
====
. Click *Update all*.
