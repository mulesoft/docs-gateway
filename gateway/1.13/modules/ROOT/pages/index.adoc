...
A single Omni Gateway can support up to {gateway-api-max} backend APIs. For high availability, it is best to deploy multiple gateways running in parallel. Deploying multiple gateways increases gateway performance and robustness, and is recommended for high-performance applications that must scale.
...
Starting in Omni Gateway v1.13.0, WebSocket support is available as a dedicated API instance type in API Manager.
You can create WebSocket instances from HTTP or AsyncAPI assets, and configure upstream protocols using `ws` or `wss`.
...
* <<managed, Managed Omni Gateway>>
* <<self_managed, Self-Managed Omni Gateway>>
...
* <<connected_mode,Self-Managed Omni Gateway in Connected Mode>>
* <<local_mode,Self-Managed Omni Gateway in Local Mode>>
...
You can deploy Managed Omni Gateway on CloudHub 2.0 and Runtime Fabric using Runtime Manager. For details see xref:flex-gateway-managed-set-up.adoc[].
...
+
To learn more, see https://blog.heroku.com/mastering-api-integration-salesforce-heroku-mulesoft-anypoint-flex-gateway[Mastering API Integration: Salesforce, Heroku, and MuleSoft Anypoint Omni Gateway].
...
For information about Omni Gateway version retirement dates, refer to https://www.salesforce.com/en-us/wp-content/uploads/sites/4/documents/legal/Agreements/versioning-back-support-policy.pdf[MuleSoft Product Versioning and Back Support Policy].
...
See xref:general::usage-reports.adoc[Viewing Usage Reports] for information about your monthly Omni Gateway usage.
