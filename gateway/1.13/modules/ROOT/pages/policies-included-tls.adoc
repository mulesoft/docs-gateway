...
>s|Release Notes| xref:release-notes::flex-gateway/policies/tls-release-notes.adoc[]
...
[NOTE]
====
The following document applies only to Omni Gateway running in Local Mode. To configure TLS in Connected Mode, see xref:conn-tls-config.adoc[].
====

== Summary
Omni Gateway supports inbound Transport Layer Security (TLS) and inbound mutual authentication TLS (mTLS) in Local Mode.

To apply inbound TLS in Local Mode, use the xref:local-tls-config.adoc[Configuring TLS Context for Omni Gateway in Local Mode] tutorial and refer to the following configuration files in this documentation.
