...
>s|Release Notes| xref:release-notes::gateway/policies/rate-limiting-sla-release-notes.adoc[]
...
Distributed Rate-Limiting requires Shared Storage to be enabled. For more information about Shared Storage, refer to xref:conn-shared-storage-config.adoc[].
