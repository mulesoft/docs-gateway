...
When *Fail Open* is enabled, Azure API errors are logged but traffic continues without moderation. The `x-flex-azure-guardrails-reason` header is still set to `service_unavailable` for observability, allowing you to detect partial-coverage moderation in monitoring systems.
