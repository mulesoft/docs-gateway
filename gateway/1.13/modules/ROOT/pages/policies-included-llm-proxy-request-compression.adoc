...
In an ongoing agent conversation, each message and each tool result is added to the conversation history. To keep context, a client or runtime might re-sends the whole history with each new request. Every time it does, the provider charges input tokens for that history again. Compressing those repeated strings reduces the input tokens.
