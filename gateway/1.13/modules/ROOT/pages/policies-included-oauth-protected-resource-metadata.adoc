...
IMPORTANT: Only apply this policy to the `/.well-known/oauth-protected-resource` route. To apply resource level policies, see xref: 

Requests to paths that don't match the well-known segment receive a `404 Not Found` response. Non-GET requests to the well-known path receive a `405 Method Not Allowed` response with an `Allow: GET` header.
