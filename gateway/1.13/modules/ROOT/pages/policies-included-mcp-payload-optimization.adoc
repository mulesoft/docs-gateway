...
>s|Release Notes| xref:release-notes::gateway/policies/mcp-payload-optimization-release-notes.adoc[]
...
. *Smart Response Trimming* - Uses a Model proxy to reduce large responses to match the agent's intent, extracting only the information the agent requires.
...
| Enable Smart Response Trimming | When enabled, a Model proxy distills large tool responses down to match the caller's declared intent (the `expected_info` of the request), reducing tokens returned to the model.For more information, see <<smart-response-trimming>>. | Disabled
| Model Proxy URL | The endpoint of the Model proxy service used for Smart Response Trimming (for example, `http://llmproxy.svc.cluster.local/v1/chat/completions`). Must be an OpenAI-compatible `/chat/completions` endpoint. Required when Smart Response Trimming is enabled. | N/A
| Model Proxy ID | Identifier of the selected Model Proxy asset, stored for reference. This value is parsed and retained but not used at runtime by the policy. | N/A
| Model-Based Routing | Determines how the Model proxy selects the target model. When disabled (default), the proxy owns model selection (Semantic Routing). When enabled, the caller selects the model via the `LLM Model` parameter (Model-Based Routing). | Disabled
...
| Client ID | Client ID for Model proxy authentication. Sent as a `client_id` request header. | N/A
| Client Secret | Client secret for Model proxy authentication. Sent as a `client_secret` request header. | N/A
...
| LLM Timeout (ms) | Timeout in milliseconds for each Model proxy request. If the proxy exceeds this timeout, the call fails and only completes the *Clean Payloads* step. | `10000`
...
[start=5]
. *Intent Extraction* — A Model proxy distills the sanitized content to match the agent's declared intent, reducing token count further.
...
Smart Response Trimming uses a Model proxy to distill large tool responses to match the agent's declared intent. When enabled, the policy:

. Applies the Token Sanitization transforms (HTML to Markdown, Strip Base64, Collapse Whitespace, Remove Nulls).
. Checks if the sanitized content exceeds the *Threshold Bytes* parameter (default: 8000 bytes) and includes `expected_info`
. Sends the sanitized content to the configured Model proxy.
. Replaces the original content with the LLM extracted response if it's smaller than the sanitized version.

Smart Response Trimming:

* Requires a Model proxy endpoint configured via *Model Proxy URL*.
...
* Uses client credentials (*Client ID* and *Client Secret*) for Model proxy authentication, sent as separate `client_id` and `client_secret` request headers (not HTTP Basic Auth).
