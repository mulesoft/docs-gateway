...
>s|Release Notes| xref:release-notes::gateway/policies/message-logging-release-notes.adoc[]
...
image::apim-message-logging-policy-ui.png["A form with text fields and checkboxes to set up new logging configurations"]
...
The Message Logging policy supports the xref:policies-dataweave-support.adoc[standard DataWeave Expressions supported by Omni Gateway policies] and the following additional xref:dataweave::dataweave-variables-context.adoc[`vars`] DataWeave expressions. Format the following expressions `#[vars.<expressions>]`, for example `#[vars.orgId]`:
