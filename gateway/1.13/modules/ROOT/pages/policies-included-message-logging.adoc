...
>s|Release Notes| xref:release-notes::flex-gateway/omni-gateway-policies/message-logging-release-notes.adoc[Message Logging]
...
The Message Logging policy supports the xref:policies-flex-dataweave-support.adoc[standard DataWeave Expressions supported by Omni Gateway policies] and the following additional xref:dataweave::dataweave-variables-context.adoc[`vars`] DataWeave expressions. Format the following expressions `#[vars.<expressions>]`, for example `#[vars.orgId]`:
