...
:page-aliases: api-manager::policies-policy-level.adoc, api-manager::resource-level-policies-about.adoc, api-manager::resource-level-policy-reference.adoc, policies::policies-resource-level-overview.adoc
...
For Omni Gateway running in Connected Mode, you configure resource-level policies in API Manager.

For information about configuring resource-level policies for Omni Gateway running in Local Mode, refer to xref:flex-local-secure-api-with-auto-policy.adoc[].

// In Mule 4, resource-level policies support HTTP-based APIs in addition to RAML APIs.
// You can apply multiple conditions to filter your resources and HTTP methods using the xref:policies-resource-level-config-uri-regex.adoc[URI template regex] to any number of methods in your API.
...
//
// image::rlp-conditions.png["A form that enables users to apply configurations to particular API methods and resources"]
...
// * xref:policies-resource-level-disable-outbound.adoc[]
