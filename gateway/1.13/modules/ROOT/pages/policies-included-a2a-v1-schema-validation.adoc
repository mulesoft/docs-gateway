...
When validation fails, the policy returns an error response in the same transport format as the request, with appropriate HTTP status codes (400 for invalid parameters, 404 for unknown methods).
...
This policy is designed for A2A v1.0 traffic only. If you need to support pre-v1.0 agents, use the xref:policies-included-a2a-schema-validation.adoc[A2A Schema Validation] policy instead. Do not apply both policies to the same agent.
...
Apply the policy with default validation rules:
...
kind: PolicyBinding
metadata:
  name: a2a-v1-schema-validation
spec:
  targetRef:
    kind: ApiInstance
    name: my-a2a-v1-agent
  policyRef:
    name: a-two-a-v1-schema-validation-flex
    namespace: default
----
