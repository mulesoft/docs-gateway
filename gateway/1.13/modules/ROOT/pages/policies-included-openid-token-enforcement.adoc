...
The values are *Contains all scopes* and *Contains any scope*. | For *Contains all scopes*, the policy accepts tokens with both the `READ` and `WRITE` scope. For *Contains any scopes*, the policy accepts tokens with at least one scope. If no scopes are define for the *Scopes* parameter, the policy doesn't validate scopes regardless of what you select for *Scope Validation Criteria*.
