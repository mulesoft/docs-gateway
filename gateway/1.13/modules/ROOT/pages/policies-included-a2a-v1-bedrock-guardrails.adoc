...
ApplyGuardrail caps synchronous calls at approximately 25,000 characters. Larger payloads are rejected (when fail-open is disabled) or passed through without moderation (when fail-open is enabled).
...
*gRPC pass-through:* gRPC traffic (`application/grpc` or `application/grpc+proto`) is *not moderated* in v1 and passes through unchanged. When gRPC traffic is detected, the policy emits observability headers (`x-flex-bedrock-guardrails-action: skip` and `x-flex-bedrock-guardrails-reason: unsupported_transport_v1`) to monitor the coverage gap.
...
IMPORTANT: DataWeave selectors bind to the raw request body as received on the wire. For JSON-RPC, A2A parameters are under `payload.params.*`. For HTTP+JSON, they're at the top level (`payload.*`). Design your selectors to handle the transports you support.
