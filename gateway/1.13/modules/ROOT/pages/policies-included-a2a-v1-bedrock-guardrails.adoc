...
When the full URL is provided, the policy automaticall derives the region, and guardrail identifier unless explicitly configured.
...
| Moderate Response
| No
| When enabled (default: `true`), evaluates the entire unary response body against the guardrail before returning to the client. Rejected responses return `403` error code. Streaming responses (`text/event-stream`) aren't moderated and pass through unchanged.
