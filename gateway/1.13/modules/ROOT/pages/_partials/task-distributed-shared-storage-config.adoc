...
Distributed shared storage isn't persistent and data is lost when all replicas go down (scaled to zero replicas). For persistent shared storage, xref:{page-mode}-shared-storage-config.adoc[configure Redis shared storage].
