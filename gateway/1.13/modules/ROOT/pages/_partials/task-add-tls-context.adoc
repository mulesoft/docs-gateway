...
:page-aliases: task-flex-add-tls-context.adoc
...
Before adding a TLS context to Omni Gateway, complete the following tasks:
...
* *Keystore* +
If necessary for your configuration, from the drop-down list, select the keystore to store in the TLS context. Only PEM type keystores are visible. The keystore contains the certificate Omni Gateway presents to the remote party for both inbound and outbound TLS.
+
[NOTE]
====
...
====
* *Truststore* +
If necessary for your configuration, from the drop-down list, select a truststore to store certificates trusted by the client. Only PEM type truststores are visible. The truststore contains the CA path that Omni uses to validate the remote party certificate for both inbound and outbound TLS. 
+
[NOTE]
====
