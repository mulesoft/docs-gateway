...
[[yaml-empty]]
If the `registration.yaml` file is invalid when you pass it to the Helm command during deployment of Omni Gateway, the following error results:
...
[[cluster-unreachable]]
If your Kubernetes cluster is not running when you attempt to deploy Omni Gateway using `helm`, the following error might occur:
