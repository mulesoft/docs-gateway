//tag::intro1[]
Enable https://redis.io/[Redis^] shared storage for distributed caching, gateway configuration caching (Connected Mode), rate limiting policies.

Configure Redis shared storage if persistent storage is required when all replicas go down (scaled to zero replicas). If persistent shared storage isn't required, xref:flex-{page-mode}-distributed-shared-storage-config.adoc[configure distributed shared storage].
...
. Create a folder in the `/etc/mulesoft/flex-gateway/conf.d` directory and name it `storage`:
+
[source,ssh]
----
sudo mkdir /etc/mulesoft/flex-gateway/conf.d/storage
----
...
[source,ssh]
----
sudo touch /etc/mulesoft/flex-gateway/conf.d/storage/shared-storage-config.yaml
----
...
[source,ssh]
----
sudo vi /etc/mulesoft/flex-gateway/conf.d/storage/shared-storage-config.yaml
----
+
