//tag::intro[]
Omni Gateway runs on the following Long Term Support (LTS) versions of Linux:
//end::intro[]
...
When deploying Omni Gateway to a Kubernetes cluster that enables role-based access control (RBAC), you must have cluster-level permissions to permit installation of custom resource definitions (CRDs) used by Omni Gateway. 
