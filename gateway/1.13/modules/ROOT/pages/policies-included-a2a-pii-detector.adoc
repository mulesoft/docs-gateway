...
The policy scans the following A2A  JSON-RPC methods:
...
Within those requests, the policy scans three field paths:

1. `params.message.parts[]` — every text part
2. `params.task.parts[]` — every text part
3. `params.task.description` — string value

A "text part" is a JSON object whose `kind` or `type` field equals `"text"`. The text is read from the part's `text` field. Parts with `kind` or `type` of `file`, `data`, or any other value are skipped.

The scan is short-circuit: the first field that produces a detection decides the action for the whole request.

For request processing:

* If *Action* is *Reject*, the policy blocks the request and returns HTTP 403 with a JSON-RPC 2.0 error response. The upstream never receives the request.
...
The policy passes through the following without scanning:

* Non-POST requests
* Requests without a JSON Content-Type
* Bodies that do not parse as JSON-RPC
...
The following tables show where raw PII values may appear based on the configured action.
