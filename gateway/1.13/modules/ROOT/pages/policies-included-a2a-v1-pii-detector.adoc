...
This example shows how the policy logs PII detections based on the configured action.

Sample request containing multiple PII types:
...
When the action is set to *Log*, the policy logs detected PII with actual values:

[source,text]
----
[WARN] Request: `Contact me at john.doe@example.com or call (555) 123-4567. My SSN is 123-45-6789.` contains PII: Email: john.doe@example.com, Phone Number: (555) 123-4567, US SSN: 123-45-6789
----
...
[source,text]
----
[WARN] Request: `Contact me at ***@example.com or call ***. My SSN is ***.` contains PII: Email: ***, Phone Number: ***, US SSN: ***
----

=== Reject Action

When the action is set to *Reject*, the policy blocks the request and returns an error response. No log entry is created for the request. A policy violation is reported for monitoring purposes.
