...
Distributed Rate Limit requires shared storage to be enabled. Configure either xref:flex-conn-distributed-shared-storage-config.adoc[distributed shared storage] or xref:flex-conn-shared-storage-config.adoc[Redis shared storage].
