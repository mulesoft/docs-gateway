= Update Pod Settings for an Omni Gateway Deployment Through a Helm Chart
:page-aliases: flex-gateway-k8-change-helm-settings.adoc
...
Register and deploy Omni Gateway to a Kubernetes cluster. For guidance, see xref:gateway-set-up.adoc[] or xref:gateway-k8-getting-started.adoc[].
...
After following the steps in <<update-cpu-memory-example>>, use the `--set` option to pass new values individually to the Helm chart for your Omni Gateway deployment. Alternatively, follow the procedure in <<use-helm-f-option>>.
...
After following the steps in <<update-cpu-memory-example>>, use the `-f` option with `helm upgrade` to identify the file that contains new Helm chart settings for your Omni Gateway deployment. Alternatively, follow the procedure in <<use-helm-set-option>>.
