...
* *Distributed shared storage*: Enables multiple gateway replicas to share state across a distributed deployment. Data is lost after gateway restarts.
* *Redis shared storage*: Provides persistent storage that survives gateway restarts. Requires https://redis.io/[Redis^] (minimum version 4.0.0).
* *Local shared storage*: In-memory implementation. All data is lost when Omni Gateway is stopped or restarted.
...
[%collapsible]
====
The local storage service is an implementation of a REST API object store that saves data in memory. All data is lost when Omni Gateway is stopped or restarted.
