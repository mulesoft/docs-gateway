...
* Denied topics
* Word filters (profanity and custom terms)
...
* **Response phase** — Moderates LLM responses before they reach the client, ensuring outputs comply with safety policies. Note that streaming responses (`text/event-stream`) are not moderated.
...
| Yes
| Amazon Bedrock Runtime endpoint URL. Must match your AWS region.
...
Find this in AWS Console → Amazon Bedrock → Guardrails.

| Guardrail Version
| No
| Guardrail version to apply.

* `DRAFT` (default): Uses the working draft version for development and testing
* Numeric version (for example, `"1"`, `"2"`): Pins to a specific published version for production deployments

| Moderate Request
| No
| When `true` (default), evaluates the user prompt against the guardrail before forwarding to the upstream LLM. Rejected prompts never reach the LLM.

| Moderate Response
| No
| When `true` (default), evaluates the LLM response against the guardrail before returning to the client. Rejected responses return HTTP 403.
...
*Important:* Must match the region in the Bedrock Runtime Endpoint URL.
...
* `false` (default): Rejects the request with HTTP 503
* `true`: Allows traffic to proceed unmoderated (fail-open mode)
...
The policy supports independent evaluation in two phases:

1. **Request Phase** (when `moderateRequest` is enabled):
   * Extracts the user prompt from the request
   * Sends the prompt to Bedrock's `ApplyGuardrail` API
   * If the prompt violates any guardrail policies, blocks the request with HTTP 403
   * If the prompt passes, forwards the original request to the upstream LLM

2. **Response Phase** (when `moderateResponse` is enabled):
   * Intercepts the LLM response
   * Sends the response to Bedrock's `ApplyGuardrail` API
   * If the response violates any guardrail policies, returns HTTP 403 to the client
   * If the response passes, forwards the original response to the client
   * *Note:* Streaming responses (`text/event-stream`) are skipped and pass through unmoderated

This two-phase approach mirrors AWS Bedrock's INPUT and OUTPUT scopes, allowing you to apply different filter strengths to prompts versus responses when configuring the guardrail in AWS.

=== Supported Guardrail Features
...
2. Configure both `groundingSourceSelector` and `groundingQuerySelector` in the policy
...
Both must be configured together. Configuring only one causes the Bedrock API to return HTTP 400.
...
4. **Guardrail ID** from the AWS Console (Bedrock → Guardrails)
