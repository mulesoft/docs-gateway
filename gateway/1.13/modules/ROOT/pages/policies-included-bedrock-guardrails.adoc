...
The policy supports independent evaluation in for request and response. The two phases mirror AWS Bedrock's INPUT and OUTPUT scopes, enabling you to apply different filter strengths to prompts versus responses when configuring the guardrail in AWS:
...
. Configure both `groundingSourceSelector` and `groundingQuerySelector` in the policy
