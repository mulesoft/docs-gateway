...
. The policy checks if sufficient tokens remain in the bucket for this key
. If tokens are available, the request proceeds to the backend
. Response tokens are counted and billed to the same bucket
. If the bucket is exhausted, subsequent requests are rejected with HTTP 429 until the window resets
