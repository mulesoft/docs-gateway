...
For Omni Gateway, the CRDs are `APIinstance`, `Service`, `PolicyBinding`, and `Configuration` and are defined in the xref:flex-local-configuration-reference-guide.adoc[].
...
For more information about how to configure Omni Gateway as an Ingress Controller, see xref:flex-gateway-k8-ingress-class.adoc[].
