...
You can block specific introspection fields independently. By default, Omni Gateway allows all three fields(`blockSchema`, `blockType`, and `blockTypename` each default to `false`). The policy affects traffic only if you set one or more fields to `true`.
...
| No
| `false`
| If `true`, Omni Gateway returns a 403 error when the operation references the `__schema` meta-field anywhere in the document.

| `blockType`
| No
| `false`
| If `true`, Omni Gateway returns a 403 error when the operation references the `__type` meta-field.

| `blockTypename`
| No
| `false`
| If `true`, Omni Gateway returns a 403 error when the operation references the `__typename` meta-field.
