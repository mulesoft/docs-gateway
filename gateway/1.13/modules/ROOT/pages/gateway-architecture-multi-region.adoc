...
:page-aliases: flex-architecture-multi-region.adoc, architecture-multi-region.adoc
...
NOTE: The implementations below discuss Redis shared storage considerations. You can also use distributed shared storage, which doesn't require cross-region synchronization. However, data is lost when all replicas go down (scaled to zero replicas).
