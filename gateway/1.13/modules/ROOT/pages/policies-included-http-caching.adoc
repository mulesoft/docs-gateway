...
>s|Release Notes| xref:release-notes::flex-gateway/omni-gateway-policies/http-caching-release-notes.adoc[HTTP caching]
...
Self-Managed Gateways require you to configure a shared storage for distributed caching. For more information about shared storage, refer to xref:flex-conn-shared-storage-config.adoc[Configuring Shared Storage for Omni Gateway in Connected Mode].
