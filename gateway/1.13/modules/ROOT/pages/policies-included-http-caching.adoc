...
For Self-Managed Gateways, HTTP caching requires that you <<configure-shared-storage-for-self-managed-gateway, configure shared storage>>. For Managed Gateways, Omni Replica shares the cache with each other directly. To learn more about the Managed Gateway architecture, see xref:flex-architecture-managed-dr-ha.adoc[].
...
Self-Managed Gateways require you to configure shared storage for distributed caching. Configure either xref:flex-conn-distributed-shared-storage-config.adoc[distributed shared storage] or xref:flex-conn-shared-storage-config.adoc[Redis shared storage].

The following example uses distributed shared storage. Redis shared storage can also be used.
...
The cache works as a dictionary, where each response stored in the cache is associated with a string called _key_. For example, the expression #[attributes.headers['key']] uses the header called `key` as the entry key. You can use any DataWeave expression that returns a string, including xref:policies-flex-dataweave-support.adoc#available-predefined-variables[predefined DataWeave variables].
