...
>s|Release Notes| xref:release-notes::gateway/policies/header-injection-release-notes.adoc[]
...
image::header-inject-remove-diagram.png["A flow chart illustrating the process of setting header injection and removal policies for clients accessing an API through a proxy"]
...
To use single quotes directly in a String value (for example `"default-src 'none'; frame-ancestors 'none';"`), either:

* Use a DataWeave expression:
+
----
