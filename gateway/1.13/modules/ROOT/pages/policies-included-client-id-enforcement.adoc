...
>s|Release Notes| xref:release-notes::gateway/policies/client-id-enforcement-release-notes.adoc[]
...
Client ID Enforcement policy isn't supported in Local Mode.
...
In this example, the requester must send the two specified query parameters with the request. Although this is a supported configuration, it poses possible security risks. The recommended method is to use headers. A xref:policies-dataweave-support.adoc[subset] of DataWeave 2.0 is supported.
