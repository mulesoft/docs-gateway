= Automating Omni Gateway with a Jenkins Pipeline using the API Manager API
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images
:page-mode: conn
:page-aliases: flex-conn-jenkins-api.adoc, gateway::flex-conn-manage-jenkins.adoc

You can use a Jenkins Pipeline to automate the workflow for Omni Gateway in Connected Mode. With a Jenkins Pipeline, you can register and run an Omni Gateway and use features of the API Manager API, such as creating and deploying API instances.

Automating your Omni Gateway in Connected Mode work flow shares many similarities with the xref:conn-manage-public-api.adoc[Manage Omni Gateway Using the API Manager API] tutorial. You can use the Jenkins Pipeline to automate the calls made to the API Manager API. To add additional features from the  https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/api-manager-api/[API Manager API^], use the <<create-an-api-instance, Create an API Instance>> and <<apply-an-api-policy, Apply an API Policy>> Pipeline stages as examples.
...
NOTE: To instead automate your Omni Gateway with the Anypoint CLI, see xref:conn-jenkins-cli.adoc[].
...
** *Organization ID* for the organization where you want to run Omni Gateway
+
For more information, see xref:access-management::organization.adoc#find-your-organization-id[Find Your Organization ID].

** *Environment ID* for the environment where you want to run Omni Gateway
+
...
Before you can create and deploy an API instance to your Omni Gateway, you must complete the following steps:
...
* `<your-org-id>`: *Organization ID* for the organization where you want to run Omni Gateway
* `<your-env-id>`: *Environment ID* for the organization where you want to run Omni Gateway
* `<gateway-version>`: Version of Omni Gateway you are using, for example `1.4.0`
...
* `<gateway-name>`: Name of your Omni Gateway
...
After registering and running your Omni Gateway, you must obtain a valid authorization token from Access Management in order to authenticate subsequent requests.
...
Having an authorization token from Access Management enables you to create an API instance and deploy it to your Omni Gateway. 
...
== Stop the Omni Gateway and archive the registration file

Use the following code blocks to stop your Omni Gateway and archive the registration file. Add the `post` block in the `pipeline` block after the `stages` block:
...
* xref:conn-reg-run-app.adoc[Registering and Running in Connected Mode with a Connected App].
...
* xref:conn-manage-public-api.adoc[Manage Omni Gateway Using the API Manager API]
