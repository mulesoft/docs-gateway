...
Omni Gateway includes a `flexctl dump` CLI that enables you to query for information related to a gateway's API instances, services, configuration, and available extensions (policies).
...
----
+
The `extensions.json` file contains all the policies available in Omni Gateway. The file contents resemble the following:
+
----
{
    "extends": [
