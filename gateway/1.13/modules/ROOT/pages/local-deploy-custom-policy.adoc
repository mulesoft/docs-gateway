= Deploying a Custom Policy for Omni Gateway in Local Mode
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images
:page-aliases: flex-local-deploy-custom-policy.adoc
...
. Save the file in the Omni Gateway configuration directory.
...
. Apply the resources to the Omni Gateway instance.
...
* Your upstream service URL. See xref:local-publish-simple-api.adoc[] for an example.
* A custom policy implemented and compiled. See xref:policies-custom-implement-rust.adoc[].

== Deploy a Custom Policy

. Create a configuration file called `custom-policy-example.yaml`.

. Save the file in the Omni Gateway configuration directory. For example:
+
...
* xref:local-configuration-reference-guide.adoc[]
