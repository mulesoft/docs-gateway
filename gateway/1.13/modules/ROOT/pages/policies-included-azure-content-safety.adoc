...
| Yes
| Azure Content Safety endpoint base URL. Example: `https://<resource>.cognitiveservices.azure.com`

| Azure API Key
| Yes
| Azure Content Safety subscription key (sent as the `Ocp-Apim-Subscription-Key` header)
...
| Moderate Response
| No
| When enabled, evaluates the LLM response against Azure Content Safety before returning to the client. Rejected responses return `403` error code.

*Note:* Streaming responses (`text/event-stream`) are not moderated and pass through unchanged.

Default: enabled

| Default Severity Threshold
| No
| Severity threshold applied to all four harm categories (Hate, SelfHarm, Sexual, Violence). Content with severity at or above this value is rejected. Supported values are *0*, *2*, *4*, and *6*.
...
| API Version
| No
| Azure Content Safety API version used for all endpoint calls.
...
| No
| Per-category override for the Hate harm category. Leave as "default" to inherit Default Severity Threshold. Supported values are *default*, *0*, *2*, *4*, and *6*.

| SelfHarm Severity Threshold
| No
| Per-category override for the SelfHarm harm category. Leave as "default" to inherit Default Severity Threshold.

| Sexual Severity Threshold
| No
| Per-category override for the Sexual harm category. Leave as "default" to inherit Default Severity Threshold.

| Violence Severity Threshold
| No
| Per-category override for the Violence harm category. Leave as "default" to inherit Default Severity Threshold.
...
| No
| DataWeave expression resolving the grounding source text from the request body. Required when Groundedness Detection is enabled.

Example: `#[payload.context.documents[0].content]`
...
| API Timeout (ms)
| No
| Per-request timeout for each Azure Content Safety API call in milliseconds. Must be between 1000 and 30000.
