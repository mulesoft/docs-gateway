...
The GraphQL Operation Limits policy restricts the depth, aliases, root fields, and directives in incoming requests. If an operation exceeds a limit, Flex Gateway returns a 400 error with a JSON `errors` array detailing each violation.
