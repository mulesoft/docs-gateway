...
The MCP Tools Progressive Disclosure policy *must be positioned first* in the MCP filter chain, before any policy that reads tool names (xref:policies-included-mcp-attribute-access-control.adoc[], xref:policies-included-mcp-tool-mapping.adoc[], xref:policies-included-mcp-schema-validation.adoc[]).

If positioned after an xref:policies-included-mcp-attribute-access-control.adoc[access control policy], that policy would see `invoke_tool` (the meta-tool) in the request, not the real tool name.
