...
The policy tracks before and after token counts in the `_mcp_token_metrics` field on responses:
...
NOTE: These metrics are estimates (~4 bytes per token).
