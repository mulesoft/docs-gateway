= Implement an Omni Gateway Custom Policy in Rust
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:page-aliases: policies-custom-flex-implement-rust.adoc, policies::policies-custom-flex-implement-rust.adoc
...
+
Omni Gateway serializes policy configuration into JSON, which is used to configure the Envoy WASM filter. You can deserialize and parse this JSON into data that will be used by your custom policy.
...
The following tutorial describes how to create an example policy implementation for a xref:policies-custom-getting-started.adoc#create-policy-definition-files[Policy Definition] that has already been published in Exchange.
...
Example policy implementations require a policy definition JSON file. Omni Gateway passes policy configuration as JSON with that policy definition structure to the Envoy WASM filter.
...
----
+
Omni Gateway configures your policy with JSON defined in the policy definition. The JSON that needs to be parsed contains a single field called `secret-value`.
+
...
[IMPORTANT]
====
Some libraries might compile properly to the wasm32-wasi target but might not work properly when deployed to Omni Gateway. Example deployment errors when this happens are:
...
+
For more information on pushing custom policies to Exchange, refer to xref:policies-custom-getting-started.adoc[Publish an Omni Gateway or Mule 4 Custom Policy].
