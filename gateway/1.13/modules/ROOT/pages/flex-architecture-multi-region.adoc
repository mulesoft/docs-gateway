...
NOTE: The implementations below discuss Redis shared storage considerations. You can also use distributed shared storage can also be used, which doesn't require cross-region synchronization. However, data is lost when all replicas go down (scaled to zero replicas).
