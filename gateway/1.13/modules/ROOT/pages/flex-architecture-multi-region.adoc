...
NOTE: The implementations below discuss Redis shared storage considerations. Distributed shared storage can also be used, but data is lost when all replicas go down (scaled to zero replicas) and doesn't require cross-region synchronization.
