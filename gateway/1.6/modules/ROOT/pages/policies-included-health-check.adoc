...
The Health Check policy enables you to monitor an API instance's connection to an upstream service and sends you an email alert if the connection status changes. The Health Check policy only monitors the health of the upstream service and not the health of the API instance or Flex Gateway.
