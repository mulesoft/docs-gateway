...
NOTE: A high CPU can lead to increased latency. Proceed with horizontal scaling or increase the node size. High memory consumption may lead to out-of-memory errors. Increase the memory allocation or the node size. Horizontal scaling can fail helping in high-memory situations.
