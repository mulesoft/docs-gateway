...
[NOTE]
The `route` policy is required only when defining `PolicyBinding` entities in resource-based configurations. For instance, using `spec.services.routes` in an `ApiInstance` entity, either inline or as a resource, does not require defining the `route` policy.
