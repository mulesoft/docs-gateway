...
To allow routing requests to a `Service` when an `ApiInstance` does not include `spec.services`, you must define a `PolicyBinding` configuration with the `route` policy. You do not need to configure a `route` policy if a custom policy is managing requests.
...
For information about disabling log output, refer to xref:flex-local-disable-logs.adoc[]
