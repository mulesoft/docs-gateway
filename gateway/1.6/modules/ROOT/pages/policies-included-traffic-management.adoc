...
Flex Gateway running in Local Mode supports API instances that expose multiple upstream services through a single consumer endpoint. For information about exposing a single upstream service, refer to xref:policies-included-traffic-management-single.adoc[].
