...
| Optional
| NA
| Specifies the headers from the authorization response to add to the client request to the upstream service. Flex Gateway overwrites existing headers. This parameter is only available for `HTTP` type serves.
