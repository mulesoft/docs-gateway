...
The External Processing policy sends the incoming HTTP request or outgoing HTTP response to an external gRPC service to modify the headers, trailers, or body. You can configure this policy to process either the request, response, or both.
...
image:ext-proccessing-policy.png[80%,80%]