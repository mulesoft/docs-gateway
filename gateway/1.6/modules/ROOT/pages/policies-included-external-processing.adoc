...
| Required
| `none`
| Mode the request body is sent in. Supported values are `none`, `streamed`, `buffered`, and `bufferedPartial`.

| `responseBodyMode`
| Required
| `none`
| Mode the response body is sent in. Processing resSupported values are `none`, `streamed`, `buffered`, and `bufferedPartial`.
