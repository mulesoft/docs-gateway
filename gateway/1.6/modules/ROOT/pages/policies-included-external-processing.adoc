...
NOTE: If other policies modify the HTTP request or response, ensure that you order the policies depending on if these modification should be present when the request is sent to the processing service or backend service. For more information about ordering policies, see xref:policies-reorder.adoc[].
