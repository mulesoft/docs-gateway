...
The External Processing policy sends the incoming HTTP requests or outgoing HTTP response to an external gRPC service to modify the request's or response's header, trailers, or body. You can configure this policy to process either the request, response, or both.
