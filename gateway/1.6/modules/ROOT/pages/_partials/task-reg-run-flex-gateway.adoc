...
When you install Flex Gateway in Kubernetes, a service type `LoadBalancer` is created by default. You must have the appropriate permissions to create a load balancer in your cloud. If the load balancer is not provisioned or has issues with the provisioning process, you must choose by changing the `service.type` property during the installation.
