...
You can configure both included and custom policies to execute as automated-policies or API-level policies. Automated policies are policies that are applied to all API instances deployed on the Flex Replica, and API-level policies are polices that are only applied to a specific API instance.
...
For example, if in API Manager you have Automated Policy 1, Automated Policy 2, API-Level Policy 1, API-Level Policy 2 ordered:
