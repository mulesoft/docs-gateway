...
Anypoint Flex Gateway enables you to deliver runtime and access logging to any supported Fluent Bit {fluentbit-ver-var} output type. You configure logs for Flex Gateway running in local mode via a configuration YAML file described in this page, as well as in the xref:flex-local-configuration-reference-guide.adoc#logging[Declarative Configuration Reference Guide].

This tutorial only demonstrates how to configure `File` or `HTTP` output types. To configure of third party Fluentbit outputs, see xref:flex-local-third-party-logs-config.adoc[].
...
In addition to configuring logging for Fluent Bit `File` output, you can also configure for Fluentbit `HTTP` output. As an example, the following definition specifies a `spec.logging.outputs.type` value of `http`:
...
Flex Gateway also records standard output logs seprate from the logs you configured in this tutorial. To view the standard output logs, run the following commands:
