...
Anypoint Flex Gateway enables you to deliver runtime and access logging to any supported Fluent Bit {fluentbit-ver-var} output type. You configure logs for Flex Gateway running in local mode via a configuration YAML file described in this page, as well as the xref:flex-local-configuration-reference-guide.adoc#logging[Declarative Configuration Reference Guide].
