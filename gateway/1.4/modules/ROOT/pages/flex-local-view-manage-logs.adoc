...
Anypoint Flex Gateway enables you to deliver runtime and access logging to any supported Fluent Bit {fluentbit-ver-var} output type. You configure log output via a custom YAML file. This enables you to configure `File` or `HTTP` logging outputs. This tutorial only demonstrates how to configure `File` or `HTTP` Fluent Bit output types. To configure different Fluent Bit output types for third party services, see xref:flex-local-third-party-logs-config.adoc[].

As well as being able to configure additional Fluent Bit logging output types, Flex Gateway maintains standard output,`flex-gateway-fluent`, runtime and access logs you do not need to configure. Use the standard output logs for quick logging checks where an additional logging output is not needed. To view the standard output logs, see xref:flex-local-view-logs[View Logs].
