= Configure Message Logging for Flex Gateway in Local Mode
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images

Anypoint Flex Gateway enables you to deliver runtime and access logs to any supported Fluent Bit {fluentbit-ver-var} output type, `File` or `HTTP` log outputs. You configure log output via a custom YAML file.

This tutorial only demonstrates how to configure `File` or `HTTP` Fluent Bit output types. To configure different Fluent Bit output types for third party services, see xref:flex-local-third-party-logs-config.adoc[].

As well as being able to configure additional Fluent Bit log output types, Flex Gateway maintains standard output,`flex-gateway-fluent`, runtime and access logs you do not need to configure. Use the standard output logs for quick log checks where an additional log output is not needed. To view the standard output logs, see xref:flex-local-view-logs[View Logs].
...
== Runtime Logs
Runtime logs contain information such as when Flex Gateways start and when API instances are deployed.

== Access Logs
Access logs contains information such as API requests.
