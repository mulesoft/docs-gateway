...
You can configure more detailed logs to troubleshoot problems by configuring `spec.logging.runtimeLogs.logLevel` to `debug`. As an example, the following definition specifies a `spec.logging.runtimeLogs.logLevel` value of `debug`:
