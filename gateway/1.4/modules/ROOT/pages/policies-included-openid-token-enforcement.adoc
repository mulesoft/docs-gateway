...
To control the time taken to cache the tokens, before you attempt revalidating against the token validation endpoint, configure the following parameter in the `wrapper.conf` file:
...
To control the number of tokens that can be cached simultaneously, configure the following parameter in the `wrapper.conf` file.
