...
. In your terminal window, copy the command block into your `/flex-registration` directory, and provide a _unique_ name for your Flex Gateway container before running the command: 
.. In the copied command, notice `<gateway-name>` at the end of the command block, and replace that text with a name of your choice, such as `my-gateway`.
+
The name that you provide for your Flex Gateway container must be unique within the scope of your Anypoint Platform organization and environment. Completing the registration step is required once _and allowed no more than once_ per the named container and organization.
+
----
docker run --entrypoint flexctl -u $UID \
   -v "$(pwd)":/registration mulesoft/flex-gateway \
   register --_YOUR_ORGANIZATION_HERE_ \
   --token=_ACTIVE_TOKEN_VALUE_HERE_ \
...
[source,curl]
----
$ curl http://your_external_url:80/users
----
+
JSON Output (edited):
+
...
----
+
When this error occurs, check the `registration.yaml`. Make sure the file is not empty. If the file is empty, re-register your Flex Gateway container with a new, unique name to generate a new `registration.yaml` (see <<register-flex>>), and rerun the remaining procedures in this guide. 
