...
Get started with Anypoint Flex Gateway on a Kubernetes cluster. Set up a Flex Gateway deployment that connects to Anypoint Platform, and use the deployment to route traffic to your API. This guide covers the following high-level tasks:

* Downloading, registering, and deploying Flex Gateway in Connected Mode. 
* Creating a simple API in Anypoint API Manager that runs on your gateway. 
* Protecting the API with a basic authentication policy that you configure for it in API Manager. 
...
//TODO: VERIFY Manage Servers and Read Servers permissions in RTM NO LONGER NEEDED

//os prereqs
* A Linux or MacOS machine with an x86 processor. M1 and M2 processors are not supported.
//TODO: VERIFY not Windows x86 -- pending testing 
...
+
The deployment process creates a resource of this type by default. To avoid deployment errors, your cluster must support this process.
...
To determine whether your cluster is ready for the <<deploy-flex, deployment procedure>>, it is important to review <<check-cluster>> _before starting_ the deployment. The checks require you to run a set of `kubectl` commands.
...
----
+
If you receive an error after executing this command, see the information on <<docker-troubleshooting,Docker troubleshooting>>.  
...
. In your terminal window, copy the command block from Runtime Manager into your `./flex-registration` directory.
+
...
----
+
The command either adds the repository or skips this process if a Helm repository with that name exists already on your machine:
+
...
Confirm that your Kubernetes cluster is available by following the steps in <<check-cluster>>. Review <<k8-cluster-reqs>> if you have not done so already. These prerequisites help avoid deployment errors. 
...
----
+
The `REVISION` increments the `ingress` release by `1` each time you run this command with the same namespace, repository, and chart names. For example, if you run the command a second time, you find a new revision number (`REVISION: 2`). The `LAST DEPLOYED` date reflects the date of that revision.
+
If you receive an `UPGRADE FAILED` response, see <<upgrade-failed,troubleshooting>> information about this issue. 

* A new gateway is running in your terminal window.
...
.. Navigate to your registered gateway in the *Flex Gateways* page that opens:
+
...
+
If the status is *Disconnected*, it is likely that the cluster configuration is incorrect or there is a network issue. Check <<troubleshooting>> for more information.
...
+
The remaining procedures walk you through steps for creating and testing a new API that is running on your gateway. Notice, however, that it is also possible to select an existing API from Exchange from this page. That process is described in the xref:api-gateway::create-instance-task.adoc#add-a-new-api[Add a New API] within the API Manager documentation.
+
...
+
Inside the API instance, all HTTP requests to the external IP associated with your gateway are proxied to the `jsonplaceholder` service.
...
+
.. Run the following `curl` command, replacing `_your_external_ip_` with _your_ `EXTERNAL-IP` from the preceding step:
+
[source,curl]
----
curl -s -o /dev/null -w "%{http_code}\n" --request GET 'http://_your_external_ip_:80/users'
----
+
...
[source,curl]
----
$ curl http://_your_external_ip_:80/users
----  
...
+
If the field is pre-populated with a value, you can keep the value or change it if you need to protect it.
+
...
kubectl -n gateway get services
----  
.. Run the following `curl` a command _without_ authentication parameters, replacing `_your_external_ip_` with the `EXTERNAL-IP` returned by preceding step:
+
[source,curl]
----
curl -s -o /dev/null -w "%{http_code}\n" --request GET 'http://_your_external_ip_:80/users'
...
----
+
.. Run the following `curl` a command _with_ authentication parameters, replacing `_your_external_ip_` with your `EXTERNAL-IP` from the preceding step:
+
[source,curl]
----
curl -s -o /dev/null -w "%{http_code}\n" --request GET 'http://_your_external_ip_:80/users' -u user:pw
----
+
...
[[daemon-bad-response]]
* If Docker is not started, a `docker` command returns an error message similar to these:
+
----
...
+
To avoid this issue, provide a _unique name_ for your gateway when running the <<reg-command, registration command>>. Note that you _cannot reuse the same name_ even if a previous registration with that name was successful. When this error occurs, the command generates an _empty_ `registration.yaml`, which prevents re-registration _and_ produces an error when you try to use the file when deploying the gateway to your cluster. 

[[no-token]]
** `no valid registration token was found`
+
To avoid this issue, generate a new token for the command by refreshing the *Add a Flex Gateway* page for Kubernetes and rerun the <<reg-command, registration command>>.


[[helm-troubleshooting]]
=== Helm Command Issues

Troubleshoot errors returned by a `helm` command:

[[cluster_unreachable]]
* If your Kubernetes cluster is not running when you attempt to deploy Flex Gateway using `helm`, the following error can occur:
+
----
...
+
You can check that your Kubernetes cluster host is running your cluster, for example:
+
[source,kubectl]
----
...
----
+
For additional information, see <<check-cluster>>.

[[upgrade-failed]]
* Some issues can cause the Helm upgrade command to hang for 5 minutes (due to the `--wait` flag in the command) and produce the following timeout error:
+
----
Error: UPGRADE FAILED: timed out waiting for the condition
----
+
This error typically means that your cluster configuration is incorrect or that your cluster configuration tool does not support this process. See <<k8-cluster-reqs>>. One potential cause is the failure to generate a `Service` resource of the `LoadBalancer` type. You can troubleshoot further by running the following commands:
...
* If the `registration.yaml` file is invalid when you pass it to the Helm during deployment of Flex Gateway, the following error results:
+
----
...
----
+
When this error occurs, check the `registration.yaml`. Make sure the file is not empty. If the file is empty, re-register your gateway with a new, unique name to generate a new `registration.yaml` (see <<register-flex>>) and rerun the procedures that follow the registration step. 

//TODO: this was from Israel. Not clear what upgrade error and 
//      why we would uninstall the ingress for the namespace. 
//Israel: "If you receive an upgrade error when execute the command, you can  
//         remove the ingress released chart or omit the released argument:"
//
//## Command to remove ingress chart released
//helm -n <namespace> uninstall ingress
//
// TODO: WHEN WOULD WE OMIT -i --create-namespace --wait?
//## Command without install ingress chart arguments
// helm -n <namespace> upgrade \
//ingress flex-gateway/flex-gateway \
//--set-file registration.content=<registration_file>

[[check-cluster]]
== Check Your Cluster

Before deploying Flex Gateway to your Kubernetes cluster in <<deploy-flex>>, you must make sure that your cluster is ready. To check, you can use `kubectl` to list and select (use) a cluster and to get a list of nodes that the cluster uses. If any steps in this section fail, the deployment process is likely to produce errors.

To prevent potential timeout and connectivity errors when running procedures in this guide, your cluster must meet the <<k8-cluster-reqs>>. 
