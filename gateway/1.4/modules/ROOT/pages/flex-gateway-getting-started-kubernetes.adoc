...
* Protecting the API with a basic authentication policy that you configure for the API in API Manager
...
//os prereqs
* A Linux or MacOS machine with an x86 processor. M1 and M2 processors are not supported for running Flex Gateway.
//TODO: Determine whether they can use M1 and/or M2 for the guide without errors or workarounds
...
This procedure is a prerequisite to registering the Flex Gateway image in <<register-flex>> and _is not_ used by the gateway deployment through Helm in <<deploy-flex>>. 
...
+
The command uses `--set gateway.mode=connected` because the default for the Helm chart is `--set gateway.mode=local`.
+
...
. Check the status of your new pod by running the following `kubectl` command:
+
[source,kubectl]
----
...
+
If the status is *Disconnected*, it is likely that the cluster configuration is incorrect or a network issue occurred. Check <<troubleshooting>> for more information.
...
+
The remaining procedures walk you through steps for creating and testing a new HTTP API in Anypoint Exchange. Notice, however, that it is also possible to select an existing API from Exchange from this page. That process is described in the xref:api-gateway::create-instance-task.adoc#add-a-new-api[Add a New API] in the API Manager documentation.
+
.. Provide an API name, such as *my-api*.
.. Select *HTTP API* as the asset type.
+
Though this guide focuses on a simple HTTP API, you can find HTTPS configuration details, see xref:flex-conn-tls-config.adoc[].
...
+
The example in this guide assumes an external IP that uses port `80` instead of the default port, `8081`. 
...
----
+
When this error occurs, make sure that the `registration.yaml` file is not empty. If the file is empty, reregister your gateway with a new, unique name to generate a new `registration.yaml` (see <<register-flex>>) and rerun the procedures that follow the registration step. 
...
. If you have not started the Flex Gateway setup procedures yet, be sure to review <<k8-cluster-reqs>> before proceeding to <<download-flex>>.
