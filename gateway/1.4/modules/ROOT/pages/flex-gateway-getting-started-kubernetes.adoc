...
+
The example in this guide uses HTTP port `80` instead of the API Manager default port, `8081` because the created `Service` created maps the incoming port `80` to the target port `80` on the Pod. See the `service.http.port` key in the https://artifacthub.io/packages/helm/flex-gateway/flex-gateway[Helm chart for Flex Gateway^].
