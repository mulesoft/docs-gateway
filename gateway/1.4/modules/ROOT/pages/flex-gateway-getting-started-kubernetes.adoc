...
* Downloading, registering, deploying Flex Gateway in Connected Mode. 
...
----
+
If you receive an error that includes `ERROR: Cannot connect to the Docker daemon`, troubleshoot your Docker installation.
...
Register the Flex Gateway image from a terminal window using a temporary token generated by Anypoint Runtime Manager. This procedure produces a local `registration.yaml` file that contains registration properties for a named gateway. You will incorporate these properties into a Helm chart in <<deploy-flex>>.
...
Use Helm to deploy a Flex Gateway to a node in your cluster and connect it to Anypoint Platform. After deploying, use Runtime Manager to verify that the gateway is present and connected to Anypoint Platform. 
...
. Run the Helm command for deploying your Flex Gateway and connecting it to Anypoint Platform. 
+
[source,helm]
----
...
----
+
The `EXTERNAL-IP` value in the example is placeholder and not an actual value.
+
