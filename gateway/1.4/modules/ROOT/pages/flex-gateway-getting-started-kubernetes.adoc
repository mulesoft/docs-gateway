...
+
To avoid deployment errors when using this guide, your cluster must support this process. However, Flex Gateway supports these services: `ClusterIP`, `NodePort`, `LoadBalancer` (default), `ExternalName`. 
...
* xref:flex-conn-node-affinity-config.adoc[]
