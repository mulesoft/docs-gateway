...
+
This requirement avoids deployment errors when you use this guide. However, it is possible to customize the `Service` type when deploying a gateway. Flex Gateway supports the following services: `ClusterIP`, `NodePort`, `LoadBalancer` (default), `ExternalName`. See the `service.type` key in the xref:https://artifacthub.io/packages/helm/flex-gateway/flex-gateway[Helm chart for Flex Gateway].
