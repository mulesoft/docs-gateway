...
* Creating a simple API in Anypoint API Manager that runs on your Flex Gateway container. 
...
To complete the procedures in this guide, you must create a Kubernetes cluster or use an existing one for your Flex Gateway deployment. In this guide, you use Helm to deploy your Flex Gateway container to a Kubernetes cluster and connect the container to Anypoint Platform. 
...
Use Docker to pull the Flex Gateway Docker image.
...
Register the Flex Gateway image from a terminal window using a temporary token generated by Anypoint Runtime Manager. This procedure produces a local `registration.yaml` file that contains registration properties for a named Flex Gateway container. You will incorporate these properties into a Helm chart in <<deploy-flex>>.
...
. Before running the command, replace `<gateway-name>` at the end of the command block with a _unique_ name for your Flex Gateway container, such as `my-gateway`.
+
The name that you provide for your Flex Gateway container must be unique within the scope of your Anypoint Platform organization and environment. Completing this registration step is required once _and allowed no more than once_ per named container and organization.
...
Create a local repository that stores a Helm chart with your Flex Gateway container's configuration. Note that you can get more information about the properties of the Helm chart for Flex Gateway in https://artifacthub.io/packages/helm/flex-gateway/flex-gateway[ArtifactHUB^].
...
----
+
This command creates the `gateway` namespace and `ingress` if they do not exist. The command syntax for the Helm repository and chart names is `<helm-repo-name>/<helm-chart-name>`. 
+
...
[[bad-request]]
* The following error can occur when you use `docker run` to register your Flex Gateway container:
+
----
...
+
To avoid this issue, provide a _unique name_ for your Flex Gateway container when running the <<reg-command, registration command>>. Note that you _cannot reuse the same name_ even if a previous registration with that name was successful. When this error occurs, the command generates an _empty_ `registration.yaml`, which prevents re-registration _and_ produces and error when you try to use the file when deploying the container to your cluster. 
...
* If the `registration.yaml` file is invalid when you pass it to the Helm during deployment of the Flex Gateway container, the following error results:
+
----
...
----
+
When this error occurs, check the `registration.yaml`. Make sure the file is not empty. If the file is empty, re-register your Flex Gateway container with a new, unique name to generate a new `registration.yaml` (see <<register-flex>>) and rerun the procedures that follow the registration step. 
...
Before deploying a Flex Gateway container to your Kubernetes cluster in <<deploy-flex>>, you must make sure that your cluster is ready. To check, you can use `kubectl` to list and select (use) a cluster and to get a list of nodes that the cluster uses. If any steps in this section fail, the deployment process is likely to produce errors.
...
//TODO: SHOULD I replace NAME AND VERSION data with made-up data?
