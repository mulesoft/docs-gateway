...
+
To avoid deployment errors when using this guide, your cluster must support this default process. However, note that Flex Gateway supports these services: `ClusterIP`, `NodePort`, `LoadBalancer` (default), `ExternalName`. 
...
. If you want to check the status of the Pod that contains your gateway, run the following `kubectl` command:
+
[source,kubectl]
----
