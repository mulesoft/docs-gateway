...
+
The name that you provide for your Flex Gateway container must be unique within the scope of your Anypoint Platform organization and environment. Completing the registration step is required once per the named container and organization.
+
----
docker run --entrypoint flexctl -u $UID \
   -v "$(pwd)":/registration mulesoft/flex-gateway \
   register --organization=346d7416-8f46-4256-8e45-c85d10787232 \
...
+
To avoid this issue, provide a unique name for your Flex Gateway container when running the <<reg-command, registration command>>.
