...
* Minimal compute resources required for the deployment procedure are no greater  (and _likely far less than_) the following equivalent to EKS t3.micro size: 
...
+
This process generates a Kubernetes `Service` resource of the `LoadBalancer` type by default. To avoid deployment errors, your cluster must support this process.
...
+
image::gateway-quickstart-k8-api-config-api.png["Screenshot of 'API' page",width=100%]
+
.. Click *Create an API*.
...
+
image::gateway-quickstart-k8-api-config-endpoint.png["Screenshot of 'Endpoint' page",width=100%]
+
...
----  
+
The command returns a result that looks similar to this one:
+
----
...
+
.. Run the following `curl` a command, replacing `your_external_ip` with the `EXTERNAL-IP` from the preceding step:
+
[source,curl]
----
...
+
To see from the *Implementation URI*, you can also run a simpler `curl` command to return an array of JSON objects, for example: 
+
[source,curl]
----
...
+
image::gateway-quickstart-add-policy.png["Add Policy page with Add policy button",width=100%]
. In the page that opens, find the *Security* policy called *Basic Authentication - Simple* policy.
+
image::gateway-quickstart-k8-basic-auth.png["Screenshot of 'Add a policy' page"]
+
. Click *Next*.
. Configure a Basic Authentication policy:
+
image::gateway-quickstart-config-basic-auth.png["Screenshot of 'Configure Basic Authentication - Simple policy' page"]
+
...
.. Run the following `curl` a command _without_ authentication parameters, replacing `your_external_ip` with the `EXTERNAL-IP` from the preceding step:
+
[source,curl]
----
curl -s -o /dev/null -w "%{http_code}\n" --request GET 'http://your_external_ip:80/users'
----
+
The command returns an error similar to this one because the URI lacks authentication parameters:
+
----
...
----
+
To address the error, return to the policy configuration page for your API in in API Gateway, and edit the policy configuration.
+
image::gateway-quickstart-k8-config-basic-auth-edit-dialogue.png["Screenshot of 'Configure Basic Authentication - Simple policy' edit dialogue"]
+
image::gateway-quickstart-k8-config-basic-auth-edit.png["Screenshot of 'Configure Basic Authentication - Simple policy' configuration page"]
