...
Port sharing is not a concern for outbound TLS. You can select the outbound TLS context for each individual upstream. 
...
Though a port must have an API instance to have an inbound TLS context applied, it is easiest to think of the inbound TLS context as being applied to the port rather than the API Instance. 
