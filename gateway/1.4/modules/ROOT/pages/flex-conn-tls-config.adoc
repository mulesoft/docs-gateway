...
Flex Gateway supports inbound and outbound TLS and mutual authentication TLS (mTLS).

TLS context configuration for a Flex Gateway Connected Mode is the same whether it is running in a Docker container, as a Linux Service,
...
. Either create a new API Instance using Flex Gateway as the runtime or edit the endpoint of an existing Flex Gate API Instance:
* To the create a new API using Flex Gateway as the runtime, complete xref:docs-api-manager::create-instance-task.adoc[Adding an API Instance] task up to the endpoint configuration page. 
* To edit an the existing existing endpoint of an API Instance, click *Runtime & Endpoint Configuration* in the *Settings* page of your API Instace. 
