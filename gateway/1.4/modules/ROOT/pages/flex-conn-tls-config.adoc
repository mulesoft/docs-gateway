...
Though the port must have an API Instance to have a TLS context applied, it is easiest to think of TLS context as being applied to a port rather than an API Instance. 
...
In API Manager, if you apply or edit the TLS context of an API Instance sharing a port, a warning with a list of the instances sharing the port is provided. You can then choose to override the TLS context of the API Instances that share the port. Overriding the TLS context of the API Instances that share the port applies the TLS context to the instances and then redeploys the instances.
