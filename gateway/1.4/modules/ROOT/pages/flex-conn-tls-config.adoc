...
+
If adding outbound TLS context, first click *Advanced options*, then to add TLS context for either traffic direction:
+
...
If you apply a TLS context to an API Instance that shares a port with another instance, a warning with a list of the API Instances that share the port is provided. If you click *Save & Apply*, a warning window appears and you can choose to override the TLS context of the API Instances that share port. Overriding the TLS context of the API Instances that share the port applies the TLS context to the instances.

Editing the port of an API Instance that shares a port with other instances and that already has a TLS context applied does not remove the TLS context from the other instances.
