...
You can deploy Flex Gateway to a Kubernetes cluster in Connected Mode or Local Mode. The choice determines whether Flex can act as an ingress controller.
...
* In Local Mode, Flex Gateway uses an ingress controller to manage an `Ingress` resource that is either created manually or created through a continuous integration/continuous development (CI/CD) pipeline by using `kubectl apply`. 
