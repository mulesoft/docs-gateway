...
| image:ad-external-non-org-mtls.png[grey hexagon with mtls written in the middle] | Non-organization external mTLS | For external communication with entities that aren't trusted, the third party organization controls the CA. The third party CA issues certificates for the organization to use when communicating with the third-party services.
|===
