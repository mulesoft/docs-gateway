...
When xref:service-mesh::obtain-connected-apps-credentials.adoc[Configure a Connected App] as part of the xref:flex-conn-reg-run.adoc#before-you-begin[register and run before you begin tasks], you must configure your connected app using the *App acts on its behalf (client credentials)* type and include the following scopes to use API Manager API:
...
Then <<obtain-an-authorization-token-from-access-management, Obtain an Authorization token from Access Management>> as the first step in using the API Manager Developer API.
