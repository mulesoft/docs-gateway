...
To automate these commands using a Jenkins pipeline, see xref:flex-conn-manage-jenkins.adoc[Automate Flex Gateway in Connected Mode Using a Jenkins Pipeline].
...
To make calls using the API Manager API, you must obtain an authorization token:
...
* `<your-proxy-uri>`: Proxy URI, a combination of your port and base path, for you API Instance, for example, 'http://0.0.0.0:<port-to-populate>/<path>'

. Optionally, collect your or remove the `"endpointUri"` field:
* `<your-consumer-endpoint>`: Consumer endpoint of you API, for example, "http://consumer-endpoint.com"
