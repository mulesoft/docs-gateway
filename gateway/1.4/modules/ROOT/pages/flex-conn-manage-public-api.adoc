...
To automate these commands using a Jenkins pipeline as well as registering a Flex Gateway, see xref:flex-conn-manage-jenkins.adoc[Automate Flex Gateway in Connected Mode Using a Jenkins Pipeline].

For additional features of the API Manager API, see the https://anypoint.mulesoft.com/exchange/portals/anypoint-platform/f1e97bc6-315a-4490-82a7-23abe036327a.anypoint-platform/api-manager-api/[API Manager API Documentation].
...
. Collect the Group ID and Asset ID from the asset URL.
+
For example, the IDs collected from the API Gateway HTTP basic authentication policy template Exchange URL, `https://anypoint.mulesoft.com/exchange/68ef9520-24e9-4cf2-b2f5-620025690913/http-basic-authentication/minor/1.3/?status=published`, is:
+
...
. Execute the following request to obtain your Authorization Token after replacing the sample content:
+
[source,ssh]
----
...
* `"configurationData"` = The necessary configuration parameters for the policy.
+ 
