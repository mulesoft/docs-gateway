...
Use the following tasks to get started monitoring Flex Gateway running in Local Mode:
...
* xref:flex-local-view-logs.adoc[View Logs] - View Flex Gateway standard output logs. Use the standard output logs for quick logging checks when additional logging output is not needed, such as ensuring when an external service is running or API requests are reaching Flex Gateway.
