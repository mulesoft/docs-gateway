...
Get started with Anypoint Flex Gateway in a Kubernetes or a Kubernetes-based, OpenShift cluster. Set up a Flex Gateway deployment that connects to Anypoint Platform, and use the deployment to route traffic to your API. This guide covers the following high-level tasks:
...
include::partial$prerequisites.adoc[tag=rbac-permission-k8;rbac-role-openshift]
