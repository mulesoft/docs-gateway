...
The metrics avaliable in API Manager are considered *Key Metrics* and are described here. Use xref:monitoring::api-analytics-dashboard.adoc[Anypoint Monitoring] to view Flex Gateway API metrics not included in the *Key Metrics* section.

To see an API instance in Anypoint Monitoring, from the *Key Metrics* section click *View more metrics in Anypoint Monitoring dashboard*.

[NOTE]
====
The *Analytics* tab in API Manager is currently not available for Flex Gateway APIs. Use the Key Metrics or the additional metrics in Anypoint Monitoring.
====
