...
Flex Gateway supports schemas defined in JSON or YAML, contained in a single file, or in an external URI. Additionally, when applying the policy in Connected Mode, the platform automatically transforms RAML into OAS3. However, the transform might contain discrepancies.
