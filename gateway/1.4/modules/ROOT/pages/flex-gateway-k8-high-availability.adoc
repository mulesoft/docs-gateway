= Add Flex Gateway Replicas through a Helm Chart
...
include::partial$reference-k8.adoc[tags=flex-k8-helm-chart-info]
...
The update steps to follow depend on whether a Helm chart is installed. 

. Update your Flex Gateway chart to add replicas:
...
<release-name> <helm-repo-name>/<helm-chart-name> \
--reuse-values \
...
----
+
When successful, the command returns output similar to this example: 
+
----
