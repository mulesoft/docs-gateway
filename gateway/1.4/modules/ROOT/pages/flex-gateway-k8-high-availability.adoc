...
| `autoscaling.extraMetrics` | A list of custom Pod configuration metrics. When a Pod runs with these settings, you can collect and monitor application-specific metric data. Defaults to `[]`. Define the metrics with nested parameters, for example:
