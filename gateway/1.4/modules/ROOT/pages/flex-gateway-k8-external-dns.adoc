...
Use the `extraAnnotations` parameter with `helm upgrade` to set the domain names. 

. Create a YAML configuration file to specify an ExternalDNS key and a comma-separated set of domain values.
+
Replace the following placeholder values `<domain-0>,...,<domain-N>` with your domains:
+
----
extraAnnotations:
