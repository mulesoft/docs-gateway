...
Use the `extraAnnotations` parameter in `helm upgrade` to set an annotation that specifies an ExternalDNS key with your gateway domains:
+
----
--set extraAnnotations."external-dns\.alpha\.kubernetes\.io/hostname"=<domain-1>,<domain-2>...
----

The command options to use depends on whether your registered gateway is deployed to a cluster:
