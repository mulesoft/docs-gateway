...
. Add the `Check brach` and `Check parameters` stage to your Jenkinsfile:
+
[source,Jenkinsfile]
----
stage('Check branch') {
...
Add the install Anypoint CLI 4.x stage to your Jenkinsfile to install Anypoint CLI 4.x to your Docker container:
...
Use the following code blocks to archive the registration file and remove the Docker container running Flex Gateway:
