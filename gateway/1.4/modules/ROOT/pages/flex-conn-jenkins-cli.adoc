...
Anypoint CLI 4.x can take a moment to install. To validate Anypoint CLI 4.x to your Docker container, add the validate Anypoint CLI 4.x installation `stage` to the `stages` block in your Jenkinsfile. You can add this stage anywhere after the installation stage but it must come before you use any CLI:
