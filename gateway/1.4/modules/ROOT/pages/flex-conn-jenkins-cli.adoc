= Automating Flex Gateway with a Jenkins Pipeline using the Anypoint CLI
...
Automating your Flex Gateway with Anypoint CLI shares many similarities with Automating your Flex Gateway with the API Manager API. To automate your Flex Gateway using API Manger API requests, see xref:flex-conn-manage-jenkins.adoc[].
...
Add the install Anypoint CLI 4.x stage to your Jenkinsfile to download Anypoint CLI 4.x to your Jenkins node:
    ...
    steps {
        script {
            sh """
                ...
                sudo n 16.0.0
                sudo npm install -g anypoint-cli-v4
                sudo anypoint-cli-v4 plugins:install anypoint-cli-api-mgr-plugin
            """
        }
    }
}
...
You can repeat this step to add multiple API instances. For additional API configuration flags, see xref:4.x@anypoint-cli::api-mgr.adoc#api-mgr-api-manage[api-mgr:api:manage command].

Add the create API instance stage to your Jenkinsfile:
...
stage("Create API Instance") {
    steps {
        script {
            sh """
                sudo anypoint-cli-v4 api-mgr:api:manage ${params.EXCHANGE_ASSET_ID} ${params.EXCHANGE_ASSET_VERSION} \
...
. After replacing the sample content, add the apply Basic Authentication policy stage to your Jenkins Pipeline:
...
stage("Apply Basic Authentication Policy to API Instance") {
    steps {
        script {
            ...
            sh """
                sudo anypoint-cli-v4 api-mgr:policy:apply ${apiInstanceId} 'http-basic-authentication' \
                --policyVersion='1.3.2' --groupId '68ef9520-24e9-4cf2-b2f5-620025690913' \
                --client_id=${params.CONNECTED_APP_CLIENT_ID} --client_secret=${params.CONNECTED_APP_CLIENT_SECRET} \
...
stage("Deploy API") {
    steps {
        script {
            ...
            sh """
                sudo anypoint-cli-v4 api-mgr:api:deploy ${apiInstanceId} \
                --client_id=${params.CONNECTED_APP_CLIENT_ID} --client_secret=${params.CONNECTED_APP_CLIENT_SECRET} \
...
Use the following code blocks to stop your Flex Gateway and archive the registration file:
    ...
    always {
        script {
            if (fileExists('container-id')) {
                sh(script: "docker rm -f ${readFile(file: 'container-id')}")
            }
        }
    }
}
