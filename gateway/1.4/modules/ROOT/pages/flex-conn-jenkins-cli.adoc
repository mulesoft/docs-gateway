...
. <<validate-anypoint-cli-4-x-installation, Validate Anypoint CLI 4.x Installation>>
...
== Validate Anypoint CLI 4.x Installation

Anypoint CLI 4.x can take a moment to install. To validate Anypoint CLI 4.x is installed, add the validate Anypoint CLI 4.x installation `stage` to the `stages` block in your Jenkinsfile. You can add this stage anywhere after the installation stage but it must come before you use any Anypoint CLI commands:

[source,Jenkinsfile]
----
stage('Validate anypoint-cli-v4 installation') {
   steps {
       script {
           sh """
              docker run --name container-anypoint-cli-v4 -d node:16 \
