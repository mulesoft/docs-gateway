...
When configuring shared storage, MuleSoft recommends placing the Redis shared storage service as close to Flex Gateway as possible. Additionally, to increase communication speed, do not use mutual authentication Transport Layer Security (mTLS) to communicate with the Redis shared storage service. 
