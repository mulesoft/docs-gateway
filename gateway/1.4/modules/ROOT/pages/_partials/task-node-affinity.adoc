...
Each node affinity rule in the Pod spec requires the key of a node label and one or more of the label’s values (see <<set_node_affinity>>). You can use existing node labels or create new ones. A node label is defined as a key-value pair.
