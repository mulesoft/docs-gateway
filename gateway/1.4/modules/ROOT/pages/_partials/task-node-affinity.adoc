...
To select a suitable host node for a Pod that contains Flex Gateway, you can incorporate custom node affinity rules from your Pod specification (Pod spec) into the Flex Gateway Ingress Controller Helm chart. Helm packages files for related Kubernetes resources into a Helm chart. For more information about node affinity, see the https://kubernetes.io/docs/concepts/scheduling-eviction/assign-pod-node/#node-affinity[Kubernetes documentation^].
...
To incorporate the node affinity settings from your Pod spec into the Helm chart, you need to use a Helm command. https://helm.sh/docs/intro/install/[Helm^], a tool used to install Flex Gateway, monitoring tools, and applications. A minimum Helm version of 3.0.0 is required. 

The Helm command to use depends on whether Flex Gateway is installed:
