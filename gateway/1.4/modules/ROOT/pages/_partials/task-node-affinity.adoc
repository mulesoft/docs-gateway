...
The Kubernetes scheduler accepts node affinity rules, which are used to select nodes for Pods. The selection determines the node in which a Pod is deployed. 
