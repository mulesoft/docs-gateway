...
When configuring node affinity (see <<set_node_affinity>>), you must set a rule that includes the key of a node label and one or more of the label's values. You can use existing node labels or create new ones. A node label is defined as a key-value pair.
