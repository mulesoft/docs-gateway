...
Each node affinity rule in the Pod spec requires the key of a node label and one or more of the label’s values (see <<set_node_affinity>>).f You can use existing node labels or create new ones. A node label is defined as a key-value pair.
...
After adding the node affinity settings to your Pod spec, you must incorporate the spec file into the Helm chart for your Flex Gateway Ingress Controller so that the Kubernetes scheduler can use the setting. https://helm.sh/docs/intro/install[Helm^] is a tool used to install Flex Gateway, monitoring tools, and applications. A minimum Helm version of 3.0.0 is required. 
