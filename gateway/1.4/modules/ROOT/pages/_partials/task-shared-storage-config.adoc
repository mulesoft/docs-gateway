...
|image:install-kubernetes-logo.png[20%,20%,xref="flex-{page-mode}-shared-storage-config.adoc#k8"]
...
|xref:flex-{page-mode}-shared-storage-config.adoc#k8[Configure Shared Storage for Flex Gateway on a Kubernetes Cluster]
|===
