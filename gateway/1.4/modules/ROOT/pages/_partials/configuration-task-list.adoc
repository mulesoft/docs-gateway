...
* xref:flex-gateway-k8-{page-mode}-node-affinity-config.adoc[Configure Node Selection for Deployments on Kubernetes] - Flex Gateway enables you to host the gateway on a specialized node that meets specific requirements, instead of using the nodes that host other deployments.
