...
* xref:flex-{page-mode}-liveness-check.adoc[Configure a Liveness Check] - Flex Gateway enables you to run a liveness check command to check the health of a Flex Gateway. Additionally, Flex Gateways deployed on Kubernetes include a liveness probe to automatically restart pods after a specified number of failed liveness checks.
