...
//tag::sample-config-all-outbound-intro[]
*Sample configuration for adding outbound TLS context for all API Instances running on this Flex Gateway:*
//end::sample-config-outbound-all-intro[]
//tag::sample-config-outbound-all[]
[source,yaml]
----
apiVersion: gateway.mulesoft.com/v1alpha1
kind: PolicyBinding
...
spec:
  ...
  config:
    ...
    ciphers:
      ...
      - TLS_RSA_WITH_AES_256_CBC_SHA
----
//end::sample-config-outbound-all[]
