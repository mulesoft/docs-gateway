...
|image:install-kubernetes-logo.png[20%,20%,xref="flex-{page-mode}-tls-config.adoc#k8"]
...
|xref:flex-{page-mode}-tls-config.adoc#k8[Configure TLS Context for Flex Gateway on a Kubernetes Cluster]
|===
...
[[k8]]
== Configure TLS Context for Flex Gateway on a Kubernetes Cluster
