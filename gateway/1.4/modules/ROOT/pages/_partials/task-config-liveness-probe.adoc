...
Flex Gateway is considered healthy if both Envoy and Fluentbit are active. If the Flex Gateway instance is healthy, the command returns an exit code of `0`. Otherwise, it returns a `1`. 
