...
//end::intro1[]
//tag::intro2[]
The following example demonstrates how to redirect Flex Gateway logs to Splunk. However, you can use any Fluent Bit {fluentbit-ver-var} output type, such as `Azure Log Analytics`, `Amazon S3`, and `Kafka`. For a complete list of Fluent Bit output types, see https://docs.fluentbit.io/manual/v/1.8/pipeline/outputs[Fluent Bit Outputs documentation ^]. 
//end::intro2[]
//tag::icon-table[]
...
The configuration file used in the following examples demonstrates how to redirect Flex Gateway logs to Splunk.
...
Splunk begins ingesting logs soon after applying this resource. If no events appear in the Splunk dashboard, inspect the Flex Gateway `flex-gateway-fluent` standard output logs to determine if there are configuration errors.
//end::sample-config-all[]
//tag::view-logs-local[]
To view `flex-gateway-fluent` standard output logs, see xref:flex-local-view-logs.adoc#view-logs[View Logs].
//end::view-logs-local[]
