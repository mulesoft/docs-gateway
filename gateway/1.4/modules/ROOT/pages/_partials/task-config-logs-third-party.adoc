...
The following configuration file examples demonstrates how to redirect Flex Gateway logs to output types: `File`, `HTTP`, and `Splunk`. However, you can use any Fluent Bit {fluentbit-ver-var} output type, such as `Azure Log Analytics`, `Amazon S3`, `Kafka`, and more. For a complete list of Fluent Bit output types, see https://docs.fluentbit.io/manual/v/1.8/pipeline/outputs[Fluent Bit Outputs documentation^]. 

//end::intro1[]

//tag::intro2-conn[]
As well as being able to configure additional Fluent Bit log output types, Flex Gateway maintains standard output in API Manager. Use the standard output logs for quick log checks where an additional log output is not needed. To view the standard output logs, see *Anypoint Platform > API Manager > Logs*. For access logs to be visible in *Logs*, you must additionally apply a Message Logging policy.

//end::intro2-conn[]


//tag::intro2-local[]
As well as being able to configure additional Fluent Bit log output types, Flex Gateway maintains standard output logs names `flex-gateway-fluent`. Use the standard output logs for quick log checks where an additional log output is not needed. To view the standard output logs, see xref:gateway::flex-local-view-logs[View Logs].
...
Additionally, depending on your output, ensure the service where you are directing logs to is operational. For Splunk, you need to configure:
...
Runtime and access log both appear in the same log output. However, you can choose whether or not to include both logs in your log output. To use access logs, you must also configure a Message Logging policy.

=== Runtime Logs
Runtime logs include the following information:

* The startup and shutdown of Flex Gateway, Fluent Bit, and Envoy
* What APIs and policies are deployed
* Possible misconfigurations of Flex Gateway or other errors

You can configure the runtime logs loglevel.

=== Access Logs
Access logs describe incoming interaction with Envoy over a period of time. Access logs describe incoming requests and response for specific APIs. 

To enable access logging, you must: 

. Configure a log output configuration file.
. Apply a Message Logging policy the API instances you want to monitor. To apply a Message Logging policy, see <<configure-access-logs, Configuring Access Logs>>.
...
To see configuration files with applied values, see the following configuration file examples:
...
. Update the file with your desired output configuration details in the following sections:
+
* <<file-configuration-example, File Configuration Example>>
* <<http-configuration-example, HTTP Configuration Example>>
* <<splunk-configuration-example, Splunk Configuration Example>>
...
. Create and save a YAML file with your desired output configuration details in the following sections:
+
* <<file-configuration-example, File Configuration Example>>
* <<http-configuration-example, HTTP Configuration Example>>
* <<splunk-configuration-example, Splunk Configuration Example>>
...
a YAML configuration file with your desired output configuration details in the following sections:
+
* <<file-configuration-example, File Configuration Example>>
* <<http-configuration-example, HTTP Configuration Example>>
* <<splunk-configuration-example, Splunk Configuration Example>>
...
//tag::view-logs-conn[]
If no events appear in logging output, inspect the Flex Gateway `flex-gateway-fluent` standard output logs to determine if there are configuration errors.
+
To view `flex-gateway-fluent` standard output logs in API Manager, see *Anypoint Platform > API Manager > Logs*.
//end::view-logs-conn[]

//tag::view-logs-local[]
If no events appear in logging output, inspect the Flex Gateway `flex-gateway-fluent` standard output logs to determine if there are configuration errors.
+
To view `flex-gateway-fluent` standard output logs, see xref:gateway::flex-local-view-logs.adoc[View Logs].
//end::view-logs-local[]

//tag::configure-message-logging-step[]
. Configure a message logging policy if configuring access log. To configure a message logging policy, see <<access-logs, Configure Access Logs>>.
...
Splunk begins ingesting logs soon after applying this resource. 
