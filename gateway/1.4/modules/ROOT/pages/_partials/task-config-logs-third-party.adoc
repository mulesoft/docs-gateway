//tag::intro1[]
Anypoint Flex Gateway enables you to deliver runtime and access logs to any supported Fluent Bit {fluentbit-ver-var} output type. You configure log output via a custom YAML file.
...
As well as being able to configure additional Fluent Bit log output types, Flex Gateway maintains standard output,`flex-gateway-fluent`, runtime and access logs you do not need to configure. Use the standard output logs for quick log checks where an additional log output is not needed. To view the standard output logs, see xref:gateway::flex-local-view-logs[View Logs].
...
=== Access Logs
Access logs describe incoming interaction with Envoy over a period od time. Access logs describe incoming requests and response to specific APIs. 
