...
Splunk begins ingesting logs soon after applying this resource. If no events appear in the Splunk dashboard, inspect the Flex Gateway `flex-gateway-fluent` logs to determine if there are configuration errors.
//end::sample-config-all[]
//tag::view-logs-local[]
To view `flex-gateway-fluent` logs, see xref:flex-local-view-manage-logs.adoc#view-logs[Viewing and Managing Log Output].
//end::view-logs-local[]

//tag::view-logs-conn[]
To view `flex-gateway-fluent` logs in API Manager, see *Anypoint Platform > API Manager > Logs*.
//end::view-logs-conn[]