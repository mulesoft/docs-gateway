...
//tag::intro2-local[]
As well as being able to configure additional Fluent Bit log output types, Flex Gateway maintains standard output logs names `flex-gateway-fluent`. Use the standard output logs for quick checks when additional log output is not needed. To view the standard output logs, see xref:gateway::flex-local-view-logs.adoc[View Logs].
...
a YAML configuration file with your desired output configuration details as shown in the following examples:
+
* <<file-configuration-example, File Configuration Example>>
* <<http-configuration-example, HTTP Configuration Example>>
* <<splunk-configuration-example, Splunk Configuration Example>>
...
//tag::view-logs-conn[]
If no events appear in the logging output, check the Flex Gateway `flex-gateway-fluent` standard output logs for any configuration errors.
+
To view `flex-gateway-fluent` standard output logs in API Manager, go to *Anypoint Platform > API Manager > Logs*.
//end::view-logs-conn[]
