...
In Runtime Manager, you can click *&#8592; Flex Gateway* in the left navigation to find your gateway in the UI. Notice that your gateway's status is *Disconnected*. Refresh the page, if necessary.
...
// tag::k8s-flex-helm-chart-prereqs[]
include::_attributes-versions.adoc[]

Before installing, ensure that the following prerequisites are in place:

* A minimum {min-helm-version} version of https://helm.sh/docs/intro/install/[Helm^], a tool used to install:
+
** Flex Gateway in a Kubernetes cluster
** Monitoring tools
** Applications
+
Refer to <<helm-chart-options>> to review and modify the defaults. 

Add a Helm repository for the Flex Gateway chart:

// end::k8s-flex-helm-chart-prereqs[]
// tag::k8s-flex-helm-chart-add[]

. Add a repository named `flex-gateway` for your chart: 
+
[source,helm]
----
...
In Runtime Manager, you can click *&#8592; Flex Gateway* in the left navigation to find your new gateway in the UI. Notice that your gateway's current status is *Connected*. Refresh the page, if necessary.
