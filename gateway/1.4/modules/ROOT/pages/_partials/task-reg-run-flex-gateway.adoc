...
In Connected Mode, Flex Gateway typically serves as an ingress or Edge API gateway that receives traffic from outside of the cluster or as a standalone gateway for internal services. 
...
Resource creation through `kubectl apply` is possible but restricted to the following resource types:
...
Before registering your Flex Gateway instance, collect information for the registration command options: 
