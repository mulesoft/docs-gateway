...
Local Mode also supports `Ingress` resources from Kubernetes, which are configured through a YAML file with properties, such as `apiVersion`, `kind`, `metadata`, `spec` for configuring a load balancer or proxy server, and `rules` for directing HTTP and HTTPS traffic. For more information about Flex Gateway managing `Ingress` resources, see xref:flex-gateway-k8-ingress-class[].
