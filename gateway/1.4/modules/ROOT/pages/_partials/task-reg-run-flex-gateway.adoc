...
== Register and Run with a Connected App on an OpenShift Cluster
// end::app-openshift-heading[]
// tag::user-openshift-heading[]
== Register and Run with a Username and Password on an OpenShift Cluster
// end::user-openshift-heading[]
// tag::token-openshift-heading[]
== Register and Run with a Token on an OpenShift Cluster
...
Local Mode also supports `Ingress` resources from Kubernetes, which are configured through a YAML file with properties, such as `apiVersion`, `kind`, `metadata`, and `spec` for configuring a load balancer or proxy server, and `rules` for directing HTTP and HTTPS traffic. For more information about how Flex Gateway manages `Ingress` resources, see xref:flex-gateway-k8-ingress-class.adoc[].
