...
* A minimum {helm-min-version} version of https://helm.sh/docs/intro/install/[Helm^], a tool used to install:
+
** Flex Gateway in a Kubernetes cluster
** Monitoring tools
** Applications
+
Refer to <<helm-chart-options>> for the default chart settings.
...
. Deploy your gateway to a release (`ingress`) by installing the Helm chart for Flex Gateway into a namespace (`gateway`).
+
[source,kubernetes]
----
...
Before updating a Helm chart for a Flex Gateway deployment, it is useful to view the default Helm chart settings:
...
To view the Helm chart `README`, run the following command from a terminal window:
+
[source,kubernetes]
----
...
To update the default Helm settings with new values, such as `resource` values for CPU and memory, see xref:flex-gateway-k8-change-helm-settings.adoc[].
