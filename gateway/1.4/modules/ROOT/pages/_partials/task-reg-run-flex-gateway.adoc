...
To register a Flex Gateway instance with Anypoint Platform, run the Docker registration command with required options. After registering, use a single Helm command to install the Helm chart for the registered gateway and deploy the gateway to a cluster. The deployment process requires an existing cluster. See <<Before You Begin>> for more prerequisites to these procedures.
