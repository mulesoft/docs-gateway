...
In this mode, the gateway is managed mostly through Anypoint Platform, where you can: 
...
In addition, resource creation through `kubectl apply` is restricted to the following resource types:
...
//
// tag::k8s-local-intro[]
In Local Mode, Flex Gateway typically acts as an ingress controller that manages external access to your Kubernetes cluster. When using this mode, you must know what resources to create and apply, and use `kubectl apply` to deploy Kubernetes targets and resources such as APIs, policies, or Flex Gateway-related resources to your gateway. 
