...
If there is no default `IngressClass`, or if the default `IngressClass` resource shouldn't be the controller for a specific resource, you must manually define the `IngressClassName` parameter in the `Ingress` resource.
