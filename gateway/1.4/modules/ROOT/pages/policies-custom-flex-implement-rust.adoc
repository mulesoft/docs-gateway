...
+
Note: `get_http_request_header` has a known issue in Rust SDK when reading non UTF-8 values. If running into that issue, switch to `get_http_request_headers_bytes` methods and read the bytes using the `from_utf8` method (https://doc.rust-lang.org/std/string/struct.String.html#method.from_utf8). For example:
+
----
