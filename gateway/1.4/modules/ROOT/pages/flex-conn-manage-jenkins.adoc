...
. After replacing the sample content, add the parameter definitions to you Jenkinsfile:
+
[source,ssh]
----
parameters {
...
To login into Anypoint, add the `Anypoint login` stage to your Jenkinsfile:
...
. After replacing the sample content, add the `Create API Instance` stage to you Jenkinsfile:
...
To deploy the instance, add the `Deploy API` stage to your Jenkinsfile:
...
To validate if the instance is deployed, add the `Validate deployment` stage to your Jenkinsfile:
