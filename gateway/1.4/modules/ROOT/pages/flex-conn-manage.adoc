...
* xref:flex-conn-tls-config.adoc[Configure TLS and mTLS Context] - 
Using encryption, Transport Layer Security (TLS) protects communication between clients, API Instances, and upstream services. After you create a secret group and add a Privacy-Enhanced Mail (PEM) type keystore, you can add a TLS context and apply the TLS context to API Instances in API Manager to encrypt their inbound and outbound traffic.
