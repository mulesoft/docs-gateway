...
The goal of the following tutorial is to quickly get started with Anypoint Flex Gateway. Using Docker, the tutorial describes how to install, register, and run Flex Gateway in Connected Mode. Additionally, the tutorial describes how to publish an API that is secured with a basic authentication policy.
...
* A Docker installation. See https://www.docker.com/products/docker-desktop/[Docker Desktop^] for instructions.
