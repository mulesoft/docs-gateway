...
To avoid storing the Redis `password` in plaintext in the YAML configuration file, store it in your deployment environment's secret store and inject it into the shared storage configuration when the Omni Gateway container starts.
...
. In your Omni Gateway `Deployment`, mount the secret as a file and override the container command to read the password, write the shared storage configuration to a directory that Omni Gateway scans, and then start Omni Gateway. For example:
+
[source,yaml]
----
apiVersion: apps/v1
...
* The container command reads the password from `/etc/redis-secret/password`, generates `shared-storage-config.yaml` in the `/etc/mulesoft/flex-gateway/conf.d` directory, and then starts Omni Gateway (`/init`).
...
. Start the Omni Gateway container with the password file mounted, and override the command to generate the shared storage configuration before starting Omni Gateway:
+
[source,ssh]
----
docker run --rm \
...
----
+
This command mounts the password file read-only, writes `shared-storage-config.yaml` to the in-container `/etc/mulesoft/flex-gateway/conf.d` directory, and then starts Omni Gateway (`/init`). Because the generated file is written inside the container instead of to the mounted host directory, the password isn't stored in plaintext on the host.
...
Redis Sentinel provides high availability for Redis shared storage. For more information about Redis Sentinel, refer to https://redis.io/docs/latest/operate/oss_and_stack/management/sentinel/[Redis Sentinel^].
