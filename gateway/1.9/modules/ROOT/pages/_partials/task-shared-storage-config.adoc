...
The sample configurations in the previous sections include the Redis `password` in plaintext in the YAML configuration file. To avoid storing the password in plaintext, store it in your deployment environment's secret store and inject it into the shared storage configuration when the Flex Gateway container starts.

Flex Gateway reads shared storage configuration only from a file in the `conf.d` directory; you can't reference an external secret directly from the `sharedStorage` configuration. The following examples generate the configuration file from the stored secret at container startup, so the password isn't stored in plaintext on disk.
