...
Flex Gateway is designed to run in cloud-native architectures. Therefore, multiple installations on a single Linux VM aren't supported. In other words, you can install only one gateway instance per Linux VM.
...
| APIs per gateway instance | {gateway-api-max} | Exceeding this limit isn’t supported but doesn't cause errors.
