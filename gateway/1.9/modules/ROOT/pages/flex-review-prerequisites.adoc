...
=== Managed Flex Gateway on Hyperforce

Managed Flex Gateway isn't supported on Hyperforce deployments.

=== Self-Managed Flex Gateway on Hyperforce

Because Flex Gateway running in Local Mode is completely self-hosted, the gateway isn't affected by Hyperforce. However, for Flex Gateway running in Connected Mode, the API Manager Hyperforce limitations are relevant. To learn more, see xref:api-manager::latest-overview-concept.adoc#APIM-hyperforce[API Manager on Hyperforce].
