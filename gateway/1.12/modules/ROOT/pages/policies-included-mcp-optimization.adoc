...
[width="100%", cols="5,15"]
|===
>s| Policy Name | MCP Optimization Policy
...
The MCP Optimization policy reduces LLM token consumption by transforming MCP `tools/call` responses before they're returned to the client. Configure the policy to:
...
The policy uses the `pre_transform_tokens` and `response_tokens` metrics to estimate token reduction.

== Configuring Policy Parameters

include::partial$policy-title-headers.adoc[tag=configFileTitleOnly]

The MCP Optimization policy isn't supported in Local Mode.
...
You can override the global policy configuration for the *HTML to Markdown*, *Strip Base64*, *Remove Nulls*, and *Collapse Whitespace* parameters. You can't override *Min Response Size*.
