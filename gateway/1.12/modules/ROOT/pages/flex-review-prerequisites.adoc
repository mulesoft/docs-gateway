...
Deploying an Extra Large Managed Flex Gateways requires two units of Large Managed Flex Gateways.
...
|===
| Limit | Value | Notes
| APIs per Extra Large Managed Gateway instance | 500 | Although it shares the limit with Large Managed Gateway, the Extra Large size is designed for more resource demanding uses cases. i.e. policies with costful processing and/or higher thrughput.

The platform does not allow to exceed this limit.
| APIs per Large Managed Gateway instance | 500 | The platform does not allow to exceed this limit.
| APIs per Small Managed Gateway instance | 50 | The platform does not allow to exceed this limit.
|===
