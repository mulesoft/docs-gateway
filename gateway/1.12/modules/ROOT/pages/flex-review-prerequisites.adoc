...
Deploying an Extra Large Managed Flex Gateway requires two Large Managed Flex Gateways resources.

These resources aren't inherited. To redistribute Managed Flex Gateway resources to a business group, see xref:access-management::business-groups.adoc#redistribute-resources[Redistribute Resources Between Existing Business Groups].
...
[[limits]]
== Flex Gateway Limits and Performance Metrics
...
=== Managed Flex Gateway Limits and Performance Metrics

Each Managed Flex Gateway size has a limit on the number of APIs it can deploy. The platform doesn't allow you to exceed this limit.
...
Each Managed Flex Gateway size supports a maximum number of transactions per second (TPS). Exceeding this number can increase request latency.
...
=== Self-Managed Flex Gateway Limits and Performance Metrics
