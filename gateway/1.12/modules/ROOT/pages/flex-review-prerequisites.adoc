...
Each Managed Flex Gateway size has a limit on the number of APIs it can deploy. The platform does not allow you to exceed this limit.
...
Each Managed Flex Gateway size supports a maximum number of transactions per second (TPS). Exceeding this might increase request latency.
