...
|===
|Field |Description
|Built-in Protections | The built-in SQL and XSS injection protection settings.
|Custom Protections | The rules defined with a name and regex pattern for custom injection types.
|Protect Path and Query | If enables, the gateway checks the request path and query string.
|Protect Headers | If enadbles, the gateway checks the request headers.
|Headers to Protect | List of header names to check. Leave empty to check all headers.
|Protect Body | If enabled, the gateway checks the request body.
|Reject Requests | If enabled, the gateway rejects matching requests with `400` response code. Otherwise, the gateway allows them to reach the upstream service and logs a policy violation.
|===
