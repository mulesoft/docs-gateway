...
For Self-Managed Gateways, HTTP caching requires the you <<configure-shared-storage-for-self-managed-gateway, configure shared storage>>. For Managed Gateways, Flex Replica shares the cache with each other directly. To learn more about the Managed Gateway architecture, see xref:flex-architecture-managed-dr-ha.adoc[].
...
Self-Managed Gatewway require you to configure a shared storage for distributed caching. For more information about Shared Storage, refer to xref:flex-conn-shared-storage-config.adoc[Configuring Shared Storage for Flex Gateway in Connected Mode].
...
. The HTTP Caching policy caches the response for later reuse if it has not already been cached.
