...
The OpenID Connect Access Token Enforcement policy doesn't support in Flex Gateway running in Local Mode.
...
When a client makes a GET request to this path, Flex Gateway responds with a JSON document containing the authorization server metadata. By default, Flex Gateway derives the authorization server URL supplied in the metadata from the introspection endpoint URL. For example, `https://auth.example.com/introspect` becomes `https://auth.example.com`. |
