...
>s|Release Notes| xref:release-notes::flex-gateway/flex-gateway-hybrid-policies/mcp-tool-mapping-release-notes.adoc[MCP Tool Mapping]
...
| Source Tool Name | Defines the original tool name from the MCP server or a regex pattern for pattern-based matching with capture group replacement.
| Mapped Tool Name | Defines the new tool name to use in responses. For regex mappings, use `$1`, `$2`, `$3`, and so on to reference capture groups.
