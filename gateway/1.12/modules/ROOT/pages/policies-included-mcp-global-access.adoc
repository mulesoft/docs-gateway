...
>s|Release Notes| xref:release-notes::flex-gateway/flex-gateway-hybrid-policies/mcp-global-access-policy-release-notes.adoc[MCP Global Access Control]
...
* `tools/list` responses: The list returned to the client includes only permitted tools.
* `tools/call` requests: The gateway rejects calls to blocked tools with HTTP 403 and a JSON-RPC error stating `Access denied to: <tool name>`.
...
|===
| Element | Description
| Rules | Defines the access control rules. At least one rule is required. Each rule allows or blocks access to matching tools.
...
| Value | The tool name for literal match type, or the regex pattern for pattern match type.
|===
...
Only Allow rules:: Only tools that match at least one Allow rule are permitted. All others are blocked.
Only Block rules:: All tools are permitted except those that match a Block rule.
Both Allow and Block rules:: Only tools that match at least one Allow rule and do not match any Block rule are permitted. If a tool matches both an Allow and a Block rule, it is blocked. Tools that match no Allow rule are also blocked.
