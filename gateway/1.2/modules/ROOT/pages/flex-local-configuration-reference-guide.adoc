...
The following resource demonstrates API port sharing, with two distinct services listening on port 8080. The `spec.services.address` value specifies the service address and port that should be shared. Additionally, the first example service `spec.services.routes.config.destinationPath` value prepends `/api` to the specified paths under `rules`, acting in a similar manner as a base path.
...
spec:
  ...
  services:
    dogs:
      address: https://dog-api.kinduff.com:443/
      routes:
        - rules:
            - path: /api/dogs(/.*)
          config:
...
The services both listen on port 8080:
----
http://localhost:8080/api/dogs/facts
http://localhost:8080/api/cats/facts
----
...
The `sharedStorage` object configures the gateway for shared storage. Production workflows should use https://redis.io/[Redis^], though defining it is optional. If Redis is not defined, shared storage services at port 4000 are still available but will use an in-memory implementation. 
