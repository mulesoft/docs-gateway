...
To view message logs in API Manager you need environment level permissions *API Manager Environment Administrator* or *Deploy API Proxies* , or you must have the *Organization Administration* role for your organization. See xref:access-management::permissions-by-product[Permissions Available in Anypoint Platform].
