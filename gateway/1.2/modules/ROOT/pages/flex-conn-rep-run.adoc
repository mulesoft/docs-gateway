...
To add a Flex Replica in a Docker Container you must run the Flex Gateway start command using the file path and name of the same `registration.yaml` file created during registration. If you are running the container locally, you will also need to use a different port.
