...
. Create a configuration file called `custom-policy-example.yaml`. Save the file in the Flex Gateway configuration directory.
...
If you compiled the policy using a target of `wasm32-wasi`, locate the binary in `./target/wasm32-was/release`.
====
