...
+
Composition and conditional keywords such as `anyOf`, `oneOf`, `allOf`, `not`, or `if`/`then`/`else` are not supported when defining a policy configuration schema. Avoid using them because the policy configuration form doesn't render correctly.
