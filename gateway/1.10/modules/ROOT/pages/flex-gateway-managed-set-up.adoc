...
* You have created a private space or have access to a private space created by someone else. To create a private space, see xref:cloudhub-2::ps-create-configure.adoc[].

=== Managed Flex Gateway on Runtime Fabric

IMPORTANT: Flex Gateway is supported on Runtime Fabric as a limited availability. For access, contact your sales representative for access.

For Flex Gateway on Runtime Fabric, you must enable Open Telemetry with a `100` percent *Sampling percentage*. You cannot override the sampling percentage for individual APIs.
