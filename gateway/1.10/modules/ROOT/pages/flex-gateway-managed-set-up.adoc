...
Before getting started with Flex Gateway on CloudHub 2.0, ensure:
...
Before getting started with Flex Gateway on Runtime Fabric, ensure:

* Ensure your Runtime Fabric deploymentversion is 2.11.0 or later. To upgrade, see xref:runtime-fabric::pgrade-index.adoc[Upgrading Runtime Fabric].
* The business group you want to deploy your gateway to has Managed Flex Gateway Resources. To redestribute Managed Flex Gateway resources to a business group, see xref:access-management::business-groups.adoc#redistribute-resources[Redistribute Resources Between Existing Business Groups].
+ 
NOTE: Gateways cannot be shared across business groups. A user must be a member of the business group to access the Flex Gateway. 
...
* Run this commane to enable the `ManagedFlexGateway` resources in your Runtime Fabric namespace:
+
[source,bash]
----
...
+
NOTE: For Flex Gateway Agent Fabric deployments on Runtime Fabric, you must enable Open Telemetry with a `100` percent *Sampling percentage*. You cannot override the sampling percentage for individual APIs.
+
. Click *Save & deploy*.
