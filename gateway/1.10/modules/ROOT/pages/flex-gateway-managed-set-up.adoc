...
+ 
NOTE: Gateway's cannot be shared across business groups. A user must be a member of the business group to access the Flex Gateway. 
...
IMPORTANT: Flex Gateway is supported on Runtime Fabric as a limited availability GA release. For access, contact your sales representative for access.
...
+
NOTE: For Flex Gateway on Runtime Fabric, you must enable Open Telemetry with a `100` percent *Sampling percentage*. You cannot override the sampling percentage for individual APIs.
+
. Click *Save & deploy*.
