...
|Block on unknown quota  | For policies using distributed rate limiting, block requests if the policy cannot retrieve the quota from shared storage. Otherwise, allow requests to reach the upstream.  
|===
