...
|Block on unknown quota  | For policies using distributed rate limiting, block requests if the policy can't retrieve the quota from shared storage. If not enabled and the quota can't be retrieved, no rate limiting is applied. 
|===
