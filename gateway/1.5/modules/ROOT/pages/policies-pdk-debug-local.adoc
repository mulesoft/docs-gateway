= Debugging Custom Policies Locally
...
To test your policy, complete the following steps:
...
IMPORTANT: You only need to to run the registration command. Do not run the Docker start command. The `run` command completes this step.
...
In this example, the `user` is set to the default value `user1`, and `password` is set to `12345678`.
...
This command start the two Docker containers in the `<root-directory>/test/config/docker-compose.yaml` file:
...
* `Backend`: The Docker Container that runs thebackend service for the Flex Gateway instance. This backend service echoes any request it receives.
...
The backend service returns an echo response including the requests’s route, body, and headers sent with any modification performed by the the custom policy or additional policies. Any logs included in the custom policy's source code are visible in the terminal running the containers.
