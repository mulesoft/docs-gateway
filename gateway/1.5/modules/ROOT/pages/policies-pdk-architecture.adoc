...
`proxy-wasm`'s event driven interface forces users to keep track of all the different events received to handle a single request. This causes the following drawbacks that make developing and maintaining event driven code costly:
