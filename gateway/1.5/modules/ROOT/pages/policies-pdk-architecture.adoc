...
Flex Gateway is built on Envoy which requires developed policies be compatible with the event-driven https://github.com/proxy-wasm/spec[proxy-wasm] architecture. 

`proxy-wasm`'s event driven interface forces the user to keep the track between the different events received to handle a single request. This causes the following drawbacks that make developing and maintaining event driven code costly:
...
PDK abstracts the user from the event oriented coding approach by using reactor and executor patterns. PDK provides the developer a way to code each incoming request in a linear method which results in: 
