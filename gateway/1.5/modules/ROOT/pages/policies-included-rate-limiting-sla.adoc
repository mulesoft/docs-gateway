...
[NOTE]
====
When on distributed mode the X-Ratelimit-Remaining header is not kept in perfect synchronization with all the replicas.  With multiple replicas on distributed mode the X-Ratelimit-Remaining header can be thought of as an estimation of how much quota te replica has available for itself and not the exact value of the remaining actual quota available for the API.

To illustrate this here is a valid scenario:
