...
When assigning resources for a node, consider:
...
The following guiding table helps you choose the appropriate cluster size:
...
High memory consumption can lead to out-of-memory errors. To solve this issue, increase the memory allocation or the node size. Horizontal scaling might not help solve issues in this situation.
...
Using TLS highly affects the response time and the process consumption.
