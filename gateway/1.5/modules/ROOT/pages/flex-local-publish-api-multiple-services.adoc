...
For Flex Gateways running in Local Mode, API instances with multiple upstream services in are configured by using the `ApiInstance` and the `PolicyBinding` YAML configuration resources. The `ApiInstance` resource defines an API instance, and the `route-weighted` policy resource defines a single route to one or multiple upstream services. To add additional routes, add multiple `route-weighted` resources.

The following tutorial uses a preconfigured `route-weighted` policy resource. For information about how to configure the resource, see xref:policies-included-traffic-management.adoc[].
...
* Flex Gateway downloaded. For more information, see xref:flex-install.adoc[]. 
...
* Your upstream service URLs. The following Publish an API section refers to fictional `products-api` and `products-api-beta` services and a fictional `json-http` API instance. You can specify your own API name in `metadata.name` and your own service details.
