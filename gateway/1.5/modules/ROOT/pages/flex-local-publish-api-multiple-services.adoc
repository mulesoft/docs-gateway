...
----
+
. Enter the following command to view the pods logs and ensure that the API is created successfully: +
----
kubectl logs -f <pod-name>
----

If you use https://kubernetes.io/docs/concepts/services-networking/service/[Kubernetes Service^] as an upstream or you make a request from a policy, you must set:

* `spec.ports[].protocol`, which must be TCP
* `spec.ports[].name`
