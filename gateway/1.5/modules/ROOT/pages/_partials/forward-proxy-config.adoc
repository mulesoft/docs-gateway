...
If registering your Flex Gateway forward proxy in an internal network without internet, you must add the `--https-proxy` flag when xref:flex-local-reg-run.adoc[registering a Flex Gateway in Local Mode].

//end::localRegistration[]

//tag::connRegistration[]
If registering your Flex Gateway forward proxy in an internal network without internet, you must add the `--https-proxy` flag when xref:flex-conn-reg-run.adoc[registering a Flex Gateway in Connected Mode].
