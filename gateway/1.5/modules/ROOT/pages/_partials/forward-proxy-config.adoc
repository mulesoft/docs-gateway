...
. See <<proxy-registration, Forward Proxy Flex Gateway Registration>> if your Flex Gateway does not have direct internet access.
//end::linux[]
...
If your Flex Gateway with a forward proxy configured is in an internal network where all internet connections pass through the forward proxy, you must add the `--https-proxy` flag when xref:flex-{page-mode}-reg-run.adoc[registering a Flex Gateway].
