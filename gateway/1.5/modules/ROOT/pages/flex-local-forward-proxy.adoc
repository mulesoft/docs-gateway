= Configuring a Forward Proxy for Flex Gateway in Local Mode
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images
:page-mode: local

include::partial$forward-proxy-config.adoc[tags=intro;icon-table]

include::partial$forward-proxy-config.adoc[tags=connections;noproxy]

include::partial$forward-proxy-config.adoc[tags=forward-proxy-parameters]

include::partial$forward-proxy-config.adoc[tags=linux;sample-config-all]

include::partial$forward-proxy-config.adoc[tags=docker]
+
include::partial$task-reg-run-flex-gateway.adoc[tags=start-command-local]
+
include::partial$forward-proxy-config.adoc[tags=docker2;sample-config-all]

include::partial$forward-proxy-config.adoc[tags=k8s;sample-config-all]

include::partial$forward-proxy-config.adoc[tags=noproxy-registration]

== See also
* xref:flex-local-reg-run.adoc[Registering and Running Flex Gateway in Local Mode]