...
API instance with multiple upstream services in Local Mode are configured by using `PolicyBinding` YAML configuration resources. Each `route-weighted` resource defines a single a route. To add additional routes, you can add multiple `route-weighted` resources.

For a tutorial about how to publish an API with this policy applied, see xref:flex-local-publish-api-multiple-services.adoc[].
...
You can only define one “URI Template Regex" path for each ruleset. Only requests with the defined path are sent to this route.
...
You can direct requests to different routes using route rules. Requests must only meet the rule criteria for one of the rulesets. 
...
Setting the policy order sets the order in which traffic is directed to the routes. Requests are sent to the first route where they meet the rule criteria for one of the rulesets. Therefore, route ordering is very important when a request may meet the route rules of multiple routes. 
