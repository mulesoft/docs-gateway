...
[NOTE]
====
The following document applies only to Flex Gateway running in Local Mode. To configure multiple upstream services for Flex Gateway running in Connected Mode, see xref:api-manager::create-instance-task-flex.adoc#traffic-management[Multiple Upstream Services for Flex Gateway Running in Connected Mode].
====
...
NOTE: The Traffic Management for Multiple Upstream Services Policy does not affect the xref:policies-included-tls-outbound[Outbound Transport Layer Security Policy]. You can still apply an Outbound TLS policy to each upstream service.
...
| Required
| N/A
| The weight of total requests sent to this upstream +
 The percentage of traffic sent to that upstream is the upstream's weight divided by total weight of all upstream. For example, if there are two upstream with weight `1`, each upstream receives 50% of the traffic to that route.
...
| Optional
| Empty
| The request path the route can service +
...
You can select multiple methods for each ruleset. Only requests that are one of the defined methods are sent to this route. Supported values are `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, `OPTIONS`, `HEAD`, `TRACE`, `CONNECT`.

| `rules.headers`
| Optional
| Empty
| An array defining what headers and regular expression value must be present for this route to service the request +
...
| Optional
| Empty
| The request host URL the route can service +
...
The Traffic Management policy works by directing traffic to different sets of upstream services by using different routes with defined rulesets and route order. 
...
image:multiple-upstreams.png[Flex Gateway manages the traffic to multiple upstream]
...
You can direct requests to different routes by using route rules. Requests must only meet the rules for one of the rulesets in the route. 
...
In addition to route rules, you can direct requests to different routes by using policy ordering. 

To order policies, see xref:policies-reorder.adoc[]

Setting the policy order sets the order in which traffic is directed to the routes. Flex Gateway directs requests to the first route where the request meets one of the route rulesets. Therefore, route ordering is very important when a request may meet the route rules of multiple routes. 
