...
After installing all the xref:policies-pdk-prerequisites.adoc[], you can begin developing custom policies the Policy Development Kit (PDK).
...
After you xref:policies-pdk-create-project.adoc[Create a New Project], you might iteratively complete some steps. For example, while writing the policy's source code, you may need to return to xref:policies-pdk-create-schema-definition.adoc[Define Policy Configuration Parameters] to add additional parameters, or you might need to recompile the policy after debugging the source code.
...
To get started creating a custom policy with PDK, here are the suggested steps:
