...
The custom policy lifecycle consists of all the steps of developing a custom policy that occur in PDK. The following steps do not include steps that occur outside of PDK, such as xref:policies-pdk-deploy-policies.adoc[applying a policy] which is the same for all Flex Gateway policies.
