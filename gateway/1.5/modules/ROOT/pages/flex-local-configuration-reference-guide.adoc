...
Define a desired gateway state by creating a `Configuration` entity. `Configuration` entities specify several runtime configuration aspects for Flex Gateway itself, such as <<logging, logging>> and <<shared-storage>>. The definition includes the following:
