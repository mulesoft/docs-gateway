...
| Required
| N/A
| The client certificate used in TLS
...
| Optional
| `none`
| The certificate key passphrase. If specified, use the private key format that includes a DEK-Info header. If unspecified, the private key is not encrypted.
...
[%collapsible]
====
The Redis storage service is an implementation of a REST API object store that functions as a bridge between the object store interface and a Redis backend. Optionally, you can use Redis with TLS to protect data and ensure secure communication.
