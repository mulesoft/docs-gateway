...
The configuration parameters you define in the `gcl.yaml` appear as variables in the `src/generated/config.rs` file after you run the `build-asset-file` command.
...
* `properties`: Defines the configuration parameters
...
+
The following yaml snippit demonstrates all string formats:
+
[source,yaml]
----
properties:
...
Additionally compiling the the project with the `build` command also propagates the changes:
...
For more information about compiling the policy, see xref:policies-pdk-compile-policies.adoc[].

Properties define in the `gcl.yaml` files are mapped to the `config.rs` rust file as follows:
