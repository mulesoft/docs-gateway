...
IMPORTANT: Due to the streaming nature of `proxy wasm`, it is possible for an early response to be sent while awaiting the `headers_state.into_body_state().await` function, the request may reach partially or completely to the backend. Avoid this by not awaiting the body.
