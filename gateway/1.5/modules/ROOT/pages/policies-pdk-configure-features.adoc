...
The following sections provide Rust code templates to complete various tasks. Before adding additional features, review <<policy-template>> to learn more about the provided source code.
...
The functions have the capability to inject the predefined parameters:
...
If your filters only receives the parameters and does not modify them, you can omit the lambda in the wrapper. For example, if the signature of the filters is:
