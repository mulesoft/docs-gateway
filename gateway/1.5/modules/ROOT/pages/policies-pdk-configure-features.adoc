...
The policy performs some simple logging. For each incoming request, the policy logs the header `token` at the `info` log level by using the following elements:
...
* `request_filter`: Executes once per every request sent to the API instance to which the policy is applied. This function is the implementation of the filtering performed by the policy. Variables defined inside this function available for the duration of the request.
+
...
Instead of filtering an incoming request, you can define a function that processes the outgoing response by using the `on_response` wrapper instead of the `on_request` wrapper:
...
You can define functions other than the provided `requests_filter` and `response_filter` and define them from the `#[entrypoint]` function. The `requests_filter` and `response_filter`` are only examples.
...
You can access headers in `on_request` or `on_response` wrapped functions by:
...
Envoy handles the method, scheme, path, authority, and status codes as headers. Modify the headers by accessing the request's `:method`, `:scheme`, `:path`, `:authority`, and `:status`. Additionally PDK provides the `RequestHeadersHandler` and `ResponseHeadersHandler` traits for ease of access.
...
Envoy uses the same buffer to share data from the headers and the body to the policy. So, at a given moment, the policy can only access the headers or the body. If the policy must read both:
...
If your `on_request` or `on_response` wrapped functions only receives the predefined parameters, it is not necessary to include the lambda in the wrapper. For example, if the signature of the function is:
...
You can use the following functions definition:
...
If your policy must interact with external services, the policy must use HTTP calls. You can preform HTTP calls in the request and response filters and in the function. To perform a the HTTP call, inject the `HttpClient` as follows:
...
For both Connected and Local mode, define services using `Service` resource. If the service is not defined when the request is made, the API request fails.
...
Do this by returning a `Response` object from your `on_request` wrapped functions:
...
This function is not very useful as it intercepts the requests no matter what. You can block or allow requests to reach the upstream service using a `Flow`. A `Flow` in an `enum` value with two possible values:
...
[IMPORTANT]
====
Due to the streaming nature of `proxy wasm`, it is possible for an early request to partially reach the upstream service while awaiting the `headers_state.into_body_state` function. The only way to avoid this by not awaiting the body. However, that is not possible for all policies. You should configure your upstream to ignore partial requests if the policy must read the body.
...
To increase the capability of handling requests, envoy spawns many workers for the policy. A single worker handles all the stages of a specific request including outgoing connections and the response flow. Due to this, you can track an internal status for a specific request and create sequential execution flow even though the underlying implementation is event driven. 
...
Communicating a shared global state to all workers is more complex. Achieve this is by using the provided `Cache` mechanism.
...
If your policy to must interact with external services, achieve this is using Flex Gateway's exposed HttpClient. For more information about performing an HTTP call, see <<performing-an-http-call>>.
