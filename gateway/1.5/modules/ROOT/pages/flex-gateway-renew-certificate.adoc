...
. Replace `<path-to-registration-file-in-container>` with the path and filename of the existing `registration.yaml` file. If running in a container, you must specify the file in the container file system.
