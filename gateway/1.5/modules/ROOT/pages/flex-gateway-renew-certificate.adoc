...
. Rename your current registration file to `current-registration.yaml` (or similar).
...
. Replace `<path-to-registration-file>` with the path and filename of the existing `current-registration.yaml` file.
