...
Renewing Flex Gateway registration ensures the continued operation of your applications. You renew registration by using the `flexctl` CLI.

To avoid potential disruptions, verify the success of the renewal process by using the `flexctl inspect` command.

This document describes two `flexctl` CLI commands: `renew`, and `inspect`.
...
[source,ssh]
----
podman exec -u $UID <container-name-or-id> flexctl inspect \
--flex-gateway-admin-api-url=<your-custom-admin-api-url>
----
====
