...
* `make build-asset-files`: Generates all the policy asset files required to build, execute, and publish the policy. This command also updates the `config.rs` source code file with the latest configurations defined in the `gcl.yaml` file. +
 For more information about compiling a policy, see xref:policies-pdk-create-schema-definition.adoc[].
* `make build`: Compiles the WebAssembly binary of the policy before deploying the policy to a Flex Gateway. +
...
For more information about testing your policy, see xref:policies-pdk-
* `make publish`:
* `make release`:
