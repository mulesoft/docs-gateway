...
Publishing the policy creates a `development` state asset in Exchange. This enables you to test and edit the policy version. To learn more about `development` state assets, see xref:exchange::lifecycle.adoc#development-state[Development State Assets].
* <<release, Release a Custom Policy>> +
Releasing the policy creates a `stable` state asset in Exchange. After releasing the policy, you must create a new policy version to edit the policy. To learn more about `stable` state assets, see xref:exchange::lifecycle.adoc#stablet-state[Stablest State Assets].
...
To publish a `development` state policy asset, run the `publish` command from the policy's root folder:
...
After publishing the asset, a new `development` state asset appears in Exchange:

image:pdk-dev-asset.png["Development assets appear in Exchange labeled DEV."]
...
To release a `stable` state policy asset, run the `release` command from the policy's root folder:
...
After releaseing the asset, rerunning the `release` command results in a conflict error.
