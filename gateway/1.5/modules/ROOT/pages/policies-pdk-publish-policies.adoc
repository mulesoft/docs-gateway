...
Everytime you publish or release a policy, you create a new `stable` asset in Exchange. Therefore, republishing a policy does not override previous versions. To release a policy again, you must change the policy version number. For more information about `stable` state assets, see xref:exchange::lifecycle.adoc#stablet-state[Stable State Assets].
...
After publishing the asset, a new asset labled `DEV` appears in Exchange:
