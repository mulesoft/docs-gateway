...
During the custom policy development cycle, you can upload your policy to Exchange to manage its version and distribute it to other users. For more information about Exchange, see xref:exchange::index.adoc[].
...
* Ensure that the Connected App you used to authentice to the xref:policies-pdk-prerequisites.adoc#cli[Anypoint Platform CLI] has the `Exchange Contributor` scope applied.
...
After publishing the asset, a new asset appears in Exchange. The asset's group ID, asset ID, and asset version (GAV) appear, similar to the following:
