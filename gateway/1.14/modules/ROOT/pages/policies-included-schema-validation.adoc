...
>s|Release Notes| xref:release-notes::gateway/policies/schema-validation-release-notes.adoc[]
>s|Returned Status Codes| 400 - The request properties do not comply with the API specifications.
|===

== Summary

NOTE: The Schema Validation policy only supports REST APIs and currently only supports OpenAPI Specification 3.0 (OAS3) schema validations.

NOTE: The Schema Validation policy supports a maximum of 64 bits for the `number` schema data type.
...
Requests support the JSON format (content type `application/json`) and the `multipart/form-data` format. Both content types are validated against their OAS schema definitions.
...
| Block request | Select to block an incoming request that does not conform to the API specification. Selecting not to block only logs the request. Choosing to block invalid requests returns a 400 status when the request is invalid. | Block enabled | No
...
| Rule of application | Configures the runtimes the policy is applied to. Schema Validation is currently only available for Omni Gateway. Applying the policy to all runtimes only applies the policy to Omni Gateway APIs. | All runtimes | Yes
|===
