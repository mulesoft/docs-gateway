...
Omni Gateway includes a `flexctl dump` CLI that collects a full snapshot of the gateway's runtime state and writes it to a single ZIP file. The snapshot includes information related to a gateway's API instances, services, configuration, and available extensions (policies), along with logs, Envoy statistics, and other diagnostic data that is useful when working with MuleSoft Support.
...
All `flexctl` logs are written to standard error, so the standard output stream stays a clean binary ZIP that you can pipe or redirect without corrupting the file.

Streaming to standard output lets you capture a dump from a container without entering it, which is useful in environments where direct container access is unavailable, such as CloudHub:

[source,ssh]
----
docker exec -ti flex-gateway flexctl dump -o - > dump.zip
----
