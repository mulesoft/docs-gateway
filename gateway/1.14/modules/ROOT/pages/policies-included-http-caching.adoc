...
>s|Release Notes| xref:release-notes::gateway/policies/http-caching-release-notes.adoc[]
...
Policies applied after the HTTP Caching policy in the policy order are not applied to responses served from the cache. For example, a rate limiting policy applied after the HTTP Caching policy only limits the responses served by the upstream service but not the responses served by the cache. Apply all policies needed for the cached response before the HTTP Caching policy.

For Self-Managed Gateways, HTTP caching works out of the box using the default local (filesystem) storage, which persists the cache across restarts. To share the cache across replicas, <<configure-shared-storage-for-self-managed-gateway, configure distributed or Redis shared storage>>. For Managed Gateways, Omni Replica shares the cache with each other directly. To learn more about the Managed Gateway architecture, see xref:gateway-architecture-managed-dr-ha.adoc[].

NOTE: The HTTP Caching policy does not support MCP and A2A server instances.
...
[[configure-shared-storage-for-self-managed-gateway]]
==== Configure Shared Storage for Self-Managed Gateway

Distributed caching across replicas requires distributed or Redis shared storage, because the default local (filesystem) storage is not shared between replicas. Configure either xref:conn-distributed-shared-storage-config.adoc[distributed shared storage] or xref:conn-shared-storage-config.adoc[Redis shared storage].
...
Default value: `false`
| Yes
| Persistent Cache | Configures the cache to persist between different restarts of the Omni Gateway instance.
Default value: `false`
| Yes
...
. The HTTP Caching policy checks if the request is cached. If not, the cache-miss process is initiated.
...
. The HTTP Caching policy caches the response for later reuse if it hasn't already been cached.
...
. The <<key,key>> is found in the cache and the stored response is returned.
...
The cache works as a dictionary, where each response stored in the cache is associated with a string called _key_. For example, the expression #[attributes.headers['key']] uses the header called `key` as the entry key. You can use any DataWeave expression that returns a string, including xref:policies-dataweave-support.adoc#available-predefined-variables[predefined DataWeave variables].
...
A persistent cache enables the stored entries in the cache to persist if the Omni Replica is restarted.
