...
In addition to parameters above, Omni Gateway offers variables for logging output. When configured, the variables render as one of their respective outputs in the logs:
...
| `logger` | Omni Gateway service where the logged event occurred | `flex-gateway-agent`, `flex-gateway-envoy`, and `flex-gateway-fluent` 
