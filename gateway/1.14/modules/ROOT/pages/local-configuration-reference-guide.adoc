...
:page-aliases: flex-local-configuration-reference-guide.adoc

Omni Gateway running in Local Mode supports two configuration models:
...
image:gateway-configuration-models.png["Diagram of the structure of API instances, services, and policy binding",width=65%]
...
Define a desired gateway state by creating a `Configuration` entity. `Configuration` entities specify several runtime configuration aspects for Omni Gateway itself, such as <<logging, logging>> and <<shared-storage, shared storage>>. The definition includes the following:
...
The `sharedStorage` object configures the gateway for shared storage. Omni Gateway supports three types of shared storage:

* *Distributed shared storage*: Enables multiple gateway replicas to share state across a distributed deployment. Data is lost when all replicas go down (scaled to zero replicas).
* *Redis shared storage*: Provides persistent storage when all replicas go down (scaled to zero replicas). Requires https://redis.io/[Redis^] (minimum version 4.0.0).
* *Local shared storage*: File-based implementation. Data is persisted to disk, so it is retained when Omni Gateway is stopped or restarted. This is the default when no `sharedStorage` configuration is provided.
...
|`sharedStorage.objectStore.url`
|Required
|N/A
|The distributed shared storage URL in the format `distributed://<bind-address>:<port>?join=<host1>:<port1>,<host2>:<port2>,...`
...
.*Distributed Shared Storage Example*
[%collapsible]
====
Distributed shared storage enables multiple gateway replicas to share state across a distributed deployment. Data is not persistent and is lost after gateway restarts.
...
[%collapsible]
====
The local storage service is an implementation of a REST API object store that saves data to disk. Data is persisted, so it is retained when Omni Gateway is stopped or restarted. Local shared storage is the default when no `sharedStorage` configuration is provided.
