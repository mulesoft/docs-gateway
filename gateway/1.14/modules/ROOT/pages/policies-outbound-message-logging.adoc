...
>s|Release Notes| xref:release-notes::gateway/policies/outbound-message-logging-release-notes.adoc[]
...
| Optional
| `false`
| When enabled, the policy detects and masks configured PII in log messages before they are written. Request and response bodies are forwarded to the upstream service unchanged.

| `dataMasking.entities`
| Optional
| `[]`
| List of built-in PII types to mask. Supported values: `Email`, `US SSN`, `Credit Card`, and `Phone Number`.

| `dataMasking.customPatterns`
| Optional
| `[]`
| List of custom patterns to mask when built-in PII detection doesn't cover the value. Each pattern must have a `name` and a `pattern` (regular expression) property.
...
|Enable data masking |Detect and mask configured PII before writing log messages. Request and response bodies are forwarded to the upstream service unchanged. |Optional
|PII Detection |Select built-in PII types to mask: Email, US SSN, Credit Card, and Phone Number. |Optional
|Custom Patterns |Add a regular expression pattern to mask a value that built-in PII detection doesn't cover. Each pattern has a name and a regex. |Optional
|===
...
The Outbound Message Logging policy supports the xref:policies-dataweave-support.adoc[standard DataWeave Expressions supported by Omni Gateway policies] and the following additional xref:dataweave::dataweave-variables-context.adoc[`vars`] DataWeave expressions. Format the following expressions `#[vars.<expressions>]`, for example `#[vars.orgId]`:
...
* `reqId` (Referred to as `traceId` in Omni Gateway logs)
