= Configure Omni Gateway as an Ingress Controller in Local Mode
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images
:page-aliases: flex-gateway-k8-ingress-class.adoc
...
. xref:local-reg-run.adoc[Register and Run Omni Gateway in a Kubernetes Cluster in Local Mode].
...
| `ingressClass.enabled` | Enables Omni Gateway as an `IngressClass` resource | `true` or `false`
| `ingressClass.name` | The name of the `IngressClass` | Any string
| `ingressClass.setAsDefault` | Sets Omni Gateway as the default `IngressClass` | `true` or `false`
|===
...
You can modify the default `IngressClass` parameters during or after the initial installation of the Helm chart. For more information about how to modify a Helm chart, see xref:gateway-k8-change-helm-settings.adoc[]

== Creating Ingress Resources

After you xref:local-reg-run-up.adoc#kubernetes[install a Helm chart into a namespace], you can verify your new `IngressClass` resource by running the following command:
...
. xref:local-publish-simple-api.adoc#kubernetes[Publish an API in Local Mode].
...
* xref:gateway-k8-change-helm-settings.adoc[]
