...
>s|Release Notes| xref:release-notes::gateway/policies/openid-token-enforcement-release-notes.adoc[]
...
The OpenID Connect Access Token Enforcement policy doesn't support in Omni Gateway running in Local Mode.
...
| Scope Validation Criteria | Determines if the token must contain all defined scopes or just one. The values are *Contains all scopes* and *Contains any scope*.

For *Contains all scopes*, the policy only accepts tokens with both the `READ` and `WRITE` scopes. For *Contains any scopes*, the policy accepts tokens with at least one scope. If no scopes are defined for the *Scopes* parameter, the policy doesn't validate scopes regardless of what you select for *Scope Validation Criteria*.| 
...
| Validated Token TTL | Sets the maximum time, in seconds, a valid token stays in cache before requiring re-introspection. Use `0` to disable caching. Use `-1` to skip this cap and rely solely on the token's own expiration. The default value is `600`. | `600`

| Maximum Cache Entries | Specifies the maximum number of tokens to be stored in the cache. The default value is `10000`. | `10000`

| Enable Protected Resource Metadata | Enables https://www.rfc-editor.org/rfc/rfc9728[RFC 9728] OAuth 2.0 Protected Resource Metadata. When enabled, all `401` responses include a `WWW-Authenticate` header with the resource metadata URL `{api_path}/.well-known/oauth-protected-resource`. +
When a client makes a GET request to this path, Omni Gateway responds with a JSON document containing the authorization server metadata. By default, Omni Gateway derives the authorization server URL supplied in the metadata from the introspection endpoint URL. For example, `https://auth.example.com/introspect` becomes `https://auth.example.com`. |

| Override Protected Resource Metadata Authorization Server Url | Overrides the authorization service URL with a custom URL. For example, when the authorization service uses a different path, such as Keycloak realms. |

| Protected Resource Metadata Authorization Server Url | Server base URL to use when `overrideProtectedResourceMetadataAuthorizationServerUrl` is enabled. |
|===
...
image:open-id-policy-workflow.png["A flowchart illustrating the process of a user engaging with an API via a PingFederate policy"]
