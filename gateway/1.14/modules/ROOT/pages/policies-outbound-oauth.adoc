...
>s|Release Notes| xref:release-notes::gateway/policies/outbound-oauth-release-notes.adoc[]
...
| `customRequestBodyParameters`
| Optional
| N/A
| A list of custom parameters to add to the token request body, such as `audience`. Each parameter must have a `name` and `value` property. The `value` can be a static string or a DataWeave expression.
|===
...
|Custom Request Body Parameters |A list of custom parameters to add to the token request body, such as `audience`. Each parameter must have a name and value property. The value can be a static string or a DataWeave expression. |Optional
|===
