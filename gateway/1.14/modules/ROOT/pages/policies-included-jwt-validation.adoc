...
| Optional
| Empty string
| The authorization server base URL to advertise in the protected resource metadata, for example, `https://auth.example.com/realms/myrealm`. This parameter is required when `enableProtectedResourceMetadata` is `true` and must be a valid URL. The JWT Validation policy has no issuer to derive this URL from, so you must set it explicitly.
...
| The authorization server base URL to advertise in the protected resource metadata. This field is required when *Enable Protected Resource Metadata* is checked and must be a valid URL.
...
When you enable *Enable Protected Resource Metadata*, the policy implements https://datatracker.ietf.org/doc/html/rfc9728[RFC 9728^] so that clients can discover the authorization server that protects the API. The policy then:

* Serves the metadata document at `{API_Path}/.well-known/oauth-protected-resource`, advertising the authorization server set in *Protected Resource Metadata Authorization Server URL*.
* Adds a `resource_metadata` hint (realm `JWT Validation Realm`) to the `WWW-Authenticate` header of `401` responses.
* Returns `401` instead of `400` when a request omits the `Authorization` header, directing unauthenticated clients to the discovery endpoint.

Because the JWT Validation policy has no issuer to derive the authorization server from, you must set *Protected Resource Metadata Authorization Server URL* to a valid URL when the feature is enabled, for example, `https://auth.example.com/realms/myrealm`. Otherwise, the policy configuration is rejected.
