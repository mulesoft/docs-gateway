...
| Optional
| Empty string
| The authorization server base URL to advertise in the protected resource metadata, for example, `https://auth.example.com/realms/myrealm`.
...
| The authorization server base URL to advertise in the protected resource metadata. Configure this field when *Enable Protected Resource Metadata* is checked.
...
* Serves the protected resource metadata document at `{API_Path}/.well-known/oauth-protected-resource`.
* Includes a `resource_metadata` hint in the `WWW-Authenticate` header of `401` responses, pointing clients to the metadata document.

Use *Protected Resource Metadata Authorization Server URL* to specify the authorization server base URL that the policy advertises in the metadata document, for example, `https://auth.example.com/realms/myrealm`.
