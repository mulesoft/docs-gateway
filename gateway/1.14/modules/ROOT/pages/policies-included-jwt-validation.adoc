...
>s|Release Notes| xref:release-notes::gateway/policies/jwt-validation-release-notes.adoc[]
...
| Optional
| 256
| Number indicating key length. Supported values are 256, 384, 512. For ES method, Omni Gateway supports 256 and 384 key lengths.
...
| Optional
| `false`
| Serves the OAuth 2.0 Protected Resource Metadata document at `{API_Path}/.well-known/oauth-protected-resource` and includes a `resource_metadata` hint in `401` responses, per https://datatracker.ietf.org/doc/html/rfc9728[RFC 9728^].

| `protectedResourceMetadataAuthorizationServerUrl`
| Optional
| Empty string
| The authorization server base URL to advertise in the protected resource metadata, for example, `https://auth.example.com/realms/myrealm`. This parameter is required when `enableProtectedResourceMetadata` is `true`, and it must be a valid URL. Unlike the OpenID Connect Access Token Enforcement and OAuth 2.0 Access Token Enforcement policies, the JWT Validation policy has no issuer from which to derive this URL, so you must always set it explicitly.
...
| 256, 384, 512. For ES method, Omni Gateway only supports 256 and 384 key lengths.
...
| If you check this field, the policy serves the OAuth 2.0 Protected Resource Metadata document at `{API_Path}/.well-known/oauth-protected-resource` and includes a `resource_metadata` hint in `401` responses, per https://datatracker.ietf.org/doc/html/rfc9728[RFC 9728^].
|

| Protected Resource Metadata Authorization Server URL
| The authorization server base URL to advertise in the protected resource metadata. This field is required when *Enable Protected Resource Metadata* is checked, and it must be a valid URL.
...
==== Propagating Client ID

If Client ID validation is enabled, the Client ID is propagated as a claim and as a reserved authentication property. Access the property by using the following DataWeave expression:

`#[authentication.clientId]`

=== Serving Protected Resource Metadata

When you enable *Enable Protected Resource Metadata*, the policy implements the OAuth 2.0 Protected Resource Metadata specification defined in https://datatracker.ietf.org/doc/html/rfc9728[RFC 9728^]. This enables clients to discover which authorization servers protect the API and how to obtain a valid token.

With this feature enabled, the policy:

* Serves the protected resource metadata document at `{API_Path}/.well-known/oauth-protected-resource`. The document advertises the authorization server configured in *Protected Resource Metadata Authorization Server URL*.
* Includes a `resource_metadata` hint in the `WWW-Authenticate` header of `401` responses, pointing clients to the metadata document. The header uses the `JWT Validation Realm` realm.
* Returns a `401` status code instead of `400` when a request omits the `Authorization` header, so that an unauthenticated client is directed to the discovery endpoint to locate the authorization server.

Because the JWT Validation policy has no issuer from which to derive the authorization server, you must set *Protected Resource Metadata Authorization Server URL* to a valid URL whenever the feature is enabled, for example, `https://auth.example.com/realms/myrealm`. If the URL is missing or invalid, the policy configuration is rejected.

To build the metadata document, the policy derives the resource origin from the incoming request, preferring the consumer-facing URL when available, then the `x-forwarded-proto` and `x-forwarded-host` headers, and finally the request scheme and authority.
