...
:page-aliases: flex-gateway-k8-external-dns.adoc
...
* An Omni Gateway deployment to an AWS cluster or a registered Omni Gateway instance (gateway)
+
...
== Set DNS Names through Your Helm Chart for Omni Gateway

Use the `extraAnnotations` parameter with `helm upgrade` to set the domain names. This parameter is described in the https://artifacthub.io/packages/helm/flex-gateway/flex-gateway[Helm chart for Omni Gateway (flex-gateway)^].
...
.. If you are deploying Omni Gateway to the cluster for the first time, run the following command, replacing the placeholder values with the path to your gateway's registration YAML file (by default, `registration.yaml`) and your domain names: 
+
[source,helm]
----
