...
After Omni Gateway extracts the GraphQL operation text, the policy evaluates your rules against the request. The policy skips persisted queries, because Omni Gateway doesn't inspect their operation text, and passes them to the upstream service without authorization checks. Non-GraphQL requests also pass through.
...
The following `principal` attributes come from the upstream authentication policy:
...
The following request `context` attributes describe the incoming GraphQL request:
