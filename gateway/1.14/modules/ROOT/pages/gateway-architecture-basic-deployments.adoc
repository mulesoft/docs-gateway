...
:page-aliases: flex-architecture-basic-deployments.adoc, architecture-basic-deployments.adoc
...
image:ad-standalone-deployment.png["In a standalone deployment, Omni Gateway and the consumer applications are in the same network."]
...
image:ad-ingress-deployment.png["In an ingress deployment, Omni Gateway and the consumer applications are in different networks."]
...
image:ad-egress-deployment.png["A diagram of Anypoint Platform depicting the flow of data between the platform, an Omni Gateway, APIs, and a user"]
...
image:ad-sidecar-deployment.png["In a sidecar deployment, Omni Gateway and the consumer application are in the same Kubernetes pod."]
...
For Omni Gateway, the CRDs are `APIinstance`, `Service`, `PolicyBinding`, and `Configuration` and are defined in the xref:local-configuration-reference-guide.adoc[].
...
For more information about how to configure Omni Gateway as an Ingress Controller, see xref:gateway-k8-ingress-class.adoc[].
