...
>s|Release Notes| xref:release-notes::gateway/policies/rate-limiting-release-notes.adoc[]
...
[NOTE]
====
A Rate Limiting policy applied to an Omni Gateway API is scoped to replicas, not the gateway.
====
...
| Optional
| `true`
| Distributed: When enabled, the quota is shared among all Omni Gateway replicas. Distributed rate limiting requires shared storage to be enabled.

|blockOnUnknownQuota
| Optional
| `true`
| For policies using distributed rate limiting (`clusterizable`), block requests if the policy can't retrieve the quota from shared storage. If `false` and the quota can't be retrieved, no rate limiting is applied. 

|===

NOTE: If you enable the distributed rate limiting, the policy resizes time windows under 10 seconds linearly. For example, if you configure a maximum of 10 requests per 5 seconds, the policy resizes that window to a maximum of 20 requests per 10 seconds.
...
Distributed Rate Limit requires shared storage to be enabled. Configure either xref:conn-distributed-shared-storage-config.adoc[distributed shared storage] or xref:conn-shared-storage-config.adoc[Redis shared storage].
...
|Distributed | When enabled, the quota is shared among all Omni Gateway replicas. Distributed rate limiting requires shared storage to be enabled. | checked or unchecked
...
|Block on unknown quota  | For policies using distributed rate limiting, block requests if the policy cannot retrieve the quota from shared storage. If not enabled and the quota can't be retrieved, no rate limiting is applied. | checked or unchecked
|===
...
image::rate-limiting-generic-example.png["A timeline of accepted and rejected requests in two time windows",80%]
...
image::rate-limiting-configure-identifiers-example.png["A line chart illustrating the acceptance and rejection of GET and POST requests over time",80%]
In the example:
...
image::rate-limiting-configure-identifiers-dataweave.png["A timeline that displays both accepted and rejected requests, organized by time intervals",80%]
...
For example, suppose the DataWeave expression uses the IP address as the identifier in an Omni Gateway instance that is public to the internet. If every public IPv4 IP on the internet makes a request to this instance, there will be 3,706,452,992 algorithms running in a single instance.
