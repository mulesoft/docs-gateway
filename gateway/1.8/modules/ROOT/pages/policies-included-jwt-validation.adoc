...
If Client ID validation is enabled, the Client ID is not only propagated as a claim, but also as a reserved authentication property which can be accesed by using the following Dataweave 2.0 expression:
