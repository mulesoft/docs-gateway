...
| By default, the value will be extracted using the expression `#[vars.claimSet.client_id]` as specified in the https://datatracker.ietf.org/doc/html/draft-ietf-oauth-token-exchange-16[Oauth 2.0 token exchange draft^].
...
For more information about the algorithms in a JWS structure, refer to the  https://datatracker.ietf.org/doc/html/rfc7518[JWA RFC standard^].
...
To access the claim values you can use the following DataWeave expression by replacing `<claimName>` with the name of the desired claim to access:
