...
|Optional
|Empty
|The policy configuration. The content of this field depends on the specified policy. See xref:flex-gateway-secure-apis.adoc[the list of available policies]. To use environment variables in `config` fields, refer to <<environment-variables>>. 
