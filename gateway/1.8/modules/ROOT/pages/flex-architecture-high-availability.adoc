...
This implementation is similar to <<implementation-1a>> and has the same considerations. However, Redis replication is not required because requests are sent to specific regions. To have the same API with the same policies served in two regions, use multiple upstreams to direct the traffic to the most convenient region. To learn more about multiple upstreams, see:
...
Configure high availability or disaster recovery zones in each region to ensure API availability:

image:Flex-Gateway-DR-4.png["A detailed view of implementation 3B, which contains the necessary services to enable direct customer requests to the closest region."]
