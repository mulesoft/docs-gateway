...
+
NOTE: By default, Flex Gateways running in Kubernetes or Openshift use a readiness probe. If there's no API deployed to your Flex Gateway instance, the readiness state is `false`. This is not an issue with custom readiness probe configurations.
+
