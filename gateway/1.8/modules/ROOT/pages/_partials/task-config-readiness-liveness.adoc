...
+
NOTE: By default, Flex Gateways running in Kubernetes or Openshift use a readiness probe. If there's no API deployed to your Flex Gateway instance, the readiness state is `false`. If you do not plan to deploy an API to the gateway, omit the `--wait` flag in the Helm chart used to deploy Flex Gateway. Not doing so might result in an installation failure because Helm waits for a readiness state of `true`. This is not an issue with custom readiness probe configurations or gateways that have an API configured.
