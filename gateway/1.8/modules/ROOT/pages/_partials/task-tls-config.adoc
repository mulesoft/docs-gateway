...
By default, Flex Gateway supports outbound TLS to communicate with upstream services that require a secure communication channel. For default outbound communication, Flex Gateway uses the default TLS 1.2 ciphers. To find the ciphers, see <<supported-ciphers>>. Applying a TLS context to an upstream overrides the default TLS context for that upstream. Because the default ciphers might change for later Flex Gateway versions context, you can apply a TLS context to an upstream to ensure it remains the same.
