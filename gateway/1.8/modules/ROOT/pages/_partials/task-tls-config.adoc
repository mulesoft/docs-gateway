...
By default, Flex Gateway supports outbound TLS to communicate that require secure communication. For default outbound communication, Flex Gateway uses the default TLS 1.2 ciphers. To find the ciphers, see <<ciphers, Flex Gateway Supported Ciphers>>. Applying a TLS context for outbound communication overrides the default outbound TLS support.
