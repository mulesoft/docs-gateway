...
In the following diagram, Flex Replicas in different availability zones have distinct Kubernetes deployments. Region B provides high availability but only receives traffic is region A is unhealthy:
...
image:high-availability-3a.png["A detailed view of implementation 3A, which contains the necessary services to enable route customer requests to the closest region."]
...
image:high-availability-3b.png["A detailed view of implementation 3B, which contains the necessary services to enable route customer requests to the closest region."]
