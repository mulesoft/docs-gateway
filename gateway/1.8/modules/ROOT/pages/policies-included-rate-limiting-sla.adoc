...
[NOTE]
====
A Rate-Limiting SLA policy applied to a Flex Gateway API is scoped to replicas, not the gateway. Enable distributed rate limiting to share the request quota across Flex Replicas.
====
