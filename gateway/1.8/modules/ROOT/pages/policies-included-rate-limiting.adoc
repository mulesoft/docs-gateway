...
A Rate Limiting policy applied to a Flex Gateway API is scoped to replicas, not the gateway. Enable distributed rate limiting to share the request quota across Flex Replicas. When configuring distributed rate limiting, each Flex Replica services a portion of the requests until there are no requests to service. Configure the distributed rate limiting algorithm to define how many requests each replica services at a time.
...
- policyRef:
  ...
  config:
    ...
    fineTuning: 
      distributionMode: <string> // OPTIONAL, default: remaining
      distributionPercentage: <string> // OPTIONAL, default: 0.1
----
...
| Optional
| `true`
| Distributed: When using interconnected runtimes with this flag enabled, the quota is shared among all nodes. Distributed rate limiting requires shared storage to be enabled. For more information about shared storage, refer to xref:flex-local-shared-storage-config.adoc[].

| `fineTuning.distributionMode`
| Optional
| `remaining`
| Mode of request quota distribution. Sets the distribution algorithm to either service a percentage of the total quota or a percentage of the remaining quota for the current window. Supported values are `remaining` and `total`.

| `fineTuning.distributionPercentage`
| Optional
| `0.1`
| Percentage distributed to each Flex Replica. Either a percentage of the total quota or the remaining quota. If left blank and `clusterizable` is `true`, each replica services `0.1` of the total or remaining quota. Enter `0` for each Flex Replica to service one API request at a time rather than a percentage of the quota.
...
==== Distributed Rate Limiting Policy Resource Configuration Example

Distributed rate limiting requires shared storage to be enabled. For more information about shared storage, refer to xref:flex-local-shared-storage-config.adoc[].

The following configuration reference file configures both shared storage and a distributed Rate Limiting policy: 
...
- policyRef:
  ...
  config:
    ...
    clusterizable: true
    fineTuning:
      distributionMode: remaining
      distributionPercentage: 0.1
----
...
|Distributed | When using multiple Flex Replicas with this flag enabled, the request quota is shared among all nodes. Distributed rate limiting requires shared storage to be enabled. For more information about shared storage, refer to xref:flex-conn-shared-storage-config.adoc[]. | Checked or unchecked
|Distribution Mode | Mode of request quota distribution. Sets the distribution algorithm to either service a percentage of the total quota or a percentage of the remaining quota. | *Remaining* or *Total*
|Distribution Percentage | Percentage distributed to each Flex Replica. Either a percentage of the total quota or the remaining quota. If left blank and *Distributed* is selected, each replica services `0.1` of the total or remaining quota. Enter `0` for each Flex Replica to service one API request at a time rather than a percentage of the quota. | 0.1
