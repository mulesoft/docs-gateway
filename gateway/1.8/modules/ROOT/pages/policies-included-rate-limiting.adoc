...
The following configuration reference file configures both shared storage and the a distributed Rate Limiting policy: 
...
|Distributed | When using multiple Flex Replicas with this flag enabled, the request quota is shared among all nodes. Distributed rate limiting requires shared storage to be enabled. For more information about shared storage, refer to xref:flex-local-shared-storage-config.adoc[]. | checked or unchecked
