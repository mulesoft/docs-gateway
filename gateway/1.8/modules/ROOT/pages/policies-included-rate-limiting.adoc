...
| Optional
| `true`
| Distributed: When using interconnected runtimes with this flag enabled, quota will be shared among all nodes. Distributed rate limiting requires shared storage to be enabled. For more information about shared storage, refer to xref:flex-local-shared-storage-config.adoc[].

| `fineTuning.distributionMode`
| Optional
| `remaining`
| Mode of request quota distribution. Sets the distribution algorithm to either service a percentage of the total quota or a percentage of the remaining quota. Supported values are `remaining` `total`.
