...
A Rate Limiting policy applied to a Flex Gateway API is scoped to replicas, not the gateway. Enable distributed rate limiting to share the request quota across Flex Replicas. Configure the distributed rate limiting algorithm to define how many requests each replica handles at a time.
