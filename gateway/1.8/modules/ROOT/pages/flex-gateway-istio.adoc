...
Integrate Flex Gateway into your Istio Service Mesh to leverage Flex Gateway policies, metrics, and the Anypoint Platform control plane. Implement Flex Gateway as an ingress gateway to manage incoming north-south cluster traffic and internal east-west traffic inside the cluster.
...
To configure the architecture in the previous diagram:
...
. Install Istio:
+
.. Install the Istio base chart that contains the cluster-wide Custom Resource Definitions (CRDs) that must be installed prior to the deployment of the Istio control plane:
+
[source,ssh]
----
