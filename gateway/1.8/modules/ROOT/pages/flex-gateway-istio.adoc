...
Integrate Flex Gateway into your Istio Service Mesh to leverage Flex Gateway policies, metrics, and the Anypoint Platform control plane. Implement Flex Gateway as a ingress gateway to manage incoming north-south cluster traffic and internal east-west traffic inside the cluster.

In the following diagram:

* An Istio virtual service forwards all ingress north-south traffic to Flex Gateway. 
* Flex Gateway protects and monitors the exposed product page service from incoming traffic with an API instance.
* Flex Gateway acts as an ingress gateway managing all east-west traffic between the product page service and the reviews service.
* Traffic between service names spaces is restricted so all traffic must pass through Flex Gateway. If a service does not require Flex Gateway management, you don't need to restrict traffic to it's name space.


image:flex-istio-deployment.png["Flex Gateway deployed using Istio sidecars and a Istio Ingress gateway"]

NOTE: In the diagram, Flex Gateway is running in Connected Mode. If Flex gateway was running is local mode, the Anypoint Platform control plane is not present.
...
. Install Istio:
+
.. Install the Istio base chart that contains cluster-wide Custom Resource Definitions (CRDs) that must be installed prior to the deployment of the Istio control plane:
+
[source,ssh]
----
...
.. Create the namespace and setup the label to enable the sidecar injection:
+
[source,ssh]
----
...
. Install the `sleep` application:
+
.. Install `sleep` application to test traffic:
+
[source,ssh]
----
...
.  Create the ingress API instance to forward all incoming traffic in the `bookinfo` namespace to the `productpage` service:
+
** Local Mode:
+
[source,ssh]
...
. Create the `flex` Istio gateway and forward the ingress traffic to Flex Gateway using a istio virtual service:
+
...
----
+
.. Wait a few seconds (approximately 10) and send a test request the `/api/v1/products/1` page:
+
[source,ssh]
----
...
. Protect the ingress API instance by adding the Basic Authentication policy:
+
.. Add the policy:
+
[source,ssh]
...
. Create the `bookinfo-reviews` API instance to forward the incoming traffic to the `reviews` service in the `bookinfo` namespace:
...
. Intercept the traffic between the `productpage` service to the `reviews` service and forward it to `flex` service:
+
...
----
+
.. Send a test request with the `foo` user and `bar` password to the `/productpage`:
+
[source,ssh]
----
...
----
+
.. Check the Flex Gateway logs to ensure the request sent from the `productpage` service to the `reviews` service was handled by Flex. The logs appear similar to the following:
+
[source,ssh]
----
...
----
+
.. Ensure you receive the following `403 forbidden` response:
+
[source,ssh]
----
...
----
+
.. Send the following request to the `details` service to find how to access the `bookinfo` service from the `flex` namespace:
+
[source,ssh]
----
