...
----
+
.. Check the logs to ensure the request sent from the `productpage` service to the `reviews` service was handled by Flex. The logs appear similar to the following:
+
[source,ssh]
----
...
----
+
.. Ensure you receive the following forbidden response:
+
[source,ssh]
----
...
----
+
.. Send the following request to the `details` service to see how you access the `bookinfo` service from the flex namespace:
+
[source,ssh]
----
