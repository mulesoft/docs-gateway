...
Integrate Flex Gateway into your Istio Service Mesh to leverage Flex Gateway policies, logging, and metrics from the Anypoint Platform control plane. Deploy Flex Gateway as an ingress gateway to manage incoming north-south cluster traffic and internal east-west traffic inside the cluster.

To deploy this Flex Gateway use case, use the following architecture diagram and tutorial. The use case is a modified version of the https://istio.io/latest/docs/examples/bookinfo/[Istio Bookinfo Application] with Flex Gateway integrated. In the use case:
...
image:flex-istio-deployment.png["A detailed image of the previously described use case."]
...
.  Create the Flex Gateway ingress API instance to forward all north-south incoming traffic in the `bookinfo` namespace to the `productpage` service:
+
** Local Mode:
+
[source,ssh]
...
. Protect the Flex Gateway API instance with the Basic Authentication policy applied:
+
.. Add the policy:
+
[source,ssh]
...
[[create-a-service]]
== Create a Service to Service Traffic
...
----
+
.. Check the Flex Gateway logs to ensure the request sent from the `productpage` service to the `reviews` service passed through the Flex Gateway. The logs appear similar to the following:
+
[source,ssh]
----
