...
To integrate Flex Gateway, use the following architecture diagram and tutorial. The use case is a modified version of the https://istio.io/latest/docs/examples/bookinfo/[Istio Bookinfo Application] with Flex Gateway integrated. In the use case:
...
. <<forward-ingress-traffic, Forward ingress traffic>>.
. <<create-a-service, >>.
