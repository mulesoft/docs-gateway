...
metadata:
  name: orders-api
  annotations:
    flex-gateway.ingress.kubernetes.io/api-instance: ingress-http.gateway
spec:
  ingressClassName: my-custom-name
  rules:
  - http:
      paths:
      - path: /api/orders(/.*)
...
----
+
The `annotations` field namespace value must match the namespace specified when the API was defined.
+
