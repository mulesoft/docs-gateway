...
If the injected header already exists in the message attributes, the policy creates a new header without overriding the value of the existing one. However, you cannot overwrite or inject the `host` header.
