...
| Payload | 1 MB | This limit applies only to buffering the payload (such as logging the payload using message logging). If you're not accessing the payload, there's no limit to payload size. + 
Payloads that exceed this limit receive a 413 response.
| APIs per gateway instance | {gateway-api-max} | Exceeding this limit doesn't cause errors, but isn't supported.
...
| Contracts per API | 1,000 | Exceeding this limit doesn't cause errors, but isn't supported.
| Contracts per gateway instance | 10,000 | Exceeding this limit doesn't cause errors, but isn't supported.
|===
