...
* `apiVersion`
* `receivedTs` (rfc3339)
* `repliedTs` (rfc3339, available only during response)
* `runtimeVersion` (eg `v1.7.0`)
* `hostId` (eg `my-host`)
* `remoteAddress`
* `method`
* `requestPath`
* `reqId` (traceId)
* `requestDisposition` (`PROCESSED` or `VIOLATION`, available only during response)
* `request.headers` (all request headers)
* `violation.clientId` (available only during response)
* `violation.clientName` (available only during response)
* `violation.policyName` (available only during response)
* `violation.violationType` (`VIOLATION` or `ERROR`, available only during response)
