...
There are multiple reasons Flex Gateway may not be able to connect to a platform endpoint, such as an error in the gateway's environment or a network issue. How you troubleshoot your error is dependent on the error returned.

If Flex Gateway cannot to a specific URL, it returns an error message that includes the URL that was unreachable and the error message returned from the connection attempt. The error message appears similar to the following:
