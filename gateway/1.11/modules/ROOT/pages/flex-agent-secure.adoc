...
Flex Gateway supports both the Model Context Protocol (MCP) and the Agent2Agent (A2A) Protocol, enabling you to protect your agents, brokers, LLM, and MCP servers. Flex Gateway provides centralized oversight, agent visibility, logging, and valuable insights, ensuring effective governance and security for your agent-based architecture. Flex Gateway secures agent interactions by enforcing policies across agent connections and by monitoring agent activities for enhanced visibility.
...
Flex Gateway MCP and A2A server instances also support Flex Gateway's other policies. For example, use policies such as xref:policies-included-rate-limiting.adoc[] or xref:policies-included-spike-control.adoc[] to manage the number of requests to MCP and A2A servers or use the xref:policies-included-message-logging.adoc[] to monitor agent interactions.
...
* https://modelcontextprotocol.io/docs/getting-started/intro[MCP Protocol spec]
