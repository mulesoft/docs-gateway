...
* Sends a modified response to the MCP server that notes the request was declined and includes the reason being `PII data requested or found in user response`.

The policy doesn't send a response to the client when PII is detected.

The policy only scans JSON-RPC responses that contain a `result` field, which represents user responses to MCP server elicitation prompts. JSON-RPC requests with a `method` field aren't scanned and continue normally.
