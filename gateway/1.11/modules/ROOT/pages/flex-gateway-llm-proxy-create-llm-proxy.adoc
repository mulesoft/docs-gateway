...
. Retrieve your API Key from your LLM Provider.
...
+
NOTE: Only Open AI format is supported for Limited Availability GA. For more information about the request format, see xref:flex-gateway-llm-proxy-request.adoc[LLM Proxy Requests].
.. Define a *Base path*.
...
You can define six prompt topics each with ten utterances between your two routes. Independent from the route topics, you can define six deny list topics each with ten utterances. Each utterance can be up to 500 characters.
