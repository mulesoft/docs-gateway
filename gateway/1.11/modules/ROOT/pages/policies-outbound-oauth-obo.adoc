...
|Token Endpoint |OAuth 2.0 token endpoint URL. Must support the selected flow (RFC 8693 or Microsoft Entra OBO).

|Target Type |Parameter type for specifying the target service. Applies only to OAuth 2.0 Token Exchange flow. Supported values:

* Audience: Logical identifier of the target service (default)
* Resource: Physical URI of the target resource (RFC 8707)
|Target Value |Target audience URI or resource URI for the exchanged token. Required for OAuth 2.0 Token Exchange flow; not used for Microsoft Entra OBO.

|Scope |OAuth 2.0 scope to request. Optional for OAuth 2.0 Token Exchange (RFC 8693). Required for Microsoft Entra OBO (for example, `api://downstream-client-id/.default`).
