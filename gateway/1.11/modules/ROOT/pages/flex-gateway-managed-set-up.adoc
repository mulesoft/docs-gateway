...
IMPORTANT: Flex Gateway is supported on Runtime Fabric as a limited availability GA release. For access, contact your sales representative.
...
* xref:runtime-fabric::create-custom-namespace.adoc[Create a Runtime Fabric Namespace for Deployments]

* Run this command to enable the `ManagedFlexGateway` resources in your Runtime Fabric namespace:
+
[source,bash]
----
# When performing the helm upgrade for the first time, during Runtime Fabric creation:
helm upgrade runtime-fabric rtf/rtf-agent -n <runtime-fabric-namespace> --version <target-rtf-version> --set global.crds.flex.enabled=true

# When enabling Managed Flex Gateway for an existing runtime fabric:
helm upgrade runtime-fabric rtf/rtf-agent -n <runtime-fabric-namespace> --reset-then-reuse-values --set global.crds.flex.enabled=true
----
...
Properties::
+
To configure how long Managed Omni Gateway waits for responses or idle connections, configure the timeout properties:
+
[cols="1a,1a"]
|===
| Parameter | Description
| *Upstream Connection Idle Timeout* | The time an upstream connection can remain idle without receiving client requests. Upstream responses aren't included in this timeout.
...
|===
+
To limit the number of connections and requests to an upstream service, configure the circuit breaker thresholds:
+
[cols="1a,1a"]
|===
| Parameter | Description
| *Maximum Connections* | Maximum number of connections per API to an upstream.
| *Maximum Pending Requests* | Maximum number of queued requests while waiting for a connection.
| *Maximum Requests* | Maximum number of outstanding requests to an upstream.
| *Maximum Retries* | Maximum number of outstanding retries.
| *Maximum Connection Pools* | Maximum number of connection pools concurrently initiated to an upstream.
|===
+
Logging::
+
