...
| Optional
| `rsa`
| String containing the key signing method. Supported values: `rsa`, `hmac`, `none`
...
| RSA, ES (supported in Mule 4.0+ and Flex 1.1+), HMAC, None.

| JWT Signing Key Length
| Specify the length of the key (in the case of the HMAC algorithm) or the algorithm (in the case of RSA or ES) used for the signing method. +
...
Ignore this field if you selected *none* as JWT Signing Method.
| A 32, 48 or 64 characters long shared secret in case HMAC was the selected JWT Signing Method or the PEM Public Key without the header nor the footer in case of selecting RSA or ES.
...
* *Asymmetric algorithms* - ES and RSA using SHA-256, SHA-384, and SHA-512. ES supported in Mule 4.0+ and Flex 1.1+.
