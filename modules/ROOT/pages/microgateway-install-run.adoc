...
== Install and Run as a Ubuntu Service

In addition to "sandboxing" Flex Gateway as a Docker container image, you can install and run Flex Gateway as a Ubuntu service. The Ubuntu instance is typically run virtually as a node in a virtual machine cluster, thereby guaranteeing that each virtual machine (with its running Flex Gateway) is standalone and encapsulated. 

The following procedure demonstrates how to install and run Flex Gateway as a service in a Ubuntu instance.
