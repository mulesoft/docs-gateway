...
In addition to sandboxing Flex Gateway as a Docker container image, you can install and run Flex Gateway as an Ubuntu LTS (Focal Fossa) service. The Ubuntu instance is typically run virtually as a node in a virtual machine cluster, thereby guaranteeing that each virtual machine (with its running Flex Gateway) is standalone and encapsulated. 
...
You can install Flex Gateway as an Ingress controller - a specialized Kubernetes application that routes external traffic to internal applications and services. The following procedures demonstrate how to install and run Flex Gateway as an Ingress controller. Installation phases are divided by typical roles, and should be run sequentially (each role depends on the completion of previous roles' procedures): 
...
. Apply the `access-log` policy to an `ingress-http` target of type `ApiInstance`, for viewing in Grafana: 
+
----
cat <<EOF | kubectl apply -f -
---
...
----
+
The Grafana UI contains four graphs: *QPS by Status Code*, *95th percentile of Request Time*, *Runtime Logs*, and *Access Logs*. The graphs resemble the following: 
+
image:grafana-request-time.png[65%,65%]

. Run the following Grafana Explorer queries to view additional logs: 
+
* Runtime logs
+
----
