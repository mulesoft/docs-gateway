...
The following procedures demonstrate how to install and run Flex Gateway as a Kubernetes Ingress controller.

=== Dependencies

* https://docs.docker.com/compose/install/[Docker^] - a platform that delivers applications in packages called containers.
* https://k3d.io/v5.0.0/#installation[K3d^] - used to creates Kubernetes clusters.
* https://kubernetes.io/docs/tasks/tools/#kubectl[kubectl^] - used to interact with Kubernetes clusters.
* https://helm.sh/docs/intro/install/[Helm^] - used to install Peregrine, the monitoring tools and applications.
...
. Download the https://drive.google.com/file/d/1eFxLbBIHHgOxL8XnVmBhwYzom1Xd-gfT/view?usp=sharing[Flex Gateway container^] and import the Docker image: 
+
----
...
. Download the https://drive.google.com/file/d/1oDHbwk46TsdO_GaJIefo8fAMFf7CDhYV/view?usp=sharing[grafana-values.yaml^] file and configure Grafana:
+
----
...
. Install Flex Gateway as an Ingress: 
+
----
...
. Verify that the Flex Gateway Ingress is running:
+
----
...
. Configure logging to forward both `runtimeLogs` and `accessLogs` to Loki, for viewing in Grafana: 
+
----
cat <<EOF | kubectl apply -f -
---
...
. Apply the *access-log* policy to the `ApiInstance` *ingress-http*, for Grafana viewing: 
+
----
cat <<EOF | kubectl apply -f -
---
...
. Apply the *rate-limit-local* policy to the `ApiInstance` *ingress-http*: 
+
----
cat <<EOF | kubectl apply -f -
---
...
=== Procedure: As the Developer

. Install the *httpbin* application, and create an ingress that routes traffic from `/httpbin/` to the `ApiInstance` *ingress-http* in namespace *gateway*: 
+
----
...
. Install the *whoami* application, and create an ingress that routes traffic from `/whoami/`` to the `ApiInstance` *ingress-http* in namespace *gateway*: 
+
----
...
----
+
The following headers added by the *rate-limit* policy should be returned: 
+
----
< x-ratelimit-remaining: 1
< x-ratelimit-limit: 3
< x-ratelimit-reset: 6871
----

. Luanch the Grafana dashboard to view logs and traffic:
+
----
