...
| Optional
| Empty Array
| List of origins to be included in group. An origin is e.g. http://www.the-origin-of-time.com

| `originGroups.origins.accessControlMaxAge` 
| Optional
| 30
|  Gives the value in seconds for how long (in seconds) the response to the preflight request can be cached without sending another preflight request
...
| Optional
| Empty Array
| header adds the specified headers to the allowlist that JavaScript in browsers is allowed to access.
