...
The gateway's status will be disconnected for now. You need to start the gateway to connect it.

Before executing the start command below, replace the `.conf` file UUID with the UUID of the `.conf` file
in the Flex Gateway configuration directory, `/etc/peregrine/conf.d/custom/` and replace the MGW_NAME` with
the name you have given your Linux service.

=== Start command

[source]
----
service flex-gateway start \
-e MGW_RTM_ARM_AGENT_URL=wss://arm-mcm2-service.kdev.msap.io:443/agent \
-e MGW_DATASOURCE_RTM_ENABLED=true \
-e MGW_RTM_ARM_AGENT_CONFIG=/etc/peregrine/rtm/076ccaab-86a7-444d-8fbb-6620bafde086.conf \
-e MGW_NAME=ubuntu
