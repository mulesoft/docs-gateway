...
For example, five replicas have been created in a single Flex Gateway registration. You configure the Rate Limiting policy for ten requests per minute. This request limit is applied *per* replica, not the five replicas.
====
...
The `#[attributes.method]` DataWeave expression creates one group for each available method in HTTP, for example, the policy rate-limits GET requests independently from POST requests.
...
|Clusterizable | The option that shares the quota defined for a group among all of the nodes of a Mule cluster. This option is not available for Flex Gateway. | checked or unchecked
...
image::rate-limiting-configure-clusters.png[Rate Limiting Policy Configure Clusters,55%]
