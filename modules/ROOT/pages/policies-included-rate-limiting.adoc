...
| `rateLimits` 
| An array containing one or more rate limit specifications 
|
----
rateLimits:
  - maximumRequests: 3
...
| `exposeHeaders` 
| The option that defines whether to expose the  <<response-header,x-ratelimit headers>> as part of the response 
| A boolean
| No

| `clusterizable` 
| The option that shares the quota defined for a group among all of the nodes of a Mule cluster 
| A boolean 
| No
