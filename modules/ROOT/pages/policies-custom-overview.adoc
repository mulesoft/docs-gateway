...
* Top-level @context
+
References the vocabulary used for the extra type annotations. The aliases used for namespacing the vocabulary are a optional, but common names can be `security` or `config`.
+
...
See xref:docs/mulesoft/exchange/to-create-an-asset#create-custom-policy-definition-asset[Create a Custom Policy Definition Asset] for more information.
...
See xref:docs/mulesoft.com/exchange/add-policy-implementation-files-to-published-policy-definition[Add Policy Implementation Files to Published Policy Definition] for more information.
