...
A Flex Replica is a server where the Flex Gateway reverse proxy is running. By default,
Flex Gateway runs a single replica. Running multiple replicas enables scaling, load balancing,
...
* xref:flex-conn-rep-run.adoc[Add Flex Replicas in Connected Mode]: If you are running Flex Gateway in Connected Mode, add Flex Replicas in Local Mode.
