= Implementing a Flex Gateway Custom Policy in Rust
...
----
+
The JSON that needs to be parsed contains a single field called `secret-value`.
+
...
+
Because this example custom policy is HTTP-based, you must implement an HTTP context using the `HttpContext` trait. Add the following HTTP context code:
+
----
...
+
You can now implement core functionality. The `proxy-wasm` ABI retrieves the header value of the incoming request and then compares the value against `secret-value`.
+
