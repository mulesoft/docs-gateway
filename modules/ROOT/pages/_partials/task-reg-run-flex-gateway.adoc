...
Before you begin, ensure that you have xref:flex-install.adoc[installed the latest version of Flex Gateway].
...
NOTE: Append the `--split=true` flag if you want to output two registration files: `registration.yaml` and `certificate.yaml`. Otherwise, by default all registration information is output to `registration.yaml`.

IMPORTANT: The generated registration files are credentials for you to connect your Flex Gateway. If you lose them you can no longer connect your Flex Gateway.
