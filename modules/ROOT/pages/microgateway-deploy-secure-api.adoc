...
Modify configuration files to deploy and secure services. You can do so directly within the Docker container (or Ubuntu) filesystem. Alternatively, for Kubernetes environments, you can apply configuration files via the Kubectl command line. With either method, the format of the configuration files is the same.

This procedure describes how to configure Flex Gateway by defining YAML, and how to deploy a secure service.
