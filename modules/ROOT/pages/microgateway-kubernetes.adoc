...
+
----
k3d cluster create flex-gateway-1 --k3s-arg "--disable=traefik@server:0" --port '80:80@server:0'
----
+
...
+
----
helm -n gateway upgrade -i --wait --create-namespace ingress peregrine/peregrine --set image.name=mulesoft/flex-gateway:1.0.0-alpha.1,ingress.enabled=true
----
+
...
. Launch the Ingress and forward the local connection to a specific port: 
+
----
kubectl --namespace gateway port-forward service/ingress 8000:80
----
+
...
. Open a new terminal and verify that the URL and port are accessible: 
+
----
...
+
----
curl -v -u foo:bar http://127.0.0.1:8000/api/users/1
----
+
