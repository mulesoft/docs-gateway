...
For mule 4 and mule 3 this policy works with both Apache log4j and Apache log4j2. For more information, see https://www.tutorialspoint.com/log4j/log4j_logging_levels.htm[log4j Logging Levels^].
...
. A request is sent to the API.
. The message is logged if the following conditions are true:
** The level and category of the log defined in the policy are included in the appropriate gateway configuration file.
** Either the conditional field of the policy was not set, or was set and the condition was met.
. The API response is returned. Message Logging Policy does not interfere with the execution of any policy nor its flow.
. The message appears in the application log.
...
* Flex:
`<date> [flex-gateway-envoy][<level>] wasm log <policy-name>.<namespace>.<api-name>.<namespace>.<scv>  main: <category>: <message>`
...
`category` indicates a custom prefix to the log message. The default value is `logging.<policy name>`.
...
+
image::apim-message-logging-policy-states-2.png[State after logging an API flow]
* Error handler after API flow (Mule 4 only):
+
image::apim-message-logging-policy-states-3.png[State after the error handler executes]
...
| REQUIRED
| N/A
| dataweave expression that resolves to the message to log

| `loggingConfiguration.itemData.conditional`
| OPTIONAL
| true
| dataweave expression that resolves whether message should be logged
...
==== Flex Resource Configuration Example
...
Note for Mule 4 API gateway: If the payload is used as part of the message log, do not configure the Listener as nonrepeatable.|Required
...
Following this configuration, whenever customers access the Les Vetments catalogue or add or remove items from their carts, or when an item is added or removed from the Les Vetments catalog, logs are generated in Mule 4:
