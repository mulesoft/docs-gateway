...
* xref:api-manager::rate-limiting-sla-policy.adoc[Rate-Limiting SLA]
* xref:api-manager::external-oauth-2.0-token-validation-policy.adoc[Mule OAuth 2.0 Access Token Enforcement] (Mule v3.8.5 or later)
* xref:api-manager::policy-ping-federate.adoc[PingFederate OAuth 2.0 Token Enforcement] (Mule v3.8.5 or later)
* xref:api-manager::policy-openid-connect.adoc[OpenID Connect OAuth 2.0 Token Enforcement] (Mule v3.8.5 or later and Flex v1.0.0 or later)
* xref:api-manager::policy-mule4-jwt-validation.adoc[JWT Validation] (Mule v3.8.5 or later and Flex v1.0.0 or later)
...
// ----
// - policyRef:
//     name: client-id-enforcement-flex
//   config:
// ----

=== Connected Mode

When you apply the policy to your API from the UI, a list of parameters are displayed based on whether your environment includes Flex, Mule or non-Mule applications.

==== Configuring the Parameters for Flex and Mule
...
* Custom Expression: Accepts an expression each for client ID and client secret, indicating where to extract the credentials from the request. For Mule 3, use MEL Expressions and for Mule 4 and Flex, use a DataWeave 2.0 expression. Use this option to send the credentials in a custom header or any other format. You can leave the field blank to configure the client secret requirement as optional.
...
In this example, the requester must send the two specified query parameters with the request. Although this is a supported configuration, it poses possible security risks. The recommended method is to use headers. DataWeave 2.0 is fully supported in Mule 4 as wells as a subset in Flex.
//TODO: LINK to DW Flex support for v1.0.0 docs TBD.
...
=== Obtaining Credentials Using HTTP Request Payload (Mule only)
...
* xref:api-manager::rate-limiting-sla-policy.adoc[Reviewing Rate-Limiting SLA Policy]
* xref:api-manager::policy-ping-federate.adoc[PingFederate OAuth 2.0 Token Enforcement]
* xref:api-manager::policy-openid-connect.adoc[OpenID Connect OAuth 2.0 Token Enforcement]
* xref:api-manager::defining-sla-tiers.adoc[Reviewing SLA Tiers concepts on API Manager]
* xref:api-manager::api-gateway-encryption-mule4.adoc[API Gateway Startup Encryption]
