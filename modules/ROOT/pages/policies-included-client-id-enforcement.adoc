...
| OPTIONAL
| customExpression
| Either `httpBasicAuthenticationHeader`` or `customExpression`` values are allowed. Indicates whether to extract header from the Basic Auth header or use a custom expression

| clientIdExpression
| OPTIONAL
| #[attributes.headers['client_id']]
| Custom expression that is used in case `credentialsOriginHasHttpBasicAuthenticationHeader` is set to `customExpression`
