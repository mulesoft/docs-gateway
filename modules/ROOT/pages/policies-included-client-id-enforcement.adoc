...
* xref:api-manager::policy-openid-connect.adoc[OpenID Connect OAuth 2.0 Token Enforcement] (Mule v3.8.5 or later)
* xref:api-manager::policy-mule4-jwt-validation.adoc[JWT Validation] (Mule v3.8.5 or later)
...
In Local Mode, you apply the Client ID Enforcement policy to your API via declarative configuration files. Refer to the following policy definition and table of parameters:

----
- policyRef:
    name: client-id-enforcement-flex
  config:
    credentialsOriginHasHttpBasicAuthenticationHeader: // OPTIONAL <enum> default: customExpression
    clientIdExpression: // OPTIONAL <dataweave expression> default: #[attributes.headers['client_id']]
    clientSecretExpression: // REQUIRED <dataweave expression>
...
| OPTIONAL
| customExpression
| Either httpBasicAuthenticationHeader or customExpression values are allowed. Indicates weather to extract header from the Basic Auth header or using a custom expression

|  clientIdExpression
| OPTIONAL
| #[attributes.headers['client_id']]
| Custom expression that is used in case field `credentialsOriginHasHttpBasicAuthenticationHeader` is set to `customExpression`

|  clientSecretExpression
| REQUIRED
|
| Expression to extract the client secret
