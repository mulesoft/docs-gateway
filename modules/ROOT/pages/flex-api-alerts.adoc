...
//verify whether this link name needs to be changed
...
After an alert is triggered, the first set of two notification emails is sent and Flex Gateway stops listening for alerts until the next alert period begins. This technique prevents repeated notification emails.
...
== Testing API Alerts
//for the testing section, use deployment content from the Flex Gateway documentation. NOTE - verify I have the correct page link for the testing section
...
If you have been granted the *Organization Administrators* role for the master organization, you can view a list of alerts that are generated for APIs in the Flex Gateway.
