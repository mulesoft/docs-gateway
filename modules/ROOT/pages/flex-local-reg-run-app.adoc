= Registering and Running in Local Mode with a Connected App
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images
// H1 and prerequisites
include::partial$task-reg-run-flex-gateway.adoc[tags=app-icon-selection;prerequisites-heading;app-prerequisites;prerequisites;environment-prerequisites]

// Register and run with a connected app in a Docker container
include::partial$task-reg-run-flex-gateway.adoc[tags=app-docker-heading;reg-command-intro;app-replace-content;environment-replace-content;replace-content;reg-command-heading;reg-command-1;docker-reg-command;app-reg-command;environment-reg-command;reg-command-2;after-reg;start-command]

// Register and run with a connected app as a Linux service
include::partial$task-reg-run-flex-gateway.adoc[tags=app-linux-heading;reg-command-intro;app-replace-content;environment-replace-content;replace-content;reg-command-heading;reg-command-1;linux-reg-command;app-reg-command;environment-reg-command;reg-command-2;after-reg;create-config-folder-file;config-content;restart-command]

== See Also

* xref:flex-local-reg-run-up.adoc[Register and Run in Local Mode with a Username and Password]
* xref:flex-gateway-getting-started.adoc[Getting Started with Flex Gateway]
* xref:flex-conn-reg-run.adoc[Register and Run in Connected Mode]