...
Rate Limiting SLA policy is not supported in Local mode.
...
|Clusterizable | The option that shares the quota defined for a group among all of the nodes of a Mule cluster. This feature is not available for Flex gateway and requires the Mule runtime running as part of a Mule cluster.|
...
=== Choosing Persistence for Your Rate-Limiting SLA Policy (Mule only)q
