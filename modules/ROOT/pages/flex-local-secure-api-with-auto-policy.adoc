= Secure an API with an Automated Policy
...
The following procedures demonstrate applying a simple resource-based YAML configuration for an API with multiple upstream services, all secured with an automated authentication policy. 
...
Additionally, refer to xref:policies-automated-overview.adoc[Automated Policies] for more information.
...
Two APIs secured with the an automated basic authentication policy are now running behind Flex Gateway.
