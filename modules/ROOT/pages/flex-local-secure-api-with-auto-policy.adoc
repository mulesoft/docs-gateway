...
----
+
The `PolicyBinding` resource applies a basic authentication policy to both `ApiInstance` resources. You accomplish this by specifying a customizable label in `spec.targetRef.selector.labels`. The specified label matches the label defined in the `ApiInstance` `metadata.labels` property, which associates the automated policy with that resource.
...
----
+
The `PolicyBinding` resource applies a basic authentication policy to both `ApiInstance` resources. You accomplish this by specifying a customizable label in `spec.targetRef.selector.labels`. The specified label matches the label defined in the `ApiInstance` `metadata.labels` property, which binds the policy to that resource.
