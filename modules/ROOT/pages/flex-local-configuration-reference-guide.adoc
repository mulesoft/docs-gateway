...
Define a desired gateway state by creating a `Configuration` entity. `Configuration` entities specify several runtime configuration aspects for Flex Gateway itself, such as logging. The definition includes the following: 
...
|Required
|N/A
|The configuration object that defines gateway characteristics. Objects include: `logging`, `tls`, `internalMetrics`.
