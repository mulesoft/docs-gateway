...
The `sharedStorage` object configures the gateway for shared storage. Setting up https://redis.io/[Redis^] is recommended for production workflows, though defining it is optional. If Redis is not defined, shared storage services at port 4000 are still available but will use an in-memory implementation. 
