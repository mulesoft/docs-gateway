...
Anypoint Flex Gateway supports two YAML configuration models: inline, and resource-based (common in Kubernetes). Refer to the xref:flex-local-configuration-reference-guide.adoc#examples[Examples] section for examples of both.

Additionally, examples of inline configuration definitions are included in the following installation procedures: 

* xref:flex-installing-as-a-linux-service.adoc[Install and Run as a Linux Service]
* xref:flex-installing-using-docker.adoc[Install and Run using Docker]

An example of a resource-based configuration definition is included in the following installation procedure: 

* xref:flex-installing-using-kubernetes.adoc[Install and Run as a Kubernetes Ingress Controller]
...
`protocol://host:port/path`, where `path` is optional.
...
|Required
|N/A
|The service address (and port). Supported format: `protocol://host:port`
...
|Optional
|Empty
|An array of rules for this route. Refer to xref:microgateway-configuration-reference.adoc#policy-binding[spec.rules] in Policy Binding.
...
|Required
|N/A
|The policy name. See xref:microgateway-policies-reference.adoc[Policy Definition Guide] for the list of available policies.
...
|Optional
|Empty
|The policy's configuration. Refer to xref:microgateway-configuration-reference.adoc#policy-binding[spec.config] in Policy Binding.

|`spec.policies[#].rules`
|Optional
|Empty
|An array of rules for applying this policy to the API Instance. Refer to xref:microgateway-configuration-reference.adoc#policy-binding[spec.rules] in Policy Binding.
...
The following resource binds a `http-basic-authentication-flex` policy to the API instance -  requiring requests to include the basic credentials defined in `config.username` and `config.password`:
...
|Required
|N/A
|The service address URL, including protocol, host and port. Supported format: `protocol://host:port`
...
Define a desired gateway state by creating a `Configuration` entity. `Configuration` entities specify several runtime configuration aspects for Flex Gateway itself, like logging, shared storage, etc. The definition includes the following: 
...
spec: 
  ...
  internalMetrics: # internal metrics configuration
----
...
|Required
|N/A
|The configuration object that defines gateway characteristics. Objects include: `logging`, `sharedStorage`, `tls`, `internalMetrics`.

|===

=== Logging

The `logging` object configures the delivery of runtime/access logs enabled via the message logging policy. Logs are delivered to any supported https://docs.fluentbit.io/manual/v/1.7/pipeline/outputs[Fluentbit 1.7.9^] output.
...
The `sharedStorage` object configures the gateway for storage shared. Setting up https://redis.io/[Redis^] is recommended for production workflows, though defining it is optional. If Redis is not defined, shared storage services at port 4000 are still available but will use an in-memory implementation. 
