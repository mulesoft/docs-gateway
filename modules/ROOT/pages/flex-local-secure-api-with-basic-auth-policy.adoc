...
|image:install-kubernetes-logo.png[20%,20%,xref="flex-local-secure-api-with-basic-auth-policy.adoc#publish-an-api-running-behind-flex-gateway-on-kubernetes"]
...
|xref:flex-local-secure-api-with-basic-auth-policy.adoc#publish-an-api-running-behind-flex-gateway-on-kubernetes[Publish an API running behind Flex Gateway on Kubernetes]
...
. Access the `products` API without providing authentication information: 
+
[source]
----
...
. Retrieve the first product in your store by accessing the `products` API, appending `1` to the address, and providing authentication information:
+
[source]
----
curl -u chris:admin http://localhost:8080/api/users/1 -v
----
+
The response would include the following:
+
...
. Validate rate limiting by executing the following command more than three times in six seconds:
+
[source]
----
curl -u chris:admin http://localhost:8080/api/users/1 -v
...
== Publish an API running behind Flex Gateway on Kubernetes