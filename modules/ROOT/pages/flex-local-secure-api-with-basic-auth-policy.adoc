...
[agent][info] Gateway default/4191c977e1ce: Adding Policy default/animal-facts-api-http-basic-authentication-flex-1
...
[source]
----
curl -u foo:bar http://localhost:8080/api/users/1 -v
----
+
The response would include the following output:
+
...
. Validate rate limiting by executing the following command more than X times in a minute:
+
[source]
----
curl -u foo:bar http://localhost:8080/api/users/1 -v
