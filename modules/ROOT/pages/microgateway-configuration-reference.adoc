...
Anypoint Flex Gateway supports two YAML configuration models: inline, and resource-based (common in Kubernetes). Refer to the xref:microgateway-configuration-reference.adoc#examples[Examples] section for examples of both.
...
The `internalMetrics` object controls whether internal usage metrics are sent to Mulesoft.
...
|`internalMetrics.enabled`
|Optional
|`true`
|Whether internal usage metrics are enabled and sent to MuleSoft.
...
kind: PolicyBinding
metadata:
  name: ingress-http-authentification
spec:
  targetRef:
    name: ingress-http
  policyRef:
