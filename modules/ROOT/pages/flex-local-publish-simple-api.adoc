...
|image:install-kubernetes-logo.png[20%,20%,xref="flex-local-publish-simple-api.adoc#kubernetes"]
...
|xref:flex-local-publish-simple-api.adoc#kubernetes[Publish an API running behind Flex Gateway on Kubernetes ]
...
. Create a configuration file called `orders.yaml` and save it in the Flex Gateway configuration directory: `/etc/peregrine/conf.d/custom/`. This directory can contain multiple configuration files.

. Copy and paste the following into `orders.yaml`:
+
[source]
----
...
[source]
----
curl http://localhost:8080/api/orders/1 -v
----
+
...
. Create a new configuration file with a `.yaml` extension. Name the file `orders.yaml`.

. Copy and paste the following into the `orders.yaml` file: 
+
[source]
----
apiVersion: gateway.mulesoft.com/v1alpha1
