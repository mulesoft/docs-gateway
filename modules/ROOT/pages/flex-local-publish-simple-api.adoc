= Publishing a Simple API
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images

For local mode, you publish an API running behind Flex Gateway by modifying YAML configuration data. API YAML configuration for Docker and Linux can be modified via `.yaml` files. API YAML configuration for Kubernetes can be modified via `kubectl apply`.

The following demonstrates a typical YAML configuration for an unsecured API with a single upstream service. 

[cols="1,1,1"]
|===

|image:install-linux-logo.png[20%,20%,xref="flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-on-linux"]

|image:install-docker-logo.png[25%,25%,xref="flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-in-a-docker-container"]

|image:install-kubernetes-logo.png[20%,20%,xref="flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-as-an-ingress-controller"]

|xref:flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-on-linux[Publish an API running behind Flex Gateway on Linux]

|xref:flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-in-a-docker-container[Publish an API running behind Flex Gateway in a Docker Container]

|xref:flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-as-an-ingress-controller[Publish an API running behind Flex Gateway on Kubernetes]

|===

== Publish an API running behind Flex Gateway on Linux 

Before getting started, ensure that you have:

* Flex Gateway installed and running in local mode. See xref:flex-install.adoc[Installing Flex Gateway] for more information on installing and running the gateway. 
* Your upstream service URL. The following example refers to a fictional `orders-api` service, but you can specify your own API name under `metadata.name`, and specify service details under `spec.services`.

To publish an API behind a gateway running on Linux:

. Create a configuration file with a `.yaml` file extension. Name the file (it can be named anything), then save the file in the Flex Gateway configuration directory: `/etc/peregrine/conf.d/custom/`. This directory can contain multiple configuration files.

. Copy and paste the following into the file`:
+
[source]
----
apiVersion: gateway.mulesoft.com/v1alpha1
kind: ApiInstance
metadata:
  name: orders-api
spec:
  address: http://0.0.0.0:8080
  services:       
    orders:
      address: https://<your orders URL>:<your port>/
      routes:
        - rules:
            - path: /api/orders(/.*)
----

. Save the file. 
+
This forces a refresh, thereby allowing you to modify and add configuration details without having to explicitly stop and start the gateway. 
+
The logs would include the following: 
+
[source]
----
[agent][info] Generating config
[agent][info] Gateway default/18b4e890fe7d: Adding ApiInstance default/animal-facts-api http://0.0.0.0:8080
[agent][info] Gateway default/18b4e890fe7d: Adding Route: &{host: path:/api/orders(/.*) methods: headerConditions:[] profile:0xc0030529f0} => {Kind:Service Name:orders-api-orders Namespace:default}
[agent][info] Gateway default/18b4e890fe7d: Adding Policy default/envoy.filters.http.router
[agent][info] Gateway default/18b4e890fe7d: Adding Service default/monitoring_metrics http://0.0.0.0:9881
[agent][debug] generating service monitoring_metrics.default.svc hostname: 0.0.0.0 port: 9881
[agent][info] Gateway default/18b4e890fe7d: Adding Service default/orders-api-orders https://<your orders URL>:<your port>/
[agent][debug] generating service orders-api-orders.default.svc hostname: <your orders URL> port: <your port>
[agent][info] Writing envoy bootstrap configuration to /tmp/envoy.json
[envoy][info] cds: add 2 cluster(s), remove 2 cluster(s)
[envoy][info] cds: added/updated 1 cluster(s), skipped 1 unmodified cluster(s)
----

. Retrieve all orders by accessing your `orders` API:
+
[source]
----
curl http://localhost:8080/api/orders/ -v
----
+
The response could include the following:
+
[source]
----
[
   {
      id:1,
      userId:1,
      date:2021-10-10,
      products:[{productId:2,quantity:4},{productId:1,quantity:10},{productId:5,quantity:2}]
   }
   /*...*/
   {
      id:20,
      userId:1,
      date:2021-11-14,
      products:[{productId:5,quantity:2},{productId:8,quantity:7},{productId:9,quantity:12}]
   }
]
----

An unsecured API is now running behind Flex Gateway.

== Publish an API running behind Flex Gateway in a Docker Container 

Before getting started, ensure that you have:

* Flex Gateway installed and running in local mode. See xref:flex-install.adoc[Installing Flex Gateway] for more information on installing and running the gateway. 
* Your upstream service URL. The following example refers to a fictional `orders-api` service, but you can specify your own API name under `metadata.name`, and specify service details under `spec.services`.

To publish an API behind a gateway running as a Docker container image:

. Open a terminal and navigate to the directory containing the Flex Gateway image.

. Create a configuration file with a `.yaml` file extension. Name the file (it can be named anything), then save the file in the the directory containing the Flex Gateway image. This directory can contain multiple configuration files.

. Copy and paste the following into the file: 
+
[source]
----
apiVersion: gateway.mulesoft.com/v1alpha1
kind: ApiInstance
metadata:
  name: orders-api
spec:
  address: http://0.0.0.0:8080
  services:       
    orders:
      address: https://<your orders URL>:<your port>/
      routes:
        - rules:
            - path: /api/orders(/.*)
----

. Save the file. 
+
This forces a refresh, thereby allowing you to modify and add configuration details without having to explicitly stop and start the gateway. 
+
The container logs would include the following: 
+
[source]
----
[agent][info] Generating config
[agent][info] Gateway default/18b4e890fe7d: Adding ApiInstance default/animal-facts-api http://0.0.0.0:8080
[agent][info] Gateway default/18b4e890fe7d: Adding Route: &{host: path:/api/orders(/.*) methods: headerConditions:[] profile:0xc0030529f0} => {Kind:Service Name:orders-api-orders Namespace:default}
[agent][info] Gateway default/18b4e890fe7d: Adding Policy default/envoy.filters.http.router
[agent][info] Gateway default/18b4e890fe7d: Adding Service default/monitoring_metrics http://0.0.0.0:9881
[agent][debug] generating service monitoring_metrics.default.svc hostname: 0.0.0.0 port: 9881
[agent][info] Gateway default/18b4e890fe7d: Adding Service default/orders-api-orders https://<your orders URL>:<your port>/
[agent][debug] generating service orders-api-orders.default.svc hostname: <your orders URL> port: <your port>
[agent][info] Writing envoy bootstrap configuration to /tmp/envoy.json
[envoy][info] cds: add 2 cluster(s), remove 2 cluster(s)
[envoy][info] cds: added/updated 1 cluster(s), skipped 1 unmodified cluster(s)
----

. Retrieve all orders by accessing your `orders` API:
+
[source]
----
curl http://localhost:8080/api/orders/ -v
----
+
The response could include the following:
+
[source]
----
[
   {
      id:1,
      userId:1,
      date:2021-10-10,
      products:[{productId:2,quantity:4},{productId:1,quantity:10},{productId:5,quantity:2}]
   }
   /*...*/
   {
      id:20,
      userId:1,
      date:2021-11-14,
      products:[{productId:5,quantity:2},{productId:8,quantity:7},{productId:9,quantity:12}]
   }
]
----

An unsecured API is now running behind Flex Gateway.

== Publish an API running behind Flex Gateway as an Ingress Controller 

Before getting started, ensure that you have:

* Flex Gateway installed and running in local mode. See xref:flex-install.adoc[Installing Flex Gateway] for more information on installing and running the gateway. 
* A Docker image stored in a Docker repository. The following example refers to a fictional `orders-api` application, but you can specify your own Docker repository and Docker image in the `Deployment` configuration resource (under `spec.spec.image`).

To publish an API behind a gateway running as an Ingress Controller:

. Install a Docker image named "orders-api" from a given repository. This can be accomplished by passing YAML configuration data into the `kubectl apply` command. 
+
The following YAML example defines two configuration resources: `Service`, and `Deployment`. The `Service` resource declares the `orders-api` micro-service. The `Deployment` resource declares the desired deployment state by specifying information such as the number of replicas to create, and the Docker image to sideload.
+
[source]
----
cat <<EOF | kubectl apply -f -
---
apiVersion: v1
kind: Service
metadata:
  name: orders-api
  labels:
    app: orders-api
    service: orders-api
spec:
  ports:
  - name: http
    port: 80
    targetPort: http
  selector:
    app: orders-api
---
apiVersion: apps/v1
kind: Deployment
metadata:
  name: orders-api
spec:
  replicas: 1
  selector:
    matchLabels:
      app: orders-api
      version: v1
  template:
    metadata:
      labels:
        app: orders-api
        version: v1
    spec:
      containers:
      - image: <repository>/<dockerfile>
        imagePullPolicy: IfNotPresent
        name: orders-api
        ports:
        - name: http
          containerPort: 3000
        resources:
          limits:
            cpu: 100m
            memory: 64Mi
EOF
----

. Create the ingress to route traffic. This can be accomplished by passing YAML configuration data into the `kubectl apply` command. 
+
The following YAML example defines one configuration resource: `Ingress`.
+
[source]
----
cat <<EOF | kubectl apply -f -
---
apiVersion: networking.k8s.io/v1
kind: Ingress
metadata:
  name: orders-api
spec:
  ingressClassName: ingress-http.gateway
  rules:
  - http:
      paths:
      - path: /api/orders(/.*)
        pathType: ImplementationSpecific
        backend:
          service:
            name: orders-api
            port:
              number: 80
EOF
----

. Retrieve all orders by accessing your `orders` API:
+
[source]
----
curl http://localhost/api/orders/ -v
----
+
The response could include the following:
+
[source]
----
[
   {
      id:1,
      userId:1,
      date:2021-10-10,
      products:[{productId:2,quantity:4},{productId:1,quantity:10},{productId:5,quantity:2}]
   }
   /*...*/
   {
      id:20,
      userId:1,
      date:2021-11-14,
      products:[{productId:5,quantity:2},{productId:8,quantity:7},{productId:9,quantity:12}]
   }
]
----

An unsecured API is now running behind Flex Gateway.

// == See Also
 
// Note: Do not change heading

// Don’t use this section as filler. Link only to relevant related info and not just any info that is “kinda” related.

// Must contain: unordered list
// * xref:file-name.adoc[link-text]
// * xref:file-name.adoc#paragraph-target[link text]
// * xref:repo-name::filename.adoc[link text]
// * https://www.url.com[link-text^]