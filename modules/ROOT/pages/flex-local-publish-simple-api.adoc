...
The following procedures demonstrate a simple YAML configuration for an unsecured API with a single upstream service. 
...
|image:install-kubernetes-logo.png[20%,20%,xref="flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-as-a-kubernetes-ingress-controller"]
...
|xref:flex-local-publish-simple-api.adoc#publish-an-api-running-behind-flex-gateway-as-a-kubernetes-ingress-controller[Publish an API Running Behind Flex Gateway as a Kubernetes Ingress Controller]
...
.. Save the file in the Flex Gateway configuration directory `/etc/peregrine/conf.d/custom/`. This directory can contain multiple configuration files.
...
. Create a configuration file with a `.yaml` file extension:
.. Give the file a custom name.
.. Save the file in the directory containing the Flex Gateway image. This directory can contain multiple configuration files.
