...
include::partial$task-reg-run-flex-gateway.adoc[tags=token-k8s-heading;k8s-reg-command-intro;sub-coll-info;token-replace-content;replace-content;reg-command-heading;reg-command-1;linux-reg-command;token-reg-command;reg-command-2;after-reg;k8s-after-reg;k8s-install-helm-chart;helm-chart-options]
