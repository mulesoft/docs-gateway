...
include::partial$task-reg-run-flex-gateway.adoc[tags=token-linux-heading;reg-command-intro;token-replace-content;replace-content;reg-command-heading;reg-command-1;linux-reg-command;token-reg-command;reg-command-2;after-reg;create-config-folder-file;config-content;restart-command]
