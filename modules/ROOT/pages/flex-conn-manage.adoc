...
include::gateway::partial$task-tls-config.adoc[tags=intro1]
* xref:api-manager::create-instance-task.adoc[Add an API Instance in API Manager] - An API Instance is an instantiation of an API that describes the configuration of the API deployed on one of the following three runtime options: Flex Gateway, Mule Gateway, or Service Mesh.
and can be deployed to an API gateway. 
* xref:api-manager::configure-api-task.adoc[Configure an API Instance Endpoint in API Manager] - There are several optional configurations you can use to customize your API Instance to fit your specific situation. Not all options are available depending on the runtime as well as the API.
* xref:policies-included-apply.adoc[Apply a Policy in API Manager] - A policy extends the functionality of an API and enforces certain capabilities such as security. A policy
