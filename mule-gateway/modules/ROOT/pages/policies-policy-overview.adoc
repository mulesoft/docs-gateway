...
Downloading policy jars can be an expensive operation, specially for policies that have a large size. Therefore, the policy assets, jar and yaml, are never deleted, not even when the policy is unapplied in API Manager.

However, starting from releases and monthly versions released after February 1, 2024, a new configuration property was introduced to automatically delete unused cached files on runtime start up.

The feature can be enabled by adding the following entry in the `wrapper.conf` file:
