...
:page-aliases: policies::policies-policy-overview.adoc, api-manager::policy-mule3.adoc, policies::policies-mule3.adoc, api-manager::policy-mule3-setting-your-api-url.adoc, policies::policies-mule3-setting-your-api-url.adoc
...
Based on Mule runtime engine (Mule), a policy Exchange asset consists of:
...
* API Manager
* One or more API gateway runtimes, or Mule runtime engine (Mule)
...
Each API proxy application receives requests on HTTP or HTTPS URLs specified by the API. The flow referenced for autodiscovery must use an xref:connectors::http/http-listener-ref.adoc[HTTP Listener] component, not other connectors that may use HTTP as an underlying transport. Normally requests are forwarded to the corresponding API implementation, and then the response travels through the API proxy application back to the requesting client application.
