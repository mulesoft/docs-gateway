...
=== Create the Implementation Metadata YAML File
...
* maxRuntimeVersion
+
A semantic version representing the latest runtime version that the implementation binary is compatible with. For example, a mule binary might only be compatible from 4.3.0 and earlier.
+
`supportedJavaVersions`
+
The Java versions supported by the custom policy, either `"8"`, `"11"`, or `"17"`. You can list one or multiple versions. 
