...
+
image::environment-api-manager.png["The credentials for the environment in Mule 3",align=center]
...
+
image::orgid-api-manager.png["A form displays organization information including names, IDs, and a client secret in Mule 3",align=center]
...
--
+
Where `MULE_HOME` is a variable holding the root directory of your Mule runtime installation. For example, `/opt/Mule/mule-enterprise-standalone/`.
...
+
image::org-credentials-config-mule3-af73e.png["A window with configuration settings for API Manager",align=center]
+
