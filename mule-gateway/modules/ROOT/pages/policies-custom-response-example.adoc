...
NOTE: Parameters using the `dependsOnKey` must be `optional: true`.
...
image::custom-response-policy-ui.png["A form that modifies the response payload and status code when a condition is met"]

The following screenshot illustrates the `dependsOnKey` and `dependsOnValue` parameters:

image::depends-on-key-demo.png["Two checkboxes allow the user to evaluate the condition in the request or the response"]
...
image::custom-response-configuration.png["A form with checkboxes and a text field for a DataWeave expression"]
...
image::custom-response-policy-platform.png["A custom response setting with version 1.0.0 selected"]
