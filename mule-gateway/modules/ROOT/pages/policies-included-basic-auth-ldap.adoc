...
:page-aliases: api-manager::basic-authentication-ldap-concept.adoc, gateway::policies-included-basic-auth-ldap.adoc, api-manager::policy-mule3-ldap-security-manager.adoc, policies::policies-mule3-ldap-security-manager.adoc
...
The Lightweight Directory Access Protocol (LDAP) authentication policy specifies how to restrict access to an API using LDAP authentication mechanism. This policy is available in Omni and Mule 4 or later.
...
image::search-scope.png["A hierarchical chart displays organizational structure and user IDs"]
...
image::search-scope2.png["A hierarchical chart displays a company's organizational structure"]
...
image::ldap-verification.png["A sequence diagram that illustrates the message flow for client authentication through an LDAP Server",height=358,width=488]
