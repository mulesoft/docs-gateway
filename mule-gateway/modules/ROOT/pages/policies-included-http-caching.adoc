...
:keywords: http caching, api gateway, mule gateway, gateway, policy
// :page-aliases: api-manager::http-caching-policy.adoc, gateway::policies-included-http-caching.adoc
...
Default value: `false`
| Yes
| Persistent Cache | Configures the cache to persist between different restarts of the Mule/Flex instance.
If you are using CloudHub, ensure that you enable Object Store v2 when deploying an application.
Default value: `false`
| Yes
...
image:http-policy-cache-miss.png["A flowchart that illustrates the data flow between a web browser and a backend system"]
...
image:http-policy-cache-hit.png["A flowchart illustrating the data flow from a web browser through an API gateway to the backend and the object store"]
...
A persistent cache enables the stored entries in the cache to persist if Mule runtime engine (Mule) or Flex replica is restarted.
...
To obtain more control over the cache, the HTTP Caching policy interprets some of the HTTP directives from the https://datatracker.ietf.org/doc/html/rfc7234[RFC-7234^] protocol by taking the following headers into account:
...
If present, this header specifies the date by which the entry in the cache expires. If you specify the `max-age` directive or `s-maxage` directive, this header is ignored. You must define the header value as stated in https://datatracker.ietf.org/doc/html/rfc1123[RFC-1123^].

=== Date

The `date` header, as defined per https://datatracker.ietf.org/doc/html/rfc1123[RFC-1123^], specifies the date and time when the response is created. If not defined, the `date` header is added with the time when the request was received. This header is used in conjunction with the values defined in the `max-age` and `s-maxage` directives of the Cache-Control header.
...
For conditional request expressions, only the responses for incoming requests with HTTP methods `GET` or `HEAD` are cached by default. For conditional response expressions, only status codes specified by https://datatracker.ietf.org/doc/html/rfc7231[RFC-7231^] are cached by default.
