...
A persistent cache enables the stored entries in the cache to persist if the Mule runtime engine (Mule) or Flex replica is restarted.
...
The following command invalidates the entry with key “/my/policy" from the cache. Replace the example URL before running the command:

`curl http://<MyAppURL>/my/policy -H “myInvalidationHeader:invalidate”`

The following request invalidates all entries from the cache. Replace the example URL before running the command:
