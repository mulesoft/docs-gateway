...
If your Mule runtime engine runs inside a firewall that restricts external communication through a proxy server, you can configure the Mule Gateway to route traffic through the proxy server to API Manager.

The Mule Gateway supports the Basic, NTLM and Kerberos v5 (via SPNEGO) authentication schemes for proxied communication.


== Configure the Mule Gateway to Connect Through a Proxy Server

To configure the Mule Gateway to connect through a proxy server, you must set up your proxy server configuration in the `$MULE_HOME/conf/wrapper.conf` file.
