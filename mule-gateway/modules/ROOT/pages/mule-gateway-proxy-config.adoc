...
IMPORTANT:
Set the `useKeyTab` to `false` for the principal being authenticated (in this example, `alice`), so that it uses the credentials specified for `anypoint.platform.proxy_username` and `anypoint.platform.proxy_password`.
