...
The regex will match only against the URI path component of the request. Query parameters are ignored. For example, a regex pattern like `/api/users.*123` will match requests to `/api/users/123` but won't match a request to `/api/users?id=123`. 
