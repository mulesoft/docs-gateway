...
The regex matches only against the URI path component of the request, and query parameters aren't considered. For example, a regex pattern like `/api/users.*123` matches `/api/users/123` but won't match `/api/users?id=123`. 
