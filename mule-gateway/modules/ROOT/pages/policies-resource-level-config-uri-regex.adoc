...
image:configure-regex-resources-list.png["A list shows API endpoints and their corresponding GET and POST methods",140,150]
...
image:configure-uri-regex-example.png["A form that allows you to apply configurations to specific API methods and resources"]
...
* The root path of an API must be well defined to calculate the base path of the API. Ensure that the path attribute of the HTTP Listener in the Mule application ends with `*`. Otherwise, resource-level policy matching works as if `ignoreBasePath` is set to `false`.

=== Query Parameter Exclusion

The URI regex patterns used for resource-level policy configuration match only against the URI path component of the request. Query parameters are ignored. For example, a regex pattern like `/api/users.*123` will match requests to `/api/users/123` but won't match a request to `/api/users?id=123`. 

If your API requires policy application based on query parameter values, consider:

* Restructuring your API to use path parameters instead of query parameters for critical filtering.
* Implementing custom policy code that evaluates query parameters after the initial URI matching.
