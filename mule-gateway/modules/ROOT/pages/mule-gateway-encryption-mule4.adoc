...
See xref:api-manager::custom-policy-getting-started.adoc#setting-up-a-project-with-the-archetype[Setting up a project with the archetype] for more detailed steps to do this.
...
See xref:api-manager::offline-policy-task.adoc[Applying an Offline Policy] for more information.
