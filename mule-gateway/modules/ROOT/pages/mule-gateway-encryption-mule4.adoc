...
[NOTE]
The `![` prefix and  `]` suffix are required because they specify to Mule Runtime Engine that the enclosed value is encrypted.

When starting Mule runtime engine, you must pass your secret key, using the `anypoint.platform.encryption_key` property:
...
[CAUTION]
--
If you restart Mule without an encryption key, all policies are downloaded again, and sensitive information is not encrypted.
--
