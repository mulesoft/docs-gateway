...
:page-aliases: api-manager::openam-oauth-token-enforcement-policy.adoc, policies::policies-included-openam-oauth-token-enforcement.adoc, api-manager::policy-mule3-openam-oauth-token-enforcement-policy.adoc, policies::policies-mule3-openam-oauth-token-enforcement-policy.adoc
...
When you apply the policy, you may optionally define a space separated list of https://datatracker.ietf.org/doc/html/rfc6749[OAuth 2.0 scopes^] to be enforced by it.
OAuth 2.0 scopes are a way to further limit access to a resource protected by OAuth. You may define words like READ, WRITE, or some others that make sense in the context of your organization (such as CONTRACTOR, PUBLIC, EMPLOYEES_ONLY, etc).

=== Omni Gateway Local Mode

The OpenAM OAuth 2.0 Token Enforcement policy is not supported in Omni Gateway Local Mode.

=== Omni Gateway Connected Mode

The OpenAM OAuth 2.0 Token Enforcement policy is not supported in Omni Gateway Connected Mode.
...
* https://www.openidentityplatform.org/openam[OpenAM^] identity provider
