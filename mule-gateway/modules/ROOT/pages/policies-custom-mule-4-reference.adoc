...
<5> Description of what the policy does. Also used in API Manager’s UI. Mandatory
<6> Category to which the policy belongs. Used to group and filter policies in API Manager’s UI, any String value is valid. Mandatory
<7> Value used by the Edge to show metrics about different types of policy violations. Mandatory
<8> Deprecated property. Value should be set to ‘system’. Mandatory
<9> Whether resource level pointcuts should be enabled when applying the policy. Mandatory
<10> Deprecated property. Value should be set to ‘true’. Mandatory
...
<12> Where the policy parameters are defined. Every parameter listed here will be rendered as an expected user input in API Manager’s UI. It expects an array of values. Mandatory
