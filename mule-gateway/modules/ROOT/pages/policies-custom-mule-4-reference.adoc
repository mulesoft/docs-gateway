...
:page-aliases: policies::policies-custom-mule-4-reference.adoc, api-manager::policy-mule3-custom-policy-references.adoc, policies::policies-mule3-custom-policy-references.adoc, api-manager::policy-mule3-pointcut-reference.adoc, policies::policies-mule3-pointcut-reference.adoc

The Mule 4 engine provides powerful capabilities for building custom policies through its architecture and XML schema language.
...
supportedJavaVersions: ["8", "11", "17"]  // <4>
description: Enforces access tokens by OpenAM.  // <5>
category: Security  // <6>
violationCategory: authentication  // <7>
type: system  // <8>
resourceLevelSupported: true  // <9>
standalone: true  // <10>
...
<1> Unique ID within your organization of the policy. Required
<2> User friendly name that is used for displaying the policy name in API Manager’s UI. Required
<3> Deprecated property. Value should be set to ‘>=v1’ for now. Required
<4> Java versions supported by the policy. Required for Java 17 runtime support. Without this field, the policy won’t appear in API Manager for Java 17 runtimes. Optional
<5> Description of what the policy does. Also used in API Manager’s UI. Required
<6> Category to which the policy belongs. Used to group and filter policies in API Manager’s UI, any String value is valid. Required
<7> Value used by Edge to show metrics about different types of policy violations. Required
<8> Deprecated property. Value should be set to ‘system’. Required
<9> Whether resource level pointcuts should be enabled when applying the policy. Required
<10> Deprecated property. Value should be set to ‘true’. Required
<11> Whether policy requires information about an identity management that’s configured for the API’s Organization. Optional
<12> Where the policy parameters are defined. Every parameter listed here will be rendered as an expected user input in API Manager’s UI. It expects an array of values. Required
