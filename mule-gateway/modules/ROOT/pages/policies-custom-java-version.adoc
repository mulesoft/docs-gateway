...
. xref:policies-custom-upload-to-exchange.adoc[Upload the custom policy to Exchange].
...
* If your policy uses extensions provided by Mule API Gateway, update the version of the dependency in you `pom.xml` file to a version that supports your new Java version.
+
For an example of how to use extensions in your custom policy, see xref:policies-custom-http-transform.adoc[].
+
To find the first Java 17 compatible version of the extension, see the following table. All versions support support Java 8:
+
[width="100%", cols="a,a"]
|===
