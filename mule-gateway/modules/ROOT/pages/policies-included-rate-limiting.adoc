...
:keywords: rate limiting, api gateway, mule gateway, gateway, policy
:page-aliases: api-manager::policy-mule3-apply-rate-limiting.adoc, policies::policies-mule3-apply-rate-limiting.adoc, api-manager::policy-mule3-rate-limiting-and-throttling.adoc, policies::policies-mule3-rate-limiting-and-throttling.adoc
...
|Distributed | When enabled, the quota is shared among all nodes running the API. For a Mule runtime engine target, the runtime must be running in high-availability (HA) cluster mode for the quota to be shared; if you enable this option without HA cluster mode, each node enforces the quota independently. | Checked or unchecked
...
image::rate-limiting-configure-clusters.png["A timeline chart that tracks accepted and rejected requests for two nodes",55%]
...
[NOTE]
====
To share the quota across nodes by using the *Distributed* option, the Mule runtime engine instance must be running as part of a Mule high-availability (HA) cluster. If the runtime is not running in HA cluster mode, enabling *Distributed* has no effect: each node enforces the quota independently.
====
...
Persistence provided by ObjectStore v2 on CloudHub 2.0 is not supported.
====
