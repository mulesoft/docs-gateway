...
|Distributed | When enabled, the quota is shared among all nodes running the API. For a Mule runtime engine target, the nodes share the quota only when the runtime runs in high-availability (HA) cluster mode. Otherwise, each node enforces the quota independently. | Checked or unchecked
...
[NOTE]
====
The *Distributed* option shares the quota across nodes only when the Mule runtime engine instance runs as part of a Mule high-availability (HA) cluster. If the runtime does not run in HA cluster mode, enabling *Distributed* has no effect, and each node enforces the quota independently.
====
