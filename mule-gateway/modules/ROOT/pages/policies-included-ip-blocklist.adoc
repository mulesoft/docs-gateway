...
:keywords: ip, blocklist, validation, api gateway, mule gateway, gateway, policy
...
You can specify a DataWeave 2.0 expression to define the source IP to be used when the policy tries to determine whether the IP belongs to the restricted IP list defined in the policy.
...
|IP expression  |The DataWeave 2.0 expression to be used for extracting the IP address from this API request.
