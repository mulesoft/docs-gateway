...
:page-aliases: api-manager::org-credentials-config-mule4.adoc, api-manager::org-credentials-config-mule3.adoc, mule-gateway-org-credentials-mule3.adoc
...
+
image::environment-api-manager.png["The credentials for the environment in Mule 4",align=center]
...
+
image::orgid-api-manager.png["A form displays organization information, including names, IDs, and a client secret in Mule 4",align=center]
...
+
image::configure-autodiscovert-4-studio.png["A form that enables the input of API platform and analytics URIs", align=center]
+
If your organization is defined in the EU control plane, then you must also specify the base URL for that plane:
+
image::api-gateway-enabling-0cbca.png["A form for entering Anypoint Platform API pairing properties with fields for API platform, analytics URIs, and client credentials",align=center]
+
The same applies if you are pointing at an organization defined in a Private Cloud Edition. You would need to provide the URLs for your Private Cloud Edition installation.
