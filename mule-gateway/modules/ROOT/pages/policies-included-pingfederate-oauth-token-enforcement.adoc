...
:keywords: pingfederate oauth 2.0 token enforcement, api gateway, mule gateway, gateway, policy
:page-aliases: api-manager::policy-ping-federate.adoc, policies::policies-included-pingfederate-oauth-token-enforcement.adoc
...
xref:access-management::conf-client-mgmt-pf-task.adoc[Configure PingFederate Client Management]. If you are using xref:api-manager::configure-multiple-credential-providers.adoc[multiple IdPs], verify that the PingFederate client is properly configured in the API.
...
This enables the policy to continue working even if the connection with the control plane is lost. If you want the client application information that is written on disk to be encrypted, xref:api-manager::api-gateway-encryption-mule4.adoc[configure gateway encryption in your runtime].
...
* https://www.pingidentity.com/en/capability/single-sign-on.html[PingFederate^] identity provider
