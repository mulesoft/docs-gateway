...
. In the Inbound Header Map box, type the key-value pair for the inbound header and click image:in-header-out-header-add-icon.png["A button with a plus sign",8%,8%].
+
...
+
image:apply-header-injection-policy.png["A form allows header injection configuration"]
+
. Click *Apply*.
+
