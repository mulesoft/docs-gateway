...
In contrast, Mule Gateway protects a single Mule API, and offers two key advantages: 

* First, it's easy for Mule app developers to provide basic endpoint protection, and this protection can be assured through Anypoint API Governance. 
* Second, Mule Gateway's protections are built into the app, and are applied no matter where the app is deployed. There's no way to "get around" the gateway and invoke the API directly, because the Mule Runtime itself does the enforcement. You can configure Mule in Anypoint Runtime Manager as a CloudHub proxy application, protecting multiple backends.
