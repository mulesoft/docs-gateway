...
+
Defining a string as the `uri` or `ip-range` provides additional validations, but defining a string as `dataweave` notifies Flex Gateway that the input must be transformed. For more information about supported DataWeave transformations, see xref:gateway::policies-flex-dataweave-support.adoc[].
+
