...
* https://github.com/mulesoft/pdk-custom-policy-examples/blob/support/1.5.x/multi-instance-rate-limiting/README.md[Multi-Instance Rate Limiting Policy]: Use the Multi-Instance Rate Limiting Policy as an example of how to implement xref:policies-pdk-configure-features-rate-limiting.adoc[rate limiting] with support for multiple independent rate limit configurations using clustered mode with Redis.
