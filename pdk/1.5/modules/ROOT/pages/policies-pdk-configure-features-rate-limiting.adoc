...
* https://github.com/mulesoft/pdk-custom-policy-examples/blob/{template-policies-url-ver-var}/multi-instance-rate-limiting/src/lib.rs[`entrypoint` function] for how to configure the `RateLimitBuilder`.

* https://github.com/mulesoft/pdk-custom-policy-examples/blob/{template-policies-url-ver-var}/multi-instance-rate-limiting/src/lib.rs[`request_filter` function] for how the policy applies rate limits to incoming requests.

== Redis Shared Storage Configuration Example

For an example of how to configure Redis shared storage for testing, see the example https://github.com/mulesoft/pdk-custom-policy-examples/blob/{template-policies-url-ver-var}/multi-instance-rate-limiting/playground/config/shared-storage-redis.yaml[`playground/config/shared-storage-redis.yaml`] and https://github.com/mulesoft/pdk-custom-policy-examples/blob/{template-policies-url-ver-var}/multi-instance-rate-limiting/playground/docker-compose.yaml[`playground/docker-compose.yaml`] files.
