...
To access the streaming body state, transform the `RequestState` or `ResponseState` into the body stream state by calling the `into_body_stream_state()` method and awaiting it:
