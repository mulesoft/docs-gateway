...
When using Omni Gateway Policy Development Kit (PDK), you might encounter issues with PDK itself. See the following sections to help you resolve these issues:

* <<cargo-generate>>
* <<cli-credential>>
* <<group-id-error>>
* <<authorization-error>>
* <<implementation-build-no-file>>
* <<build-asset-files-asset-not-found>>
...
When running `make build` from the implementation project, this error sometimes occurs:
...
Make sure that you've executed `make setup` and `make release-local` and that you've published the definition asset version before building the implementation.

[[build-asset-files-asset-not-found]]
== Error Looking for the Specified Asset When Running Make Build-Asset-Files (Split Model Policy)

When running `make build-asset-files` from the implementation project, this error sometimes occurs:
