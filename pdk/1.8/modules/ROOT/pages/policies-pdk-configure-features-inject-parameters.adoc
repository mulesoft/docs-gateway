...
Omni Gateway Policy Development Kit (PDK) provides the following parameters that you can inject into the `#[entrypoint]` configuration function:
...
* `Metadata`: Provides metadata about the policy, the Omni Gateway instance, the API instance, and the Anypoint Organization. For more information about the metadata provided, see xref:policies-pdk-configure-features-metadata.adoc[].
