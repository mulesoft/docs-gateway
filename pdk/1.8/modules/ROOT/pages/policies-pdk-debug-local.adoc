...
To begin using the `make run` command to debug your custom policy, register a Omni Gateway in Local Mode. 

For the `run` command to work, a Omni Gateway `registration.yaml` file must exist in the `<root-directory>/playground/config` directory. Create this file by running the registration command in the directory, or move a `registration.yaml` file of a previously registered Omni Gateway to the directory.
...
To learn more about editing a Omni Gateway's logging configuration file, see xref:gateway::flex-local-third-party-logs-config.adoc[].

== Use the Rust Debugger

Visual Studio Code (VS Code) includes a Rust debugger that can run unit tests. You cannot use the VS Code Debugger while the policy is deployed on a Omni Gateway. To create the API mocking, you must use unit tests for the specific logic you need to debug.
