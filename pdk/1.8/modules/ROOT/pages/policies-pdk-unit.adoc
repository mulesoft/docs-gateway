...
Use `Clock::now` or `Timer::now` for time in the policy; .
...
`UnitTestBuilder::default()` is the entry point. All methods except return `self` for chaining, except `with_entrypoint` that consumes the builder and returns a `UnitTest`:
...
|Mutates the `Metadata` injected into the policy context. Call *before* any `with_*_upstream_from_authority` methods, because those methods derive the upstream service name from metadata.
