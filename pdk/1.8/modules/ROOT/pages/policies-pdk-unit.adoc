...
`pdk-unit` is a unit testing framework for Flex Gateway Policy Development Kit (PDK) custom policies.
It lets you test the full request/response lifecycle of a Flex Gateway policy without a running Envoy proxy, a WebAssembly runtime, or external infrastructure.
...
To see `pdk-unit` in action, browse the https://github.com/mulesoft/pdk-custom-policy-examples[PDK custom policy examples^] repository on GitHub.
...
=== Testing time-based behaviour (ticks)
