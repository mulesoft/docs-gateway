...
Omni Gateway Policy Development Kit (PDK) consists of:
...
Omni Gateway is built on Envoy which requires developed policies to be compatible with the event-driven https://github.com/proxy-wasm/spec[proxy-wasm^] architecture. 
...
The Mulesoft MCP Server provides `manage_flex_gateway_policy_project` and `get_flex_gateway_policy_example` tools to manage the custom policy lifecycle.

To learn more, see xref:mulesoft-mcp-server::reference-mcp-tools.adoc#policy-management[MuleSoft MCP Server Policy Management Tools].
