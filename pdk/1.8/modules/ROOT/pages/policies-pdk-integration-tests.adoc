...
For the `make test` command to work, a Flex Gateway `registration.yaml` file must exist in the `<root-directory>/tests/config` directory. Create this file by running the registration command in the directory, or move a `registration.yaml` file of a previously registered Flex Gateway to the directory.
...
| `outbound_policies` 
| A List of `PolicyConfig` instances applied to the API as outbound policies.
|===
...
To configure an alternative Flex Docker image, update your `FlexConfig::builder()` definition as in this example:
