...
Without stop iteration, a policy sends headers to the upstream before sending the body. Use stop iteration if you must process the body before the headers are sent.
...
When a PDK policy transitions to the headers-body state with into_headers_body_state(), Flex Gateway buffers the entire body for both the request or the response before you policy runs. The policy doesn't receive the headers-body state until the full body has been buffered.
