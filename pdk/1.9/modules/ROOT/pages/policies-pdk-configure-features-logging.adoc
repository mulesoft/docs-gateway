...
Omni Gateway Policy Development Kit (PDK) provides a logging mechanism that generates a log message enriched with the API instance ID, policy ID, and request ID.
...
All examples appear in the Omni Gateway logs in the following format:
----
...
For more information about viewing Omni Gateway logs, see xref:gateway::flex-gateway-monitor.adoc[].