...
To access WebSocket functionality, use `FilterBuilder` with `on_upgrade_upstream` and `on_upgrade_downstream` handlers:
...
To ensure your policy processes complete WebSocket messages regardless of how they arrive from the network.
...
. Accumulating bytes using `state.accumulate().await` when no complete frames are available.
