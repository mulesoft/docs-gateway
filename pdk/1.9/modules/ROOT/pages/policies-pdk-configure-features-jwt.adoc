...
NOTE: To view an example policy project that uses Omni Gateway Policy Development Kit (PDK)'s JWT library, see https://github.com/mulesoft/pdk-custom-policy-examples/blob/{template-policies-url-ver-var}/jwt-validation/README.md[JWT Validation Policy Example^].
...
NOTE: The JWT library is gated behind a Cargo feature. For information about enabling or disabling JWT features in your `Cargo.toml`, see xref:policies-pdk-create-project.adoc#configure-pdk-features[Configure PDK Features in Cargo.toml].

== FIPS-Compliant JWT

PDK provides the `jwt-fips` feature for FIPS-compliant JWT support. This feature delegates cryptography to the proxy-wasm host instead of bundling native RustCrypto. Because it is mutually exclusive with the default `jwt` backend, you must disable default features when using it.

To enable FIPS-compliant JWT in your `Cargo.toml`:

[source,toml]
----
# FIPS-compliant JWT only:
pdk = { version = "1.9.1", default-features = false, features = ["jwt-fips"] }

# FIPS-compliant JWT with XML Validator:
pdk = { version = "1.9.1", default-features = false, features = ["jwt-fips", "xml_validator"] }
----
...
NOTE: For an example policy that generate JWT tokens, see https://github.com/mulesoft/https://github.com/mulesoft/pdk-custom-policy-examples/blob/{template-policies-url-ver-var}/jwt-generation/README.md[JWT Generation Example Policy].

The JWT library provides a helper for creating JWT tokens. Initialize the `JwtGenerator` with your signing algorithm configuration:
...
PDK supports these signing algorithms, key lengths, and PEM file formats. You must provide the `key` as a PEM file:

[%header%autowidth.spread,cols="a,a,a"]
|===
|Signing Algorithm |Signing Key Length| PEM File Format
|RSA |256, 384, and 512| pkcs#8
|HMAC |256, 384, and 512| Plain text
|ES |256 and 384| pkcs#8
|===

The JWT generator exposes the `jwt` function to create tokens with the given headers and claims added to the JWT token:
