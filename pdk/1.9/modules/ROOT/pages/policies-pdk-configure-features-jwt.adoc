...
PDK provides the `jwt-fips` feature for FIPS-compliant JWT support. This feature delegates cryptography to the proxy-wasm host instead of bundling native RustCrypto. Because it's mutually exclusive with the default `jwt` backend, you must disable `default-features` when using it.
