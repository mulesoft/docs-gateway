...
PDK-Unit supports testing any policy built with https://github.com/proxy-wasm/proxy-wasm-rust-sdk[proxy-wasm-rust-sdk]. You can write and run unit tests for existing proxy-wasm policies without needing to migrate them to PDK first. Use this functionality to:
...
+
* Replace the `pdk-proxy-wasm-stub` target instead of `proxy-wasm`.
+
