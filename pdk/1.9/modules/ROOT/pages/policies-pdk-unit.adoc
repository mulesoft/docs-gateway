...
. Write your tests as you would for a PDK policy, but instead of calling the `with_entrypoint` method, call the method `with_context` providing a function that creates a new boxed instance of the root filter. For example:
+
[source,rust]
----
