...
. Write your tests as you would for a PDK policy. However, use the `with_context` method instead of the `with_entrypoint` method. The `with_context` method creates a boxed instance of the root filter:
+
[source,rust]
----
