= Creating a Unified-Model (Regular) Policy Project
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images

To begin developing a new custom policy with Omni Gateway Policy Development Kit (PDK), generate a new custom policy project.
...
| `--fips` | N/A | No | Creates the policy project with FIPS compliance enabled. +
Configures the PDK to use FIPS-compliant modules. To maintain FIPS compliance, make sure any additional dependencies you add are compliant.
|===
...
[[configure-pdk-features]]
==== Configure PDK Features in Cargo.toml

The JWT and XML Validator libraries are gated behind Cargo features to reduce compiled WASM size. Both features are enabled by default (`default = ["jwt", "xml_validator"]`).

If you don't require JWT or XML validation functionality in your custom policy, you can disable these features by editing the PDK dependency in your `Cargo.toml`:

[source,toml]
----
# Disable both JWT and XML Validator:
pdk = { version = "1.9.1", default-features = false }

# Keep only JWT:
pdk = { version = "1.9.1", default-features = false, features = ["jwt"] }

# Keep only XML Validator:
pdk = { version = "1.9.1", default-features = false, features = ["xml_validator"] }
----
