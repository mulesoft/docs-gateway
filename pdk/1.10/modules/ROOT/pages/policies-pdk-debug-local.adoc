...
To test the functionality of your policy, Omni Gateway Policy Development Kit (PDK) provides a pre-configured API instance located in the `<root-directory>/playground/config` directory.

The PDK Debugging Playground only supports Omni Gateway running in Local Mode in a Docker container. If you plan to deploy or policy to Omni Gateway running in Connected Mode or to a different platform, you should still first test your policy in PDK by using Omni Gateway running in Local Mode in a Docker container.
...
To begin using the `make run` command to debug your custom policy, register an Omni Gateway in Local Mode. 

For the `run` command to work, an Omni Gateway `registration.yaml` file must exist in the `<root-directory>/playground/config` directory. Create this file by running the registration command in the directory, or move a `registration.yaml` file of a previously registered Omni Gateway to the directory.

NOTE: For policies created with the Anypoint CLI PDK plugin v1.10.0 or later, if no `registration.yaml` file is provided, a registration is generated automatically on demand when you run the debugging playground.

The `make run` command only supports Omni Gateways running in Local Mode in a Docker Container. To register Omni Gateway, see:
...
To learn more about editing an Omni Gateway's logging configuration file, see xref:gateway::flex-local-third-party-logs-config.adoc[].

== Use the Rust Debugger

Visual Studio Code (VS Code) includes a Rust debugger that can run unit tests. You cannot use the VS Code Debugger while the policy is deployed on an Omni Gateway. To create the API mocking, you must use unit tests for the specific logic you need to debug.
