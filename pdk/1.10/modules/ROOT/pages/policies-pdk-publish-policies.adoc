...
For policies created with the Anypoint CLI PDK plugin v1.10.0 or later, running `make publish` or `make release` uploads a new policy definition and policy implementation to Exchange. If the policy definition is unchanged from the published version, PDK skips republishing the definition and reuses the existing one. In this case, the publication step reports the following:
...
When the definition is reused, you only need to update the policy implementation in your API:

. Edit the policy and select the new definition.
. After selecting the new definition, click the policy and check for implementation updates. After the new implementation loads, click *Update*.

To revert to the previous behavior of always publishing a new definition, set the `SKIP_UNCHANGED_DEFINITION` flag in the Makefile to `false`:
