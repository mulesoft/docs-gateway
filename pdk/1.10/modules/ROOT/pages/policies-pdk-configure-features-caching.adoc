...
From PDK v1.10.0 onward, the cache builder supports selecting between the first-in, first-out (FIFO) and Least Recently Used (LRU) eviction strategies. By default, the cache uses the FIFO strategy.
...
// ordering is only approximate across shards. A single shard is used here so
// the demo exercises true global LRU ordering deterministically.
...
From PDK v1.10.0 onward, you can configure a time-to-live (TTL) for cache entries. Use the `ttl` method on the cache builder to set a global TTL for all entries of the cache:
