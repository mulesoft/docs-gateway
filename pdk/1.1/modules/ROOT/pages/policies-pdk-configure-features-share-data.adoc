...
If the request function ended with a `Flow::Continue` and was not xref:policies-pdk-configure-features.adoc#flow-cancellation[cancelled] by upstream policies, the `RequestData::Continue` variant contains the value sent by the request function.
