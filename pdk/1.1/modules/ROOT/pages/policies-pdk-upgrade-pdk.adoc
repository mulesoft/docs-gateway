...
Flex Gateway Policy Development Kit (PDK) is composed of the following separate versioned components that must each be updated to update a custom policy project:

* Anypoint CLI PDK plugin
* PDK Rust libraries
* Anypoint Cargo plugin

All your PDK custom policies on your device use the same Anypoint CLI PDK plugin. When you create a new custom policy project, the PDK Rust libraries and Anypoint Cargo plugin versions are the same as the Anypoint CLI PDK plugin used to create the project. The PDK Rust libraries and Anypoint Cargo plugin version of an individual custom policy project are unique from other policy projects. When you upgrade your Anypoint CLI PDK plugin, you must manually upgrade PDK Rust libraries and Anypoint Cargo plugin to be compatible with your Anypoint CLI PDK plugin version.
...
The PDK library is the Rust Code library used by a specific custom policy. Different custom policies might use different PDK library versions.
...
The Anypoint Cargo plugin extends the functionality of Cargo, the Rust package manager, for use with the PDK Rust Libraries.
