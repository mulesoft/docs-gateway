...
Render policy configuration parameters dynamically by establishing conditions to make fields visible. In the following code example, the `encryptionKey` parameter only appears if `encryptBody` is `true`:
...
* You can only define single condition per parameter.
* Required properties that are conditionally rendered must have a default value defined.
