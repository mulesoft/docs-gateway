...
By defining the `enum` node in your property, you can limit the values that the user can input into it.
Depending on the amount of options listed and the field type, the UI will render it as a radio button or a dropdown field.
+
For example:
+
[source,yaml]
...
Enums properties cannot be annotated with the @visibleOn tag as specified in the section <<define-conditional-parameters>>.
Nevertheless, you can wrap the enums in an object or an array and hide it on demand.
+
For example:
+
[source,yaml]
