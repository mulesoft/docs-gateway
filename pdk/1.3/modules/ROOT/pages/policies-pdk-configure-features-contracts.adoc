...
Authenticate a request by extracting the client credentials from the request and then invoking the `ClientValidation::authenticate()` method. The `pdk::contracts` module provides the `basic_auth_credentials()` helper function to extract Basic Authentication credentials from request headers:
...
NOTE: For security, the content of `ClientSecret` is deleted after use, and the `Debug` trait does not expose the `ClientSecret` content.
...
from the https://crates.io/crates/futures[futures] crate to join both tasks. This snippet implementes a complete `#[entrypoint]` function with polling and launching:
