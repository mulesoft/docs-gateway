...
NOTE: The previous statement introduces a cancellation point after `.await`.
During a request, `.await` could never be resumed due to a xref:policies-pdk-configure-features.adoc#flow-cancellation[flow cancellation].
...
`BodyHandler::set_body()` method returns a `Result<(), BodyError>`, what means that it is fallible. Body updating can fail due to two reasons:

. `BodyError::BodyNotSent`: The current HTTP Flow does not have a body (e.g. a `GET` request).
. `BodyError::ExceededBodySize`: The new body exceeds the maximum body buffer size allowed by Envoy.
