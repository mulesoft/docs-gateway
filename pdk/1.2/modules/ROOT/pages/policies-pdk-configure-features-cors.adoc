...
. Write an `#[entrypoint]` function where `create_cors_configuration()` is the function defined in the xref:cors-configuration[] example:
+
[source,rust]
----
use pdk::*;
...
. Write a `request_filter()` function, where the CORS validation is applied to the incoming request. The `Flow` returned must contain a `Vec<(String, String)>` to pass the headers returned by `cors::Cors::check_headers()` to the `response_filter()`:
+
[source,rust]
----
...
. Write a `response_filter()` function, where the CORS validation is applied to the response. The headers returned by `cors::Cors::check_headers()` will be available from `RequestData`:
+
[source,rust]
----
