...
. Write an `#[entrypoint]` function that defines the `cors::Configuration` instance. `create_cors_configuration()` is the function defined in the xref:cors-configuration[] example:
+
[source,rust]
----
use pdk::*;
...
. Write a `request_filter()` function to validate the incoming requests by using the CORS validation methods. The `Flow` returned must contain a `Vec<(String, String)>` to pass the headers returned from `cors::Cors::check_headers()` to the `response_filter()`:
+
[source,rust]
----
...
. Write a `response_filter()` function that validates the response by using the CORS validation methods. `RequestData` contains the headers returned by `cors::Cors::check_headers()`:
+
[source,rust]
----
