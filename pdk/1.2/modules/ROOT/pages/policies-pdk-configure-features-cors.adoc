...
The result is `Err` when the validation fails. If the result is `Ok`, the struct `cors::Check` can be evaluated.
...
. Write a `configure()` function, where `create_cors_configuration()` is the function defined at xref:cors-configuration[] example:
+
[source,rust]
----
use pdk::*;
