...
By default, all policies are created as inbound policies. To transform your policy to an outbound policy, you must edit the `labels` section in the `gcl.yaml` file. There, you will have to add the `metadata/capabilities/injectionPoint: outbound` label.
