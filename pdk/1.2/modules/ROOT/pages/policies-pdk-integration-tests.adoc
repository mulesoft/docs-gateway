...
The test module contains a common configuration directory included by default:
...
`FlexConfig` can be build using the following properties:
...
If your test requires further configuration you can add a directory with the specific configuration for the test function.
|`with_api`
|The `ApiConfiguration` of the api that will be deployed to the flex instance. Can be called multiple times if your test needs multiple apis.
|`ports`
|List of ports where the `Flex` service is listening for requests. Use if any of your configuration directories included in `config_mount` directories contains a yaml exposes an API.It must coincide with the ports configured in the `spec.address` property.
|===

`ApiConfig` can be build using the following properties:
[cols="1,1"]
|===
|Property |Content
...
|`upstream`
|The reference to the service to use as an upstream.

|`path`
|Path to which the requests to the upstream will be sent.
|`port`
|Ports where the Api is listening for requests
|`policies`
|A List of `PolicyConfig` that will be applied to the api.
|===

`PolicyConfig` can be build using the following properties:
[cols="1,1"]
|===
|Property |Content
