...
. Rust programming language version {rust-ver-var} to {rust-ver-max-var}:s
...
+
.. Due to the `wasm32-wasi` compilation target used by the PDK being depricated in Rust v1.84.0, downgrade Rust to the last compatible version execute the following command:
+
[source,ssh]
----
rustup default {rust-ver-max-var}
----
