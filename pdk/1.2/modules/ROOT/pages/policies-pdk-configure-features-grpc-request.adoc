...
* Check `protobuf` crate documentation for code generation support.
Now suppose that `HelloService` gRPC services has two related messages, `HelloMessage` and `HelloResponse`. You can write
...
impl HelloClient {
   ...
   pub fn say_hello(&self, message: &HelloMessage) -> Result<HelloResponse, GrpcClientError> {
      let response = client
