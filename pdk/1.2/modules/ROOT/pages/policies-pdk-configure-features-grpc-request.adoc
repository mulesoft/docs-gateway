= Performing a Unary gRPC Request
ifndef::env-site,env-github[]
include::_attributes.adoc[]
endif::[]
:imagesdir: ../assets/images

NOTE: To view an example policy project that makes gRPC requests, see https://github.com/mulesoft/pdk-custom-policy-examples/blob/main/simple-oauth-2-grpc-validation/README.md[Simple OAuth 2.0 gRPC Validation Policy Example^].

To interact with external services, a policy can use gRPC requests. You can only make gRPC requests from the request filter or the response filter.

Custom policies only support unary gRPC requests. Policies do not support gRPC streaming.
...
== Inject the gRPC Client Parameters to Enable Requests

Depending on where you inject the `GrpcClient`, you can make requests from the request, response, or both filters.
...
. Use the following code as an example of how to write a `grpc::GrpcClient` wrapper for encapsulating requests to `HelloService`:
+
[source,rust]
----
