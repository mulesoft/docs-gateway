...
NOTE: The `pdk::hl::grpc` module contains the gRPC client functionality. If you import all PDK modules (`pdk::hl::*`), you must refer to gRPC client as `grpc::GrpcClient`.
...
The `grpc::GrpcClient` supports Protocol Buffers. To use Protocol Buffers, you must include the https://crates.io/crates/protobuf[protobuf crate] for encoding Protocol Buffers messages.


To configure Protocol Buffers:

. Add the `protobuf` crate as dependency to your `Cargo.toml` file:
+
[source,toml]
----
[dependencies]
...
. Add the `profobuf-codegen` crate to the `build-dependencies` section of you `Cargo.toml` file in order to make it available for the `build.rs` file created in the next step:
+
[source,toml]
----
[build-dependencies]
...
. Create a file named `build.rs` file in the root folder of your project. This following code is populated automatically in the `build.rs` file by `cargo` when your project is built:
+
[source,rust]
----
fn main() {
...
. Use the wrappers by call them from an `async` context:
+
[source,rust]
----
