...
In the policy's Rust source code, you can access the defined external service in the `Config` struct. For example, the following code snippet defines the `HelloService` gRPC service that has the `SayHello` method. Invoke the method by sending a gRPC message with the `grpc::GrpcClient`:
...
The `grpc::GrpcClient` includes Protocol Buffers support. To configure Protocol Buffers:
...
* Ensure the `protobuf` crate https://crates.io/crates/protobuf[ documentation] for code generation support.
