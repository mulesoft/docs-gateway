...
In PDK 1.6.0, the PDK libraries moved from the Anypoint platform to https://crates.io/crates/pdk[crates.io]. When upgrading from versions earlier than 1.6.0, you can remove the certificates needed to connect to Anypoint Platform. To upgrade policies with versions earlier than 1.6.0:
...
. Remove Anypoint registry references from your `setup` command:
.. Open the `Makefile` of your custom policy.
...
setup: registry-creds login install-cargo-anypoint ## Setup all required tools to build
	cargo fetch
----
.. Remove Anypoint registry references:
+
----
.PHONY: setup
