...
To share a status between the request and the response functions, the request function must return a `Flow::Continue` enum variant with a parameter that that you wish to pass to the response function. The response function must receive the `RequestData` parameter that contains the request function's final status and the data that was passed.
