...
Flex Gateway Policy Development Kit (PDK) enables you to both read and writer to request and response headers and bodies.

[[event-flow]]
== Event Flow

When filtering requests and responses, Proxy Wasm splits handling the header and body into two main events that must happen in a specific order. Before the policy can handle the body, the header event has to fully propagate through all policies. For example, the event flow for an API with two policies applied is:
...
* All policies must fully process the header event before reading the body. It is not possible to save data of the body on one policy and then modify the headers headers using that body data in a different policy.
...
In the policy source code, reading the body separates the header and body event. For example, if the the declaration `let body_state = headers_state.into_body_state().await;` is used to read the body, everything prior to the declaration occurs in the header event, and everything after occurs in the body event.
