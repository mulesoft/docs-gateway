...
Envoy handles the method, scheme, path, authority, and status codes as headers. PDK provides the following methods to access and modify the headers by accessing the request's `:method`, `:scheme`, `:path`, `:authority`, and `:status`. :
