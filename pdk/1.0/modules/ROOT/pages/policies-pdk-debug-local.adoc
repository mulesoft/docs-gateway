...
The Policy Development Kit (PDK) provides a pre-configured API instance located in the `<root-directory>/playground/config` directory to test the functionality of your policy.
...
The `run` command only supports Flex Gateways running in Local Mode in a Docker Container. To register a Flex Gateway, see xref:gateway::flex-local-reg-run.adoc#docker[]
