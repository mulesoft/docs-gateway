...
After you successfully xref:policies-pdk-compile-policies.adoc[compile your custom policy project] and add a <<register, Flex Gateway registration file>> to the configuration directory, you can use the `make run` command to deploy the API instance and your policy to begin testing.
...
The `make run` command only supports Flex Gateways running in Local Mode in a Docker Container. To register Flex Gateway, see xref:gateway::flex-local-reg-run.adoc#docker[]
