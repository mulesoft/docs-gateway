...
When installing Cargo Generate, you might receive a `E0405` or `E0433` error:
...
These errors occur when transitive dependencies auto update while installing Cargo Generate. Avoid this by locking the dependency versions used during the installation. Use the following command to lock the dependencies and install Cargo Generate:
