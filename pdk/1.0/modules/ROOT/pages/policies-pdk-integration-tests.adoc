...
The integration testing framework provided by PDK only supports Flex Gateways running Local Mode in a Docker container.
...
The `make test` command only supports Flex Gateways running in Local Mode in a Docker Container. To register a Flex Gateway, see xref:gateway::flex-local-reg-run.adoc#docker[]
