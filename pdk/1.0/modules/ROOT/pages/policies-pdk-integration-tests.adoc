...
PDK integration tests contain a composite of services running inside a Docker network.
...
Every configured service instance in the composite has a handle for custom interaction. The handle returns the sockets
...
Configure a `Flex` service by registering a `FlexConfig` in the `TestComposite`.
...
`Flex` service expose endpoints with external base URLs dependant on the port. To access those URLs, the
Flex service instance handle provides `external_url()` method indexed by port, for example:
...
For the example, the `say_hello` tests has the following service log:
