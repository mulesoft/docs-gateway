...
When the policy project is created with `anypoint-cli` a dummy test is placed into `<root-directory>/tests/requests.rs` file.

The `<root-directory>/tests/requests.rs` file is actually an individual module and executable. `anypoint-cli` also generates c `<root-directory>/tests/commons.rs` module
...
=== `pdk-test` library
`pdk-test` is the crate containing all the functionality for writing integration tests in PDK. It is included by the
`anypoint-cli` in the `[dev-dependencies]` section of `Cargo.toml`.
...
The next code is the default test generated by the `anypoint-cli` tool.
