...
The integration testing framework provided by PDK only supports Flex Gateways running in Local Mode in a Docker container. If you plan to deploy or policy to a Flex Gateway running in Connected Mode or to a different platform, you should still test your policy in PDK by using Flex Gateways running in Local Mode in a Docker container for initial testing.
