...
If the policy can successfully parse a Dataweave configuration parameter, it transforms it into a `pdk::script::Script` expressions. To obtain the result of the script:

. Create an evaluator.
. Call the binding function for each parameters defined in schema definition.
+
...
+
The binding methods for each variable are:
