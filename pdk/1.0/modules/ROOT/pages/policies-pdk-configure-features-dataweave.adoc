...
If the policy can successfully parse a Dataweave configuration parameter, it transforms it into a `pdk::script::Script` expressions. You must create an evaluator to obtain the result of the script.

To obtain the result of the script you’ll need to create an evaluator. Once the evaluator is created you’ll need to define the values for each of the bindings that were defined in the configuration GCL (See <link to define gcl bindings section>).  
...
NOTE: You only need to call the methods for the bindings that are defined in the schema definition. For example, if the `attributes` binding is not used, you don't need to call the `bind_attributes` method.
