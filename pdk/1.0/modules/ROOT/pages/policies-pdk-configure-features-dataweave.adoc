...
To solve the script in <<example-evaluator-code>>, the code needed to inject the `StreamProperties` and `Authentication` parameters. To simplify, PDK provides these parameters in the `DefaultBindings` object that wraps the evaluator and transforms the methods to simplify the resolution of the script. 

`DefaultBindings` provides a way to instance an evaluator given a `DataWeave` expression exposes the following methods to bind different pieces of data before evaluating the Script:
