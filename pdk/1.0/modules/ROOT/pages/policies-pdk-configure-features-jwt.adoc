= Configuring JWT Library Functions
...
JSON Web Token (JWT) is a URL-secure method of representing claims to be transferred between two parties. The JWT token contains claims encoded in a JSON object as either the payload of a JSON Web Signature (JWS) or as a JSON web encryption (JWE) structure in plain text. This enables the claims to be digitally signed and integrity protected with a message authentication code (MAC). Because the token is signed, you can trust the information and its source.
...
To extract the JWT token, the library provides a `TokenProvider` enum that has to variants:
...
Both providers implement the following trait:
...
. Use the following code example the `tokenExtractor` to extract the custom JWT token with the defined expression:
+
[source,Rust]
----
use anyhow::Result;
...
Because, the `SignatureValidator` confirms that the policy configurations is correct during initialization, initializing the `SignatureValidator` in the `#[entrypoint]` function throws early error when the policy is applied rather than when the API receives a request.
...
Because JWT tokens typically contain a set of standard claims that can be validated, the JWT library provides a small set of claim validators in the `ClaimValidator` enum:
...
The `ScriptClaimValidator` takes two properties:
...
The `ScriptClaimRule` accepts two parameters:
...
In case the signature is invalid or you choose not to validate the signature, PDK provides additional methods to parse the JWT claims by using the `JWTClaimsParser`. This structure provides a “parse” method that returns a `Result<JWTClaims, JWTError>`.
...
To access any claim, the `JWTClaims` struct exposes the following methods to access standard JWT claims:
