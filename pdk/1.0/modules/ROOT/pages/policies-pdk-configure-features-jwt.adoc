...
To extract the JWT token, PDK provides a `TokenProvider` struct that has the `bearer` function to extract tokens that come with the format `Bearer <token>` in the authorization header. The following code demonstrates how to use the `bearer`
...
If you are using a JWT token from a different source or with a different format, you can implement Rust logic to get any other part of the request. Use a DataWeave parameter to configure extraction to create a policy where you can dynamically change the JWT format.
...
* Has the correct custom `role` claim by using a custom validator configured as DataWeave expression policy parameter.
...
The the above example, the custom `role` claim is defined by a DataWeave expression configured as an input parameter in the `gcl.yaml` schema definition as follows:
