...
JSON Web Token (JWT) is a URL-secure method of representing claims to be transferred between two parties. The JWT token contains claims encoded in a JSON object as either the payload of a JSON Web Signature (JWS) or as a JSON web encryption (JWE) structure in plain text. This enables the claims to be digitally signed and protected with a message authentication code (MAC). Because the token is signed, you can trust the information and its source.
...
In case the signature is invalid or you choose not to validate the signature, PDK provides additional methods to parse the JWT claims by using the `JWTClaimsParser`. This structure provides a `parse` method that returns `Result<JWTClaims, JWTError>`.
