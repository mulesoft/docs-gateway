...
=== Extract a Token with the Bearer Provider
...
=== Extract a Token with the Custom Provider
...
=== Validate Exp (Expiration) Claims
...
. Initialize the `ScriptClaimValidator` in the `#[entrypoint]` function using the configured DataWeave expression for the claim rule, and inject the validator into the wrapped function:
+
[source,rust]
----
#[entrypoint]
