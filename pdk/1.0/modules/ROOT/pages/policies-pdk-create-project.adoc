...
To create a custom policy project, run the PDK CLI `make create` command, replacing `<my-custom-policy>` with the name of your new policy:
...
Running the `make create` command creates a custom policy project with the specified name. The project appears in a new directory or a directory specified using the `--output-dir` flag. The following tutorials refer to this as your project's root directory.
...
The `make create` command supports the following flags:
