...
For the wrapped functions, you may inject some of the above parameters and some additional parameters:
...
RequestState: Provides access to the different stages of request filtering to operate over the incoming requests headers and body. See Read and Write the Headers and Read and Write the body sections.
HttpClient: Used for performing HTTP Calls. See Perform an HTTP call section.
DefaultBindings: Provides an easy way to bind relevant information to Dataweave evaluators. See DefaultBindings subsection from Use dataweave expressions.
StreamProperties: It is used to share properties with other policies that process the same request. See StreamProperties section.
Authentication: It is used to read or propagate Authentication data shared with other policies. See Authentication section.

* `on_response` wrapped functions:
ResponseState: Provides access to the different stages of request filtering to operate over the upstream responses headers and body. See Read and Write the Headers and Read and Write the body sections.
HttpClient: Used for performing HTTP Calls. See Perform an HTTP call section.
RequestData: Used to share data between request and response filters. See Share Data Between Requests and Responses section.
DefaultBindings: Provides an easy way to bind relevant information to Dataweave evaluators. See DefaultBindings subsection from Use dataweave expressions.
StreamProperties: It is a structure used to share properties with other policies that process the same request. See StreamProperties section.
Authentication: It is used to read or propagate Authentication data shared with other policies. See Authentication section.
