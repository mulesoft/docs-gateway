...
PDK provides the following parameters that you can inject into the #[entrypoint] configuration function:
...
The wrapped functions can receive the following parameters:
...
If your wrapped functions only receives the above parameters, the lambda is not required in the wrapper. For example, if the signature of the function is:
...
If your wrapped functions require other parameters, such as the configuration parameters defined in the `#[entrypoint]` function. To pass the parameters, define the lambda and send a variable reference, for example:
