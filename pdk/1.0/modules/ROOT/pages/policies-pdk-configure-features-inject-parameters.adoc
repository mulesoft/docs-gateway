...
By default, the #[entrypoint] configuration function receives the following injectable parameters:
...
* `Metadata`: Provides metadata about the Policy, the Flex instance, the API instance and the Anypoint Organization. For more information about the metadata provided, see <<metadata>>.
...
You can also directly inject the `HttpClient` and `StreamProperties` parameter into the wrapped functions. Choose where to inject these parameters based on where they are used. For example, because the HTTP call must be performed inside the wrapped functions, it is best to directly inject. However, if you want to use the `HttpClient` in both `on_response` and `on_request` functions, you should inject the `HttpClient` in the `#[entrypoint]`. 

If the parameters require some initial configuration steps that must only be performed once. It is best to inject the parameters into the `#[entrypoint]` to complete the configuration steps to increase the performance of the wrapped functions.

== on_request and on_response Wrapped Functions

The wrapped functions receive the predefined parameters:

* `StreamProperties`
* `HttpClient`
* `RequestState`: Provides access to the different stages of request filtering to operate over the incoming requests headers and body. `RequestState` is only availbile in `on_request` functions. To read and write to the `RequestState`, see xref:policies-pdk-configure-features-headers.adoc[].
* `ResponseState`: Provides access to the different stages of request filtering to operate over the upstream responses headers and body. `ResponseState` is only availbile in `on_response` functions. To read and write to the `ResponseState`, see xref:policies-pdk-configure-features-headers.adoc[].
...
The wrapped functions may require other parameters, such as the configuration parameters defined in the `#[entrypoint]` function. To pass the parameters, define the lambda and send a variable reference, for example:
...
If your wrapped functions only receives the predefined parameters, the lambda is not required in the wrapper. For example, if the signature of the function is:
...
To inject metadata into the wrapped functions:

. Add the `metadata` as a parameter in the entrypoint configure function and inject it into the desired wrapped functions:
+
[source,Rust]
----
#[entrypoint]
...
. Add the `metadata` parameter to the wrapped function:
+
[source,Rust]
----
