...
For best practice, inject the parameters only where they must be used. However, if the parameters require initialization steps that only need to be performed once when the policy is applied, it is best to inject the parameters into the `#[entrypoint]` to complete the configuration steps to increase the performance of the wrapped functions.
