...
If you use a network proxy to connect to the internet, it may be necessary to define the network proxy for the PDK tools.

Besides Docker, all tools downloaded in the xref:policies-pdk-prerequisites.adoc[PDK prerequisites] accept the `HTTP_PROXY` and `HTTPS_PROXY` environment variables. 
...
Configure the environment variables by running following relevant command after replacing the variables with your information:
...
The PDK tools may visit the following domains during execution:
...
Docker may visit the following domains during execution:
