...
* Review <<policy-template>> and <<wrapped-functions>> to learn about the policy's source code structure before adding <<provided-rust-source-code, additional policy capabilities>>.
...
Even though custom policies runs on a single-threaded environment, every filter (i.e. a couple of request and response `async` functions) runs actually as a concurrent task.
...
is a potential task interruption point. When an `.await` is invoked, the underlying async runtime is able to send the current task
