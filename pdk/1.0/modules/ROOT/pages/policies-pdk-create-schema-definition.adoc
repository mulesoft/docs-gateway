...
Protect sensitive data, such as passwords or certificates, by adding the `security:sensitive` certificate:
...
Clicking *Show* revels the obscured data. However, this is not available for previously stored data. For example, if a user configures the example *Key* parameter and applies the policy, they cannot view the parameter if they edit the policy's configuration in the future.
