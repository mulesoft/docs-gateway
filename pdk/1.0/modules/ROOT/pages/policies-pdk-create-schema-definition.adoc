...
* `payload`: The body of the imcoming request. Define the `mimeType` as either `text` or `json` to set how to read the payload. By default, `mimeType` is set to `text`. To restrict the use of the `payload` variable, you must exclude the payload variable, for example:
+
[source,yaml]
----
bindings:
...
* `attributes`: Attributes of the incoming request such as headers. Set `attributes` as `true` to enable the policy to read the attributes of the incoming request. By default, attributes is set to `true`.
...
If the user's expression contains bindings that are not defined in the policy's schema, the configuration of the policy fails. For example, if the user expressions is `[authentication.principal]` and `authentication` is not set to true, an error similar to the following occurs:
