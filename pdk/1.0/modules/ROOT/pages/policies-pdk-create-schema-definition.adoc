...
Toggling *Show* reveals any new data currently being entered. However, you cannot reveal previously stored data. For example, if a user configures the example *Key* parameter and applies the policy, they cannot view the parameter if they edit the policy's configuration in the future.
...
+
* `dataweave`: specifies the expected text is a DataWeave variable. Notifies Flex Gateway that the input must be transformed. For more information about DataWeave expressions, see <<dataweave-expressions>>.
* `service`: specifies the expected text is an URI for which a Flex service is automatically created by the PDK to be able to perform an HTTP Call to it from the policy.
* `ip-range`: Specifies the expected text is a range or IPs. Provides additional validation on the input.
* `uri`: Specifies the expected text is an URI. Provides additional validation on the input. Should be used when the value is not being used for an HTTP Call from the policy.
+
...
* `vars`: Specifies the names of the variables that are available for the policy to use. By default, `vars` is empty. If the user inputs a variable that is not defined in the schema, the policy evaluates the variable as `null`.

For more information about the supported expression bindings, see xref:dataweave::dataweave-variables-context.adoc[].

If the user's expression contains bindings that are not defined in the policy's schema, policy configuration fails. For example, if the user expression is `[authentication.principal]` and `authentication` is not set to `true`, an error similar to the following occurs:
