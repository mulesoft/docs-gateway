...
----
+  
T`make build` propagates the changes to `src/generated/config.rs` and generates the policy's target files. Do not run `make build` if you do not want to override your policy's target file. For more information about compiling your policy, see xref:policies-pdk-compile-policies.adoc[].
