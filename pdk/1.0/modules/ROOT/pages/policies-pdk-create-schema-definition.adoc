...
Similar to the policies listed in the xref:gatewaypolicies-included-directory.adoc[], custom policies have configuration parameters. Use the configuration parameters to improve the usability of your custom policy and support a wider range of API configurations.
...
The `dataweave` variable supports the following expressions bindings:
...
* `payload`: The body of the imcoming request. Define the `mineType` as either `text` or `json` to set how to read the payload. the default `mimeType` is `text`. However, you must include the payload variable, for example, the following schema definition fails when applied to the API instance:
+
[source,yaml]
----
bindings:
...
* `attributes`: Attribute of the incoming request such as headers. Set `attributes` as `true` to enable the policy to read the attributes of the incoming request. By default, attributes is set to `true`.
* `authentication`: Authentication information of the incoming request. Set `authentication` to `true`to enable the policy to read authenticaton details of the incoming request. By default, `authentication is set to false.
* `vars`: Specifies the names of the variables that are available for the policy to use. By default, `vars` is empty. If the user inputs variables that are not defined in the schema, they evaluate to null.

For more information about the supported expression bindings, see xref:dataweave::dataweave-variables-context.adoc[].

If the user's expression contains bindings that are not defined in the policy's schema, the configuration of the policy fails. ßFor example, the user expressions is `[authentication.principal]` and `authentication` is not set to true, an error similar to the following occurs:
----
...
When evaluating the DataWeave expression in your policy you must provide the script to resolve the expression. To learn how to implement DataWeave expressions in your Rust source code, see xref:policies-pdk-configure-features.adoc#eus-dataweave-expressions[Implement DataWeave Expressions].
