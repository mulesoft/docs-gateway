...
+
* `enum`: Specifies the input as an Enum parameters. In the API Manager UI, Enum parameters appear as radio buttons when there are three or less Enum values and as a drop-down selection when there are more than three Enum values.
+
...
pub struct Config {
    ...
    pub enum JWTOrigin {
        #[serde(alias = "httpBearerAuthenticationHeader")]
        Bearer,
        #[serde(alias = "customExpression")]
        CustomExpression,
    }
