...
Toggling *Show* revels any new data currently being entered. However, this is not available for previously stored data. For example, if a user configures the example *Key* parameter and applies the policy, they cannot view the parameter if they edit the policy's configuration in the future.
...
+
Defining a string as the `uri` or `ip-range` provides additional validations, but defining a string as `dataweave` notifies Flex Gateway that the input must be transformed. For more information about dataweave expressions, see <<dataweave-expressions>>.
+
...
The `dataweave` format for `string` type parameters specifies the input parameter as a dataweave expression. For more information about what DataWeave transformations are, see xref:gateway::policies-flex-dataweave-support.adoc[].

The supported bindings are: 
* `payload`: by defining the mimeType you’ll be able to read the payload as ‘text’ or ‘json’. If the mimeType entry is omitted the type will default to text. Furthermore, if  the payload entry is omitted entirely, the payload will not be bound and expressions using it will be rejected during policy deployment state, for example:
 bindings:
   attributes: true
   authentication: false
   vars: []
* `attributes`: true or false. By setting this value you’ll be able to read attributes of the requests or responses such as the headers. If the entry is set to false it won’t be bound and expressions using it will be rejected during policy deployment state.
authentication: true or false. By setting this value you’ll be able to read the authentication object. If the entry is set to false it won’t be bound and expressions using it will be rejected during policy deployment state.
* `vars`: specify the names of the vars that are available for usage in the expression. If the list is empty, expressions using the `vars` binding will be rejected. If the expression uses other vars not defined here they will be evaluated as null. 

When evaluating the dataweave expression in your policy you’ll need to provide the script with the appropriate information to be able to resolve these bindings. For more information check the “Use dataweave expressions“ section of Implementing your custom policy features in rust.


When using dataweave variables in your policy schema, the default binding type is `text`. You can over

If the input expression contains bindings that are not specified in the policy's schema, the configuration of the policy fails. If we submit the expression "#[authentication.principal]" to the field of the example, the policy will fail since “authentication” was not bound. The error appears similar to the following:
...
| `dataveave` string | `pdk::api::script::Script`
| `number` | `f64`
| `integer` | `i64`
| `boolean` | `bool`
...
pub struct Config {
    ...
    pub token_extractor: pdk::api::script::Script,
}
----
|===
