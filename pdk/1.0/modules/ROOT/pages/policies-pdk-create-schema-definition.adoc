...
* `service`: Specifies the input parameter is an URI and automatically creates a Flex Gateway service to enable HTTP calls from the policy.
* `ip-range`: Specifies the expected text is a range or IPs and provides additional validation on the input.
* `uri`: Specifies the expected text is an URI and provides additional validation on the input. Only use `uri` when the uri is not a service making HTTP calls from the policy.
+
