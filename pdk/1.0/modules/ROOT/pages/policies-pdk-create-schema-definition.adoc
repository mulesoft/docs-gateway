...
Make policy configuration render dynamically by establishing conditions to make fields visible:
...
properties:
    ...
    conditioned:
        ...
        "@rendering":
            "@visibleOn":
                - property: conditionant
                  value: "show"
----

When applying the policy, the fields will be hidden or visible depending on other property values. Required properties that are conditionally rendered must have a default value specified.

The following restrictions apply to conditional properties:
- Enum fields cannot be conditionally renders, they are always visible
- A single condition can be specified per field.
- Only root-level properties can be conditioned. Inner properties of arrays or objects do not support conditional rendering. 
