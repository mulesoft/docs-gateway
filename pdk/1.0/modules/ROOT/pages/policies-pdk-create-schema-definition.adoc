...
* `payload`: The body of the imcoming request. Define the `mineType` as either `text` or `json` to set how to read the payload. the default `mimeType` is `text`. However, you must include the payload variable. The following schema configuration fails when applied to the API instance:
+
[source,yaml]
----
bindings:
