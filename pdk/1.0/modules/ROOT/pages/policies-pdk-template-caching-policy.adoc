...
Before using a custom policy template, ensure that you have installed the xref:policies-pdk-prerequisites.adoc[Policy Development Kit prerequisites].
...
. If the request is outside of business hours and if a response for the path has not been previously cached, the policy caches the response. 
. The policy returns the cached response for all following requests to the path outside of business hours. 
. When business hours resume, the policy clears the cache and is ready to cache new responses during the next period of non-business hours.
...
. Replace the `definition/gcl.yaml` file with the following snippet:
+
