...
To create a Split-Model project, complete the following tasks:
...
* `make publish`: Publishes the policy definition asset in Exchange. Since the publish goal is intended to publish a policy asset in development, the _assetId_ and name published will explicitly say `dev`, and the versions published will include a timestamp at the end of the version. For example:
...
* `make release-local`: Publishes the policy definition as a release asset to the local cache (no actual release to Exchange is made). You can override it. This target is useful if you are also developing the policy implementation.
...
NOTE: If your implementation and definition projects have different names, link them by ensuring the `definition_asset_id.name` field in the implementation project's `Cargo.toml` file matches the asset ID in the definition project's `exchange.json` file.
...
* `implementation_asset_id`: The asset ID that will be used to release the policy.
