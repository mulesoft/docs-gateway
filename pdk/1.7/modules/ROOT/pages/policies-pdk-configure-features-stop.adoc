...
To send an early response during the `on_response` phase, use the `send_response` method of the `ResponseHeadersState` to return a `Response` object:
...
The `send_response` method enables you to to add a body to responses that don't already have one. Sending an early response using the `send_response` method terminates the execution flow. You can't perform any other operation after sending an early response:
