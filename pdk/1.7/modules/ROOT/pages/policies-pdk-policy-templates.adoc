...
* https://github.com/mulesoft/pdk-custom-policy-examples/blob/{template-policies-url-ver-var}/ip-filter/README.md[IP Filter Policy Example]: Use the IP Filter Policy as an example of how to implement IP-based access control using PDK's xref:policies-pdk-configure-features-ip-filter.adoc[IP Filter feature], supporting both individual IP addresses and CIDR ranges for allowlist/blocklist filtering with configurable header extraction.
