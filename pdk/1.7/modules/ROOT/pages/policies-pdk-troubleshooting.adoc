...
When running `make build` from the implementation project, the following error might occur:
...
Ensure that you have executed `make setup`, `make release-local`, and published the definition asset version before building the implementation.

[[build-asset-files-asset-not-found]]
== Error Looking for the Specified Asset When Running Make Build-Asset-Files (Split Model Policy)

When running `make build-asset-files` from the implementation project, the following error might occur:
